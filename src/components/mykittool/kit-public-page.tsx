      ...
      <div className="fixed inset-0 bg-[#0a0a0c] z-[9999] flex flex-col items-center justify-center p-6 text-center gap-10">
        ...
        <div className="space-y-3">
          ...
          <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">
            Verify the ID or ensure the host is online
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() => (window.location.href = "/html-to-url")}
          className="h-14 px-10 rounded-2xl border-white/10 bg-white/5 text-white font-black uppercase text-[10px] tracking-widest shadow-2xl"
        >
          Back to Studio
        </Button>
      </div>
    ...
    <div className="fixed inset-0 bg-white dark:bg-[#0a0a0c] z-[9999] flex flex-col">
      {isHtmlTag ? (
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#060608] relative">
          <div className="p-6 border-b border-white/5 bg-background flex items-center justify-between z-10 backdrop-blur-xl">
            ...
            <Button
              onClick={handleCopy}
              className="h-10 px-6 rounded-xl bg-white text-black font-black uppercase text-[9px] tracking-widest shadow-xl"
            >
              {isCopied ? (
          ...
          <div className="flex-1 overflow-auto custom-scrollbar p-8 sm:p-12">
            <pre className="max-w-5xl mx-auto font-mono text-xs text-foreground/80 leading-relaxed whitespace-pre-wrap break-all bg-background p-10 rounded-[2.5rem] border border-white/5 shadow-inner">
              {html}
            </pre>
          </div>
        </div>

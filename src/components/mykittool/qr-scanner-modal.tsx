...
export function QrScannerModal({ isOpen, onClose }: QrScannerModalProps) {
      ...
      Html5Qrcode.getCameras()
        .then((devices) => {
          if (devices && devices.length > 0) {
            ...
            setSelectedCameraId(
              backCamera ? backCamera.id : formattedCameras[0].id,
            );
          } else {
  ...
  const toggleTorch = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Hardware Block",
        description: "This device does not support torch control.",
      });
    }
  };
  ...
  const handleOpenLink = () => {
    if (scanResult && isUrl(scanResult)) {
      window.open(
        scanResult.startsWith("http") ? scanResult : `https://${scanResult}`,
        "_blank",
      );
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="!top-16 !translate-y-0 flex max-h-[calc(100vh-6rem)] max-w-4xl flex-col overflow-hidden border-white/20 p-0 text-foreground outline-none [&>button.absolute]:hidden">
        <DialogHeader className="flex h-11 shrink-0 flex-row items-center justify-between border-b border-white/10 px-4 py-0">
          ...
          <button
            onClick={handleClose}
            className="w-10 h-10 rounded-xl bg-background/50 border border-white/10 flex items-center justify-center text-foreground/50 hover:text-foreground transition-all"
          >
                ...
                <div className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <Select
                      ...
                      disabled={isInitializing || isProcessingFile || !isLive}
                    >
                ...
                <div className="pt-8 border-t border-white/5 space-y-6">
                  <div className="flex items-start gap-4 p-5 rounded-[2rem] bg-primary/5 border border-primary/10">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest leading-relaxed text-white">
                        Decoding occurs strictly in local memory. Visual
                        identifiers never leave your browser.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full max-w-sm mx-auto relative aspect-square rounded-[3rem] overflow-hidden border-2 border-primary/20 bg-black/40 shadow-[0_0_50px_-10px_rgba(59,130,246,0.3)]">
                  ...
                  {isPaused && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex flex-col items-center justify-center gap-3 animate-in fade-in">
                      ...
                      <p className="text-[10px] font-black uppercase text-amber-500 tracking-[0.4em]">
                        Paused
                      </p>
                    </div>
                  )}
                  ...
                  {(isInitializing || isProcessingFile) && (
                    <div className="absolute inset-0 bg-[#060608]/80 backdrop-blur-xl flex flex-col items-center justify-center gap-6 z-20">
                      ...
                      <p className="text-[10px] font-black uppercase text-primary tracking-[0.3em]">
                        Checking
                      </p>
                    </div>
                  )}
                  ...
                  {error && (
                    <div className="absolute inset-0 bg-[#060608]/95 backdrop-blur-2xl flex flex-col items-center justify-center gap-6 p-10 text-center z-30">
                      ...
                      <Button
                        variant="outline"
                        onClick={handleReset}
                        className="h-12 px-8 bg-secondary/50 border-white/10 text-[9px] font-black uppercase rounded-xl tracking-widest"
                      >
                        Reset
                      </Button>
                    </div>
            ...
            <div className="p-6 sm:p-12 w-full flex flex-col items-center gap-10 bg-[#0a0a0c]">
              <div className="w-full max-w-2xl space-y-10 animate-in zoom-in-95 duration-500">
                <div className="p-10 rounded-[3.5rem] bg-primary/5 border border-primary/20 space-y-10 shadow-2xl relative overflow-hidden group/result">
                  ...
                  <div className="flex items-center justify-between border-b border-white/5 pb-8 relative z-10">
                    <div className="space-y-1">
                      ...
                      <h3 className="text-lg font-bold text-foreground/40 uppercase tracking-widest">
                        Binary Logic Matrix
                      </h3>
                    </div>
                        ...
                        >
                          <div className="flex items-center gap-4 min-w-0">
                            ...
                            <div className="min-w-0 overflow-hidden">
                              <p className="text-[9px] font-black uppercase text-foreground/20 tracking-widest">
                                {item.label}
                              </p>
                              <p
                                className={cn(
                          ...
                          <div className="flex items-center gap-3">
                            {item.isSecret && (
                              <button
                                ...
                                className="w-10 h-10 rounded-xl bg-background/50 border border-white/5 flex items-center justify-center text-foreground/20 hover:text-primary transition-all"
                              >
                                {showWifiPassword ? (
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                isCopied === item.label
                                  ...
                                  : "bg-background/50 border border-white/5 text-foreground/20 hover:text-primary",
                              )}
                            >
                              {isCopied === item.label ? (
                  ...
                  <div className="flex flex-col gap-4 relative z-10">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-8 border-t border-white/5 mt-4">
                      <Button
                        variant="outline"
                        onClick={handleReset}
                        className="h-14 border-white/10 bg-secondary/50 hover:bg-secondary rounded-2xl text-[10px] font-black uppercase tracking-widest text-foreground/40 transition-all"
                      >
                ...
                <div className="flex items-center justify-center gap-3 opacity-20">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-[0.6em]">
                    Hardware End Point Protocol Active
                  </span>
                  <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                </div>
              </div>

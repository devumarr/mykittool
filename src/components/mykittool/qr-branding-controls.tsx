...
export function QrBrandingControls({
  state,
  updateState,
}: QrBrandingControlsProps) {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        toast({
          ...
          description: "Standard limit is 5MB.",
        });
        return;
      }
      ...
      reader.onloadend = () => {
        ...
        toast({
          ...
          description: `Brand ${field === "logo" ? "icon" : "imagery"} integrated.`,
        });
      };
  ...
  const handleAiBackgroundGen = async () => {
    ...
    try {
      ...
      toast({
        title: "AI Background Ready",
        description: "Artistic imagery generated and optimized.",
      });
    } catch (e) {
      toast({
        variant: "destructive",
        title: "Generation Failed",
        description: "Imagen engine temporarily busy.",
      });
    } finally {
    ...
    <Card className="glass-card shadow-2xl border-border relative overflow-hidden">
      <CardHeader className="border-b border-border bg-secondary/30 py-8">
        <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
          <Shield className="w-5 h-5 text-primary" /> Brand Studio
        </CardTitle>
      </CardHeader>
            ...
            {state.logo && (
              <div className="space-y-3">
                <Slider
                  ...
                  step={1}
                  onValueChange={(val) =>
                    updateState({ logoSize: val[0] / 100 })
                  }
                />
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={cn(
                    "flex-1 h-10 rounded-xl border flex items-center justify-center gap-1.5 text-[8px] font-black uppercase tracking-wider transition-all px-2",
              ...
              {state.backgroundMode === "auto" ? (
                <div className="p-4 rounded-2xl bg-primary/10 border border-primary/20 flex items-center gap-4 animate-in fade-in zoom-in duration-500">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[9px] text-foreground/40 font-medium">
                      System has set density to 25% for peak reliability across
                      all scanners.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-500">
                  ...
                  <Slider
                    ...
                    step={1}
                    onValueChange={(val) =>
                      updateState({ backgroundOpacity: val[0] / 100 })
                    }
                  />
                </div>
              )}

...
export function QrStylingControls({
  state,
  updateState,
}: QrStylingControlsProps) {
  return (
    <Card className="glass-card shadow-2xl border-border relative overflow-hidden">
      <CardHeader className="border-b border-border bg-secondary/30 py-8">
        <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
          <Palette className="w-5 h-5 text-primary" /> Styling
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-10 space-y-10">
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-4">
            <Label className="text-[9px] uppercase font-black text-foreground/40 tracking-[0.2em]">
              DOT STYLE
            </Label>
            <Select
              value={state.dotStyle}
              onValueChange={(val) => updateState({ dotStyle: val as any })}
            >
          ...
          <div className="space-y-4">
            ...
            <Select
              value={state.cornerStyle}
              onValueChange={(val) => updateState({ cornerStyle: val as any })}
            >
              <SelectTrigger className="h-12 bg-secondary border-border rounded-xl text-foreground">
                <SelectValue />
              </SelectTrigger>

...
export function QrFormSection({ state, updateState }: QrFormSectionProps) {
  ...
  const handleAiRefine = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Text optimized for QR efficiency.",
      });
    } catch (e) {
      toast({
        variant: "destructive",
        title: "Refinement Failed",
        description: "AI engine temporarily unavailable.",
      });
    } finally {
      setIsRefining(false);
    }
  ...
  const handleAiSuggestType = async () => {
    ...
    try {
      ...
      toast({
        title: "Type Optimized",
        description: `Switched to ${result.type} profile.`,
      });
    } catch (e) {
      toast({
        variant: "destructive",
        title: "Suggestion Failed",
        description: "Could not analyze content type.",
      });
    } finally {
    ...
    <div className="space-y-10">
      <Card className="glass-card border-border shadow-2xl overflow-hidden">
        ...
        <CardContent className="pt-10">
          <Tabs
            value={state.type}
            onValueChange={(val) => updateState({ type: val as any })}
            className="w-full"
          >

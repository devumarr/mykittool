...
const DEFAULT_HELP: HelpContent = {
  ...
  privacy:
    "This tool operates 100% locally in your browser memory. No data is logged or transmitted.",
};
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-secondary border border-border text-[9px] font-black uppercase tracking-widest text-primary hover:bg-primary/5 hover:border-primary/20 transition-all shadow-sm active:scale-95">
          <HelpCircle className="w-3.5 h-3.5" />
          Get Help
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-1xl w-[calc(100%-32px)] md:w-full border-white/10 p-0 overflow-hidden outline-none flex flex-col max-h-[85vh]">
        <DialogHeader className="p-5 sm:p-8 border-b border-white/5 bg-primary/5 relative shrink-0">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div className="space-y-1 min-w-0">
              ...
              <p className="text-[9px] sm:text-[10px] font-black text-primary uppercase tracking-[0.2em] truncate">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 space-y-10 bg-transparent">
          <div className="space-y-3">
            <h4 className="text-[11px] font-black uppercase tracking-widest text-foreground/40">
              Capability Matrix
            </h4>
            <p className="text-[14px] sm:text-[15px] font-medium text-foreground/70 leading-relaxed">
              {content.description}
            </p>
          </div>
            ...
            <div className="space-y-4">
              {content.steps.map((step, i) => (
                <div key={i} className="flex gap-4 sm:gap-5 group">
                  ...
                  <p className="text-sm font-medium text-foreground/60 leading-relaxed pt-0.5">
                    {step}
                  </p>
                </div>
              ))}
              ...
              <ul className="space-y-3">
                {content.tips.map((tip, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-[10px] sm:text-[11px] font-bold text-foreground/40 leading-relaxed uppercase">
                      {tip}
                    </span>
                  </li>
                ))}
        ...
        <div className="p-4 sm:p-6 bg-secondary/30 border-t border-white/5 flex items-center justify-between shrink-0">
          <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.3em] text-foreground/20">
            MY KIT TOOL • TECHNICAL DOCS
          </span>
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[8px] font-black text-emerald-500 uppercase tracking-widest">
            ...
            <span className="hidden xs:inline">Verified Protocol</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>

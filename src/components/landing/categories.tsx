    ...
    <section className="w-full py-16 sm:py-20 bg-opacity-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-purple-500">
            Browse by Category
          </h2>
          <p className="mt-2 text-base text-gray-400 max-w-xl mx-auto">
            Find the right tool for the job from our comprehensive categories.
          </p>
        </div>
          ...
          {categories.map((category) => (
            <div key={category.name}>
              <Link href={category.href} className="group block">
                <div
                  className={`relative h-80 rounded-2xl overflow-hidden p-6 flex flex-col justify-between items-center text-center text-white bg-gradient-to-br ${category.gradient} transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-2xl`}
                >
                  <div className="absolute inset-0 bg-background group-hover:bg-background/10 transition-colors duration-300"></div>

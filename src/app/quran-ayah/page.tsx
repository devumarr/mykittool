...
export default function QuranAyahPage() {
  ...
  const fetchAyah = useCallback(
    async (reference: string | number = "random") => {
      setIsLoading(true);
      setLocalError(null);
      try {
        ...
        if (transJson.code !== 200) {
          throw new Error(
            "Reference not identified. Ensure correct Surah:Ayah format (e.g. 2:255).",
          );
        }
        ...
        if (reference !== "random") {
          toast({
            title: "Reference Isolated",
            description: `Surah ${data.surah.englishName} : ${data.numberInSurah}`,
          });
        }
      } catch (err: any) {
        console.error(err);
        ...
        toast({
          ...
          description: err.message || "Could not retrieve ayah matrix.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleCopy = (text: string, label: string) => {
    ...
    toast({
      title: `${label} Copied`,
      description: "Linguistic matrix saved to clipboard.",
    });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional linguistic discovery engine. Explore the Quranic
              matrix through random synthesis or clinical reference lookups with
              1:1 textual fidelity in Arabic, English, and Urdu.
            </p>
          </div>
            ...
            <CardContent className="pt-10 space-y-8">
              <form onSubmit={handleSearch} className="space-y-4">
                ...
                <Button
                  ...
                  className="w-full h-12 bg-primary text-white font-black text-[9px] uppercase tracking-widest rounded-xl shadow-xl shadow-primary/30"
                >
                  {isLoading ? (
                ...
                <p className="text-[9px] text-foreground/20 font-bold uppercase tracking-widest text-center">
                  Enter Surah:Ayah or Ayah Number
                </p>
              </form>
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Linguistic queries are processed strictly in your browser
                session. The studio does not log or transmit your reading
                history to any database.
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="w-full space-y-12 animate-in zoom-in-95 duration-500">
                  ...
                  <div className="text-center space-y-8">
                    <div className="flex flex-col items-center gap-4">
                      <h3 className="text-3xl sm:text-5xl font-bold text-primary font-headline tracking-tight">
                        {ayah.surah.name}
                      </h3>
                      <p className="text-[10px] font-black uppercase text-foreground/30 tracking-[0.6em]">
                    ...
                    <div className="p-10 rounded-[3rem] bg-white/5 border border-white/5 shadow-2xl relative overflow-hidden group/arab">
                      <div className="absolute top-0 right-0 p-6 text-white/5 font-mono text-[80px] leading-none pointer-events-none">
                        {ayah.numberInSurah}
                      </div>
                      <p
                        className="text-4xl sm:text-6xl md:text-7xl font-medium text-foreground leading-[1.6] text-center"
                        dir="rtl"
                      >
                        {ayah.text}
                      </p>
                  ...
                  {ayah.urduTranslation && (
                    <div className="relative p-10 rounded-[2.5rem] bg-emerald-500/[0.03] border border-emerald-500/10 group/urdu overflow-hidden">
                      ...
                      <div className="space-y-6 relative z-10 text-center">
                        <p
                          className="text-2xl sm:text-4xl font-medium text-foreground/90 leading-relaxed"
                          dir="rtl"
                        >
                          {ayah.urduTranslation}
                        </p>
                        <div className="flex items-center justify-center gap-3 pt-4">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500">
                            Urdu Protocol: Fateh Muhammad Jalandhry
                          </p>
                        </div>
                      </div>
                      <button
                        onClick={() =>
                          handleCopy(ayah.urduTranslation!, "Urdu")
                        }
                    ...
                    <div className="space-y-6 relative z-10">
                      ...
                      <div className="flex items-center gap-3 pt-4">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-primary">
                          English Protocol: Muhammad Asad
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="pt-6 border-t border-white/5 flex flex-col sm:flex-row gap-4">
                    ...
                    <Button
                      onClick={() => fetchAyah()}
                      variant="outline"
                      className="h-16 px-10 border-white/10 bg-white/5 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl active:scale-95 transition-all"
                    >

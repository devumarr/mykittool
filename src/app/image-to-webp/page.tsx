...
export default function ImageToWebPPage() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!activePreviewId && newAssets.length > 0)
      setActivePreviewId(newAssets[0].id);
    toast({
      title: "Photo Added",
      description: `Added ${newAssets.length} items to your list.`,
    });
  ...
  const processBatch = async () => {
    ...
    toast({
      title: "Complete",
      description: "Photos converted to the new format.",
    });
  };
  ...
  const activeAsset = useMemo(
    () => assets.find((a) => a.id === activePreviewId),
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="min-w-0">
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Save space by converting your photos to a more efficient format.
              Everything happens on your device with complete privacy.
            </p>
          </div>
              ...
              <Button
                ...
                onClick={() => {
                  assets.forEach((a) => {
                    ...
                    if (a.convertedUrl) URL.revokeObjectURL(a.convertedUrl);
                  });
                  setAssets([]);
                ...
                {assets.map((a) => (
                  <div
                    ...
                    className={cn(
                      ...
                      activePreviewId === a.id
                        ? "border-primary ring-2 ring-primary/20 scale-105"
                        : "border-border hover:border-primary/40",
                    )}
                  >
                    <div className="aspect-video rounded-xl overflow-hidden mb-3 bg-background">
                      <img
                        src={a.originalUrl}
                        alt="Thumb"
                    ...
                    <div className="space-y-1 min-w-0">
                      <p className="text-[9px] font-black text-foreground uppercase truncate pr-4">
                        {a.file.name}
                      </p>
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 w-6 h-6 rounded-lg bg-background/60 text-white flex items-center justify-center opacity-0 group-hover/item:opacity-100 transition-opacity hover:bg-red-500"
                    >
                      <X className="w-3 h-3" />
                    </button>
                    {a.status === "completed" && (
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <span className="text-[9px] font-black uppercase text-white/30">
                    {assets.length > 0 ? "Add More" : "Select Photos"}
                  </span>
                  <input
                    type="file"
                    ref={fileInputRef}
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Processing happens on your device. Your photos and data are

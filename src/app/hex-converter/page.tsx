  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        toast({
          title: "File Loaded",
          description: "Ready for hex conversion.",
        });
      };
  ...
  const convertToHex = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Binary matrix translated to hex.",
      });
    }, 100);
  };
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl">
          Professional binary analysis utility. Convert any local file into a
          formatted hexadecimal matrix instantly without server-side processing.
        </p>
      </div>
                  ...
                  {fileInfo ? (
                    ...
                    <>
                      ...
                      <p className="text-[10px] font-black uppercase text-foreground/40 tracking-widest group-hover:text-primary transition-colors">
                        No file chosen or drop file here
                      </p>
                    </>
                  )}
                  <input
                ...
                {fileInfo && fileInfo.size > 1024 * 1024 && (
                  <div className="p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20 flex items-start gap-3">
                    ...
                    <p className="text-[9px] text-yellow-500/70 font-bold leading-relaxed uppercase tracking-wider">
                      Warning: Large files (&gt;1MB) may cause temporary browser
                      performance degradation during conversion.
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-3 p-4 rounded-2xl bg-secondary border border-border">
                    <Checkbox
                      id="c-like"
                      checked={useCLike}
                      onCheckedChange={(v) => setUseCLike(v as boolean)}
                    />
                    <Label
                      htmlFor="c-like"
                      className="text-[10px] font-black uppercase tracking-widest text-foreground/60 cursor-pointer select-none"
                    >
                      C-Style (0x, 0x...)
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 p-4 rounded-2xl bg-secondary border border-border">
                    <Checkbox
                      id="newlines"
                      checked={useNewlines}
                      onCheckedChange={(v) => setUseNewlines(v as boolean)}
                    />
                    <Label
                      htmlFor="newlines"
                      className="text-[10px] font-black uppercase tracking-widest text-foreground/60 cursor-pointer select-none"
            ...
            <CardContent className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[10px] font-medium text-foreground/50 uppercase tracking-wider leading-relaxed">
                ...
                <div className="p-4 rounded-xl bg-secondary border border-border hover:border-primary/20 transition-all">
                  ...
                  Prefixed with 0x and comma-separated for source code
                  integration.
                </div>
                <div className="p-4 rounded-xl bg-secondary border border-border hover:border-primary/20 transition-all">
                  <span className="text-foreground font-black block mb-1">
                  ...
                  Optional line breaks every 16 bytes for standard memory
                  visualization.
                </div>
                <div className="p-4 rounded-xl bg-secondary border border-border hover:border-primary/20 transition-all">
                  <span className="text-foreground font-black block mb-1">
                  ...
                  Zero server transmission. Entirely processed within browser
                  memory.
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="pt-10 space-y-8">
              <div className="relative group/output">
                <Textarea
                  ...
                  className="w-full min-h-[440px] bg-white dark:bg-background border-border text-foreground font-mono rounded-[2.5rem] p-8 text-sm leading-relaxed resize-none shadow-inner custom-scrollbar transition-all overflow-auto"
                />
                {!output && (
              ...
              <div className="p-6 rounded-2xl bg-secondary border border-border flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-foreground/40 font-medium leading-relaxed">
                    Our engine automatically pads hex values to 2 digits and
                    applies sanitized formatting based on active protocols.
                  </p>
                </div>
              </div>
            </CardContent>

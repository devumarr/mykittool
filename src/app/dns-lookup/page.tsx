...
export default function DnsLookupPage() {
  ...
  const executeFullLookup = async (e?: React.FormEvent) => {
    ...
    if (!domainRegex.test(domain)) {
      setError(
        "Invalid Protocol: Provide a valid domain identifier (e.g. google.com).",
      );
      return;
    }
    ...
    const fetchPromises = RECORD_PROTOCOLS.map(async (proto) => {
      try {
        const response = await fetch(
          `https://dns.google/resolve?name=${encodeURIComponent(domain)}&type=${proto.id}`,
        );
    ...
    try {
      ...
      const finalGroups = settledResults
        .filter(
          (r): r is PromiseFulfilledResult<RecordGroup> =>
            r.status === "fulfilled",
        )
        .map((r) => r.value);
      ...
      if (
        !hasAnyData &&
        !finalGroups.some((g) => g.error === "Domain not found")
      ) {
        setError(
          "Zero records identified for this domain across all protocols.",
        );
      ...
      toast({
        ...
        description: "All DNS protocols synchronized.",
      });
    } catch (err) {
      setError(
        "Critical Retrieval Failure: DNS discovery nodes are restricted.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCopy = (text: string, label: string) => {
    ...
    toast({
      title: "Copied",
      description: "Linguistic value saved to clipboard.",
    });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Advanced clinical DNS auditing. Execute multi-protocol scans to
              isolate A, MX, TXT, and CNAME records in a single hardware-native
              cycle.
            </p>
          </div>
          ...
          {isLoading && (
            <div className="h-[400px] flex flex-col items-center justify-center space-y-12">
              ...
              <div className="text-center space-y-2">
                <p className="text-[11px] font-black uppercase text-primary tracking-[0.5em]">
                  Synchronizing Multi-Node Protocols...
                </p>
                <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-widest">
                  Querying Cloudflare & Google DNS
                </p>
              </div>
            </div>
          )}
              ...
              {results.map((group, idx) => (
                ...
                >
                  ...
                  <CardContent className="flex-1 p-0 overflow-hidden">
                    <div className="divide-y divide-white/5 max-h-[350px] overflow-auto custom-scrollbar bg-background/10">
                      {group.records.length === 0 ? (
                        <div className="p-10 text-center space-y-2">
                          <p className="text-[9px] font-black uppercase text-foreground/10 tracking-widest">
                            {group.error || "Zero Records"}
                          </p>
                        </div>
                      ) : (
                        group.records.map((record, rIdx) => (
                          <div
                            key={rIdx}
                            className="p-6 space-y-4 hover:bg-white/[0.02] transition-all group/item"
                          >
                            <div className="flex justify-between items-start gap-4">
                              <div className="space-y-1 min-w-0 flex-1">
                                <p className="text-[8px] font-black uppercase text-foreground/20 tracking-widest mb-1">
                                  Record Value
                                </p>
                                <p className="text-[13px] font-mono font-bold text-foreground break-all leading-relaxed">
                                  {record.data}
                                </p>
                              </div>
                              <div className="flex flex-col items-end gap-2 shrink-0">
                                <button
                                  ...
                                  className={cn(
                                    ...
                                    isCopied === `${group.type}-${rIdx}`
                                      ? "bg-green-500 text-white"
                                      : "bg-secondary text-foreground/20 hover:text-primary",
                                  )}
                                >
                                  {isCopied === `${group.type}-${rIdx}` ? (
              ...
              <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Clinical DNS deconstruction occurs strictly via encrypted
                    REST handshakes. Search payloads are never logged.
                  </p>
                </div>
              </div>
              <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Utilizing parallel discovery cycles across global Google DNS
                    edge nodes for 1:1 record fidelity.
                  </p>
                </div>
              </div>
              <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Comprehensive extraction of TXT, MX, and AAAA records
                    provides a complete technical profile for any domain.
                  </p>
                </div>
              </div>
            </div>

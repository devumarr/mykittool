  ...
  const fetchRepos = async (login: string) => {
    ...
    try {
      const response = await fetch(
        `https://api.github.com/users/${encodeURIComponent(login)}/repos?sort=updated&per_page=6`,
      );
      if (response.ok) {
  ...
  const fetchUser = async (target?: string) => {
    ...
    try {
      ...
      if (response.status === 404) {
        setError(
          "Identity Not Found: This username does not exist in the GitHub registry.",
        );
      } else if (!response.ok) {
        ...
        toast({
          ...
          description: `Signal mapped for ${data.login}.`,
        });
      }
    } catch (err) {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              GitHub{" "}
              <span className="text-primary italic">Identity Finder</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional developer discovery engine. Isolate public profile
              metadata, repository density, and social reach locally via the
              GitHub REST protocol.
            </p>
          </div>
            ...
            <CardContent className="pt-10 space-y-8">
              ...
              <div className="p-6 rounded-2xl bg-secondary/50 border border-border space-y-4">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Profiles are retrieved via public REST nodes. Hardware-native
                  memory isolation ensures search strings remain private to your
                  session.
                </p>
              </div>
            </CardContent>
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000">
          {isLoading ? (
            ...
            <div className="space-y-8 animate-in zoom-in-95 duration-500">
              {/* Profile Master Card */}
              <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col bg-background/10">
                ...
                <CardContent className="p-8 sm:p-12 space-y-12">
                  <div className="flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div className="text-center md:text-left space-y-4 min-w-0 flex-1">
                      <div className="space-y-1">
                        <h2 className="text-4xl sm:text-5xl font-headline font-black text-foreground uppercase tracking-tighter leading-none truncate">
                    ...
                    {[
                      {
                        icon: Building2,
                        label: "Organization",
                        val: user.company || "Autonomous",
                      },
                      {
                        icon: MapPin,
                        label: "Location Node",
                        val: user.location || "Distributed",
                      },
                      {
                        icon: Globe,
                        label: "Web Blog",
                      ...
                      (item, i) =>
                        item.val && (
                          <div
                            key={i}
                            className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-5"
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              {item.isLink ? (
                                <a
                                  href={
                                    item.val.startsWith("http")
                                      ? item.val
                                      : `https://${item.val}`
                                  }
                                  target="_blank"
                                  className="text-[11px] font-bold text-primary hover:underline truncate block uppercase"
                                >
                                  {item.val}
                                </a>
                              ) : (
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      ...
                      <div
                        key={s.label}
                        className="p-6 rounded-[2rem] bg-primary/5 border border-primary/10 text-center space-y-2 group hover:border-primary/30 transition-all"
                      >
                        <s.icon className="w-4 h-4 mx-auto text-primary/40 group-hover:text-primary transition-colors" />
                        <p className="text-2xl font-headline font-black text-foreground">
                          {s.val}
                        </p>
                  ...
                  <div className="pt-6 border-t border-white/5">
                    <Button
                      asChild
                      className="h-16 w-full bg-white text-black hover:bg-white/90 font-black rounded-2xl flex items-center justify-center gap-4 text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                    >
                      <a
                        href={user.html_url}
                        target="_blank"
                      ...
                      >
                        ...
                        <ExternalLink className="w-4 h-4 ml-1 opacity-20" />
                      </a>
                    </Button>
                  </div>
                </CardContent>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                  ...
                  <Badge
                    variant="outline"
                    className="bg-primary/10 text-primary border-primary/20 text-[8px] font-black uppercase"
                  >
                    Recent Activity
                  </Badge>
                </div>
                  ...
                  {isLoadingRepos ? (
                    ...
                    <div className="col-span-full p-12 rounded-[2rem] border-2 border-dashed border-border flex flex-col items-center justify-center opacity-10 gap-3">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest">
                        No public repositories discovered
                      </p>
                    </div>
                  )}
                </div>
            ...
            <Card className="glass-card border-border shadow-2xl h-[600px] flex flex-col items-center justify-center text-center p-12 border-dashed bg-background/10">
              ...
              <p className="text-sm text-foreground/20 font-medium max-w-xs mt-4 uppercase tracking-tighter leading-relaxed">
                Enter a GitHub handle to isolate the target developer identity

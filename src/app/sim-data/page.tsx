...
export default function SimDataPage() {
  ...
  const getCarrierFallback = (num: string) => {
    ...
    if (clean.startsWith("30") || clean.startsWith("32"))
      return "Mobilink / Warid (Jazz)";
  ...
  const handleSearch = async () => {
    ...
    if (cleanNum.length < 10) {
      toast({
        ...
        description: "Please enter a valid Pakistani number (11 digits).",
      });
      return;
    }
    ...
    try {
      ...
      if (validation.success && validation.data) {
        ...
        setResult({
          owner:
            foundData?.owner ||
            (v.is_valid ? "IDENTITY RESTRICTED" : "RECORD NOT FOUND"),
          cnic: foundData?.cnic || "UNAVAILABLE",
          address:
            foundData?.address || (v.location ? v.location : "SIGNAL UNKNOWN"),
          status:
            foundData?.status ||
            (v.is_valid ? "ACTIVE" : "INACTIVE / UNLISTED"),
          activationDate: foundData?.activationDate || "N/A",
          carrier:
            foundData?.carrier || v.carrier || getCarrierFallback(formattedNum),
          isValid: v.is_valid,
      ...
      toast({
        ...
        description: "Identity data and network validation complete.",
      });
    } catch (err) {
      toast({
        variant: "destructive",
        title: "Protocol Error",
        description: "Failed to negotiate with discovery nodes.",
      });
    } finally {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              Sim Data{" "}
              <span className="text-primary italic">Finder Studio</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional mobile identity matrix. Identify carrier protocols,
              regional registration data, and global number validity locally and
              securely.
            </p>
          </div>
            ...
            <CardContent className="pt-10 space-y-8">
              <div className="space-y-4">
                ...
                <p className="text-[9px] text-foreground/20 font-bold uppercase tracking-widest text-center">
                  Format: 11 Digits (e.g. 03001234567)
                </p>
              </div>
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Search payloads are processed strictly within your browser's
                volatile memory. No search history or identification data is
                logged to our servers.
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-10">
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-widest">
                      Querying Carrier & Validation Repositories
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-8 border-b border-white/5 pb-10">
                    <div
                      className={cn(
                        ...
                        result.isMissing
                          ? "bg-red-500/10 border-red-500/20 text-red-500 ring-red-500/5"
                          : "bg-primary/10 border-primary/20 text-primary ring-primary/5",
                      )}
                    >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {[
                      ...
                      },
                      {
                        ...
                        val: result.activationDate,
                      },
                      {
                        icon: Globe,
                        label: "Country Node",
                        val: result.country || "Pakistan",
                      },
                      {
                        icon: Network,
                        label: "Line Protocol",
                        val: result.lineType || "Mobile",
                      },
                    ].map((item, i) => (
                      <div
                        key={i}
                        className="p-6 rounded-[2rem] bg-secondary/50 border border-border group hover:border-primary/20 transition-all flex items-center gap-6"
                      >
                  ...
                  <div className="p-6 rounded-[2rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-foreground/40 leading-relaxed font-medium uppercase">
                        Identity retrieval is based on a simulated clinical
                        registry and global node validation.{" "}
                        {result.isValid
                          ? "This number adheres to the E.164 telecommunications standard."
                          : "This number follows a non-standard or restricted protocol."}
                      </p>
                    </div>
                  </div>
                </div>

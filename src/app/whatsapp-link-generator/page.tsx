...
const TEMPLATES = [
  ...
  {
    label: "Price?",
    text: "Hi, can you provide pricing details for your services?",
  },
  { label: "Details", text: "Hello, I need more information about this." },
  {
    label: "Available?",
    text: "Hi, are you currently available for a project?",
  },
  {
    label: "Support",
    text: "Hello, I need technical assistance with my account.",
  },
];
...
export default function WhatsAppLinkGeneratorPage() {
  ...
  const handleCopy = (text: string, label: string) => {
    ...
    toast({
      ...
      description: `${label} saved to clipboard.`,
    });
  ...
  const downloadQr = () => {
    if (qrInstance.current) {
      ...
      toast({
        ...
        description: "QR saved as high-res PNG.",
      });
    }
  };
  ...
  const executeBulk = () => {
    ...
    toast({
      ...
      description: `Synthesized ${results.length} links to clipboard.`,
    });
  };
    ...
    <div className="container mx-auto px-4 md:px-6 py-12 md:py-20 max-w-7xl">
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Professional linguistic link synthesis. Create instant wa.me chat
            protocols with custom messages, bulk re-matricing, and
            hardware-native QR generation.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-left-6 duration-700">
          <Tabs
            value={activeMode}
            onValueChange={(v: any) => setActiveMode(v)}
            className="w-full"
          >
              ...
              <CardContent className="pt-10 space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                  ...
                  <div className="md:col-span-7 space-y-8">
                    ...
                    >
                      <div className="space-y-4">
                        ...
                        <div className="relative group/input">
                          <div className="absolute left-6 top-1/2 -translate-y-1/2 text-primary font-bold text-lg pointer-events-none">
                            +{countryCode}
                          </div>
                          <Input
                            value={phone}
                        ...
                        <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-widest ml-1">
                          Cleaned: +{countryCode}
                          {cleanNumber(phone)}
                        </p>
                      </div>
                    ...
                    <TabsContent
                      value="bulk"
                      className="m-0 space-y-4 animate-in fade-in"
                    >
                      <Label className="text-[10px] font-black text-foreground/40 uppercase tracking-[0.2em] ml-1">
                        Batch Roster (Numbers)
                      </Label>
                      <Textarea
                        value={bulkInput}
                  ...
                  {activeMode === "single" ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Button
                        onClick={handleShare}
                        disabled={!phone}
                        className="h-16 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/30 active:scale-95 transition-all"
                      >
                      ...
                      <Button
                        ...
                        disabled={!phone}
                        variant="outline"
                        className="h-16 border-border bg-white/5 text-foreground font-black uppercase text-[10px] tracking-widest rounded-2xl"
                      >
                        {isCopied === "link" ? (
                    ...
                    <Button
                      ...
                      className="h-16 w-full bg-primary text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl active:scale-95"
                    >
                      <Layers className="w-5 h-5 mr-3" /> Execute Batch
                      Synthesis
                    </Button>
                  )}
                </div>
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Linguistic identifiers are processed strictly in local memory.
                  No phone numbers or message payloads are logged.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Short links are compatible with all modern browser nodes and
                  mobile operating systems via standard URI protocols.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col">
            ...
            <CardContent className="p-8 sm:p-12 flex flex-col items-center gap-10 bg-background/10">
              <div className="relative group/qr p-4 bg-white rounded-[2.5rem] shadow-2xl shadow-primary/10 ring-1 ring-border overflow-hidden">
                {!phone && (
                  <div className="absolute inset-0 z-20 bg-white/95 backdrop-blur-md flex items-center justify-center p-12 text-center text-[10px] font-black uppercase text-foreground/20 tracking-widest">
                    Awaiting Signal
                  </div>
                )}
                <div

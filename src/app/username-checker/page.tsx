...
export default function UsernameCheckerPage() {
  ...
  const handleCheck = async (targetName: string) => {
    ...
    const promises = platformNames.map(async (pName) => {
      try {
        ...
        setResults((prev) =>
          prev.map((r) =>
            r.platform === pName
              ? { ...r, status: "unknown", confidence: "low" }
              : r,
          ),
        );
      }
    ...
    toast({
      title: "Scan Complete",
      description: "All global nodes synchronized.",
    });
  };
  ...
  const handleCheckVariants = () => {
    ...
    toast({
      ...
      description: `Synthesizing matrix for "${nextVariant}".`,
    });
  };
  ...
  const handleCopyTaken = () => {
    ...
    setIsCopied("taken");
    toast({
      title: "Signals Isolated",
      description: "Profile registry copied to clipboard.",
    });
  ...
  const handleExportTxt = () => {
    const content =
      `[MY KIT TOOL - USERNAME OSINT REPORT]\nTarget: ${username}\nTimestamp: ${new Date().toLocaleString()}\n\n` +
      results
        .map((r) => `${r.platform} [${r.status.toUpperCase()}]: ${r.url}`)
        .join("\n");
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-6xl font-headline font-black text-foreground uppercase tracking-tight leading-none">
              Username{" "}
              <span className="text-primary italic">OSINT Checker</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional high-fidelity identity discovery. Isolate digital
              footprints across 40+ global registries with 1:1 signal validation
              and zero-storage local privacy.
            </p>
          </div>
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group">
            ...
            <CardContent className="pt-10 space-y-10">
              ...
              <div className="pt-8 border-t border-white/5 space-y-4">
                <Label className="text-[10px] font-black text-foreground/20 uppercase tracking-[0.2em] ml-1">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        activeCategory === cat.id
                          ...
                          : "bg-secondary/30 border-border text-foreground/40 hover:text-foreground",
                      )}
                    >
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All discovery occurs strictly via secure server-side
                  handshakes. Your search strings and identity targets are never
                  logged or stored.
                </p>
              </div>
            </div>
                    ...
                    {filteredResults.map((r, i) => (
                        ...
                        className={cn(
                          ...
                          r.status === "taken"
                            ...
                            : r.status === "available"
                              ...
                              : r.status === "checking"
                                ? "bg-primary/5 animate-pulse border-primary/20"
                      ...
                      >
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                {r.confidence && r.status !== "checking" && (
                                  <span className="text-[7px] font-bold text-foreground/10 uppercase tracking-tighter">
                          ...
                          {r.status === "taken" && (
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              className="h-10 w-10 rounded-xl hover:bg-primary/10 hover:text-primary shrink-0 shadow-lg"
                            >
                              <a
                                href={r.url}
                                target="_blank"
                        ...
                        {r.status === "taken" && r.displayName && (
                          <div className="px-4 py-3 rounded-2xl bg-background border border-white/5 flex items-center gap-4 animate-in slide-in-from-top-2">
                            {r.avatar ? (
                              <img
                                src={r.avatar}
                                alt=""
                        ...
                        {r.reason && (
                          <div className="px-4 py-3 rounded-xl bg-amber-500/5 border border-amber-500/10 flex items-start gap-2">
                            ...
                            <p className="text-[8px] font-bold text-amber-600/60 leading-tight uppercase">
                              {r.reason}
                            </p>
                          </div>
                        )}
                ...
                <div className="p-6 sm:p-8 border-t border-white/5 bg-[#0a0a0c] flex flex-col lg:flex-row items-center justify-between gap-6 shrink-0 mt-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                    ...
                    <Button
                      ...
                      className="h-14 px-10 bg-primary text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all w-full sm:flex-1 lg:flex-none"
                    >

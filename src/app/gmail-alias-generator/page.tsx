  ...
  const generateAliases = useCallback(() => {
    ...
    if (!validateEmail(trimmed)) {
      toast({
        ...
        description: "Only @gmail.com or @googlemail.com inputs are valid.",
      });
      return;
    }
    ...
    if (mode === "googlemail") {
      ...
      // Combined Logic
      for (let i = 1; i <= maxResults; i++) {
        const dotSeed = Math.floor(
          Math.random() * Math.pow(2, username.length - 1),
        );
        let res = username[0];
    ...
    toast({
      ...
      description: `Generated ${finalResults.length} unique identities.`,
    });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional linguistic mailbox synthesis. Generate unlimited
              verified aliases for your Gmail account locally using dot-trick
              and plus-tag protocols.
            </p>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    value={customTag}
                    onChange={(e) =>
                      setCustomTag(e.target.value.replace(/[^a-z0-9]/gi, ""))
                    }
              ...
              {results.length > 0 && (
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-9 px-4 rounded-xl border-white/5 bg-white/5 text-[8px] font-black uppercase tracking-widest"
                  >
                    {isCopied === "all" ? (
                ...
                {results.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {results.map((email, i) => (
                      <div
                        key={i}
                        className="p-4 rounded-2xl bg-secondary/40 border border-border flex items-center justify-between gap-6 transition-all hover:bg-secondary/60 animate-in slide-in-from-bottom-2"
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <div className="w-9 h-9 rounded-xl bg-background border border-border flex items-center justify-center text-primary/40 shrink-0 font-mono text-[10px]">
                            #{i + 1}
                          </div>
                          <p className="text-xs sm:text-sm font-bold text-foreground truncate select-all">
                            {email}
                          </p>
                        </div>
                        <button
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All generation occurs 100% locally in browser memory. Email
                  strings are never transmitted or stored on remote servers.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                <h4 className="text-[13px] font-black text-foreground uppercase tracking-widest leading-none">
                  Sub-addressing Protocol
                </h4>
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Utilizing official Gmail standards for sub-addressing,
                  ensuring all aliases route correctly to your primary inbox.
                </p>
              </div>
            </div>
          </div>

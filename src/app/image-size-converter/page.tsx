...
export default function ImageSizeConverterPage() {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        img.onload = () => {
          ...
          toast({ title: "Asset Imported", description: `${img.width}x${img.height} matrix decoded.` });
        };
        img.src = result;
      };
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      toast({ title: "Master Exported", description: "Converted asset saved to local storage." });
    };
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
           <div>
              ...
              <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
                Resize and convert images in one click. Professional local-only re-matricing with precision geometry control and multi-format encoding protocols.
              </p>
           </div>
           ...
           <Card className="glass-card border-border shadow-2xl overflow-hidden relative group">
              ...
              <CardContent className="pt-10 space-y-10">
                    ...
                    >
                      {image ? (
                        ...
                        <div className="text-center space-y-4">
                           ...
                           <span className="text-[9px] font-black uppercase text-foreground/30 tracking-widest">Import Asset Matrix</span>
                        </div>
                      )}
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 relative">
                       <div className="space-y-2">
                          <Label className="text-[9px] font-black text-foreground/20 uppercase ml-1">Width</Label>
                          <Input type="number" value={targetWidth || ''} onChange={e => updateWidth(parseInt(e.target.value) || 0)} className="h-14 bg-secondary border-border rounded-2xl text-lg font-bold px-6 focus:ring-primary/40" />
                       </div>
                       <div className="space-y-2">
                          <Label className="text-[9px] font-black text-foreground/20 uppercase ml-1">Height</Label>
                          <Input type="number" value={targetHeight || ''} onChange={e => updateHeight(parseInt(e.target.value) || 0)} className="h-14 bg-secondary border-border rounded-2xl text-lg font-bold px-6 focus:ring-primary/40" />
                       </div>
                    ...
                    <div className="space-y-3 pt-2">
                       <Label className="text-[9px] font-black text-foreground/20 uppercase tracking-widest ml-1">Identity Presets</Label>
                       <div className="grid grid-cols-1 gap-2">
                          {PRESETS.map(p => (
                            <button
                              key={p.label}
                              onClick={() => { setLockRatio(false); setTargetWidth(p.w); setTargetHeight(p.h); setAspectRatio(p.w/p.h); setLockRatio(true); toast({ title: "Preset Applied" }); }}
                              className="h-10 px-4 rounded-xl border border-border bg-background flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-foreground/40 hover:text-primary hover:border-primary transition-all"
                            >
                               <span>{p.label}</span>
                 ...
                 <div className="space-y-6 pt-6 border-t border-white/5">
                    <div className="space-y-4">
                       <Label className="text-[10px] font-black text-foreground/40 uppercase tracking-[0.2em] ml-1">Format Protocol</Label>
                       <div className="grid grid-cols-3 gap-2">
                          {['PNG', 'JPG', 'WebP'].map(f => (
                            <button
                              key={f}
                              onClick={() => setOutputFormat(`image/${f.toLowerCase() === 'jpg' ? 'jpeg' : f.toLowerCase()}` as any)}
                              className={cn(
                                "h-11 rounded-xl border text-[9px] font-black uppercase transition-all",
                                outputFormat.includes(f.toLowerCase().replace('jpg', 'jpeg')) ? "bg-primary text-white border-primary shadow-lg" : "bg-background border-border text-foreground/40 hover:text-foreground"
                              )}
                            >
                               {f}
                            </button>
                    ...
                    >
                       {isProcessing ? <Loader2 className="w-5 h-5 animate-spin mr-3" /> : <Save className="w-5 h-5 mr-3" />}
                       Execute & Save
                    </Button>
                 </div>
              </CardContent>
           ...
           <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[600px] bg-black/10">
              ...
              <CardHeader className="py-6 border-b border-border bg-secondary/30 flex flex-row items-center justify-between shrink-0 px-6 sm:px-10">
                 <div className="flex items-center gap-4">
                    ...
                    <CardTitle className="text-[10px] font-black text-primary uppercase tracking-[0.5em]">Live Matrix Visualizer</CardTitle>
                 </div>
                 {originalMeta && (
                    <div className="hidden sm:flex gap-2">
                       <Badge className="bg-primary/10 text-primary border-primary/20 text-[8px] font-black uppercase">Source: {originalMeta.width}x{originalMeta.height}</Badge>
                       <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[8px] font-black uppercase">Target: {targetWidth}x{targetHeight}</Badge>
                    </div>
                 )}
              </CardHeader>
                 ...
                 <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary/50 border border-border group hover:bg-secondary/80 transition-all shadow-lg">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                        Synthesis occurs 100% locally. Your visual matrices are never transmitted or stored on remote servers.
                      </p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary/50 border border-border group hover:bg-secondary/80 transition-all shadow-lg">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-[12px] font-black text-foreground uppercase tracking-widest leading-none">Zero-Loss Scaling</h4>
                      <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                        Utilizing browser-native bi-linear interpolation to ensure high-fidelity pixel preservation during transformation.
                      </p>
                    </div>
                 </div>
              </div>

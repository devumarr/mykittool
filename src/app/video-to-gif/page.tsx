...
export default function VideoToGifPage() {
  ...
  const loadFFmpeg = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Failed to load FFmpeg. Ensure your browser supports SharedArrayBuffer.",
      });
      return false;
    }
  };
    ...
    if (selectedFile) {
      if (selectedFile.size > 50 * 1024 * 1024) {
        toast({
          variant: "destructive",
          title: "Heavy Payload",
          description: "Videos over 50MB may slow down the browser.",
        });
      }
      ...
      toast({
        ...
        description: "Studio analyzed media container.",
      });
    }
  };
  ...
  const convertToGif = async () => {
    ...
    try {
      ...
      toast({
        title: "GIF Master Exported",
        description: "Asset synthesized successfully.",
      });
    } catch (err: any) {
      ...
      toast({
        ...
        description:
          "An error occurred during GIF synthesis. The range may be too long.",
      });
  ...
  const handleClear = () => {
    ...
    toast({
      title: "Studio Reset",
      description: "Memory purged and fields cleared.",
    });
  };
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl">
          Professional-grade GIF synthesis. Convert MP4 or WebM clips into
          optimized, high-fidelity animated GIFs entirely in your browser.
        </p>
      </div>
                  ...
                  {file ? (
                    <div className="text-center p-6 space-y-2">
                      ...
                      <p className="text-[9px] font-bold text-foreground/30 uppercase tracking-widest">
                        {(file.size / (1024 * 1024)).toFixed(1)} MB Detected
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <p className="text-[10px] font-black uppercase text-foreground/40 tracking-widest group-hover:text-primary transition-colors text-center">
                        Import Video Asset
                        <br />
                        <span className="text-[8px] opacity-60">MP4, WEBM</span>
                      </p>
                    </>
                  )}
                  <input
              ...
              {file && (
                <div className="space-y-10 animate-in zoom-in duration-500">
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Scissors className="w-3 h-3 text-primary" /> Trim Range
                        Matrix
                      </Label>
                      <span className="text-[9px] font-black uppercase text-primary bg-primary/10 px-2 py-0.5 rounded-lg">
                        Selected: {(endTime - startTime).toFixed(1)}s
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <p className="text-[9px] font-black uppercase text-foreground/30 ml-1">
                          Start Mark
                        </p>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Activity className="w-3 h-3 text-primary" /> Frame Rate
                        (FPS)
                      </Label>
                    ...
                    <div className="space-y-4">
                      <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Maximize className="w-3 h-3 text-primary" /> Width
                        Protocol
                      </Label>
                  ...
                  <div className="p-4 rounded-2xl bg-yellow-500/5 border border-yellow-500/10 flex items-start gap-4">
                    ...
                    <p className="text-[9px] text-yellow-600/70 font-bold uppercase leading-relaxed tracking-wider">
                      Warning: Long durations (&gt;10s) or high widths will
                      significantly increase synthesis time and file size.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                Our studio utilizes a two-pass palette generation algorithm to
                ensure the highest visual fidelity with 256 optimized colors.
                Processing occurs 100% locally.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="relative group/output min-h-[300px] flex flex-col items-center justify-center rounded-[2.5rem] bg-secondary/30 border border-border p-10 text-center">
                ...
                {isProcessing && (
                  <div className="w-full space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="mt-4 p-4 rounded-xl bg-background/90 border border-white/10 text-left font-mono text-[9px] text-green-500/80 overflow-hidden shadow-inner">
                      ...
                      {logs.map((log, i) => (
                        <div
                          key={i}
                          className="truncate whitespace-nowrap opacity-70"
                        >
                          &gt; {log}
                        </div>
                      ))}
                ...
                {gifUrl && (
                  <div className="space-y-8 w-full animate-in zoom-in duration-500">
                    ...
                    >
                      <a
                        href={gifUrl}
                        download={`master-clip-${Date.now()}.gif`}
                      >
              ...
              <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group transition-all hover:bg-secondary/80">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-foreground/60 leading-relaxed font-medium">
                    Automatic Lanczos filtering applied for superior edge
                    definition.
                  </p>
                </div>
              </div>

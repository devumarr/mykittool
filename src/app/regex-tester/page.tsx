...
export default function RegexTesterPage() {
  ...
  const results = useMemo(() => {
    ...
    try {
      ...
      const matches = flags.g
        ? Array.from(testText.matchAll(regex))
        : (() => {
  ...
  const handleCopyPattern = () => {
    ...
    toast({
      ...
      description: "Regex string saved to clipboard.",
    });
  ...
  const highlightedJSX = useMemo(() => {
    if (results.error || !pattern || results.matches.length === 0)
      return testText;
    ...
    sortedMatches.forEach((match, i) => {
      ...
      segments.push(
        <mark
          key={i}
          className="bg-primary/30 text-transparent border-b-2 border-primary rounded-sm"
        >
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Professional regular expression evaluator. Test patterns against
            linguistic payloads with real-time match identification, capture
            group analysis, and local-only production.
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0 pb-2">
          ...
          <Button
            variant="outline"
            onClick={handleClear}
            className="h-11 px-6 rounded-xl border-border bg-secondary/50 text-[9px] font-black uppercase tracking-widest hover:text-destructive transition-all shadow-lg"
          >
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="relative group/pattern">
                    <div className="absolute left-6 top-1/2 -translate-y-1/2 text-primary/40 font-mono text-xl pointer-events-none">
                      /
                    </div>
                    <Input
                      value={pattern}
                    ...
                    />
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 text-primary/40 font-mono text-xl pointer-events-none">
                      /{flagString}
                    </div>
                  </div>
                  {results.error && (
                    ...
                    {[
                      {
                        id: "g",
                        label: "Global",
                        desc: "Find all matches in payload",
                      },
                      {
                        id: "i",
                        label: "Insensitive",
                        desc: "Ignore char case casing",
                      },
                      {
                        id: "m",
                        label: "Multiline",
                        desc: "Allow anchors to match lines",
                      },
                      {
                        id: "s",
                        label: "DotAll",
                        desc: "Allow . to match newlines",
                      },
                      {
                        id: "u",
                        label: "Unicode",
                        desc: "Support unicode sequences",
                      },
                      {
                        id: "y",
                        label: "Sticky",
                        desc: "Match only from last index",
                      },
                    ].map((f) => (
                      <div
                        key={f.id}
                        className="flex items-center gap-3 p-4 rounded-2xl bg-secondary border border-border group/flag hover:border-primary/20 transition-all"
                      >
                        <Checkbox
                          id={f.id}
                          checked={flags[f.id as keyof Flags]}
                          onCheckedChange={(val) =>
                            setFlags((prev) => ({ ...prev, [f.id]: !!val }))
                          }
                        />
                        <div className="space-y-0.5">
                          <Label
                            htmlFor={f.id}
                            className="text-[10px] font-black uppercase tracking-widest text-foreground/60 cursor-pointer"
                          >
                            {f.label}
                          </Label>
                          <p className="text-[8px] text-foreground/20 font-bold uppercase truncate">
                            {f.desc}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-3 animate-in slide-in-from-top-2 duration-300">
                      <div className="flex justify-between items-center px-1">
                        <Label className="text-[9px] font-black text-primary uppercase tracking-widest">
                          Replaced Output Matrix
                        </Label>
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(results.replaced);
                            toast({ title: "Result Copied" });
                          }}
                          className="text-[9px] font-black uppercase text-foreground/40 hover:text-primary transition-colors"
                        >
                          Copy Result
                        </button>
                      </div>
                ...
                {results.matches.length > 0 ? (
                  <div className="divide-y divide-border">
                    {results.matches.map((m, i) => (
                      <div
                        key={i}
                        className="p-6 space-y-4 hover:bg-secondary/20 transition-all group/match"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                          ...
                          <span className="text-[9px] font-mono font-bold text-primary/40 uppercase">
                            Index: {m.index}
                          </span>
                        </div>

                        <div className="p-4 bg-white dark:bg-background rounded-xl border border-border group-hover/match:border-primary/20 transition-all">
                        ...
                        {m.length > 1 && (
                          <div className="space-y-2 pt-2">
                            <p className="text-[9px] font-black uppercase tracking-widest text-foreground/30 ml-1">
                              Capture Groups
                            </p>
                            <div className="space-y-1.5">
                              {m.slice(1).map((group, gIdx) => {
                                const namedGroup = m.groups
                                  ? Object.entries(m.groups).find(
                                      ([_, val]) => val === group,
                                    )
                                  : null;
                                return (
                  ...
                  <div className="py-32 text-center opacity-10 space-y-4 px-12">
                    ...
                    <p className="text-xs font-black uppercase tracking-[0.3em] leading-relaxed">
                      Awaiting Signal Detection
                      <br />
                      <span className="text-[9px]">
                        Check pattern syntax and payload integrity
                      </span>
                    </p>
                  </div>
                )}
              ...
              <div className="p-8 border-t border-border bg-[#0a0a0c]">
                <div className="p-6 rounded-[2.5rem] bg-secondary/50 border border-border space-y-4">
                  ...
                  <p className="text-[10px] text-foreground/40 font-medium leading-relaxed uppercase">
                    The studio utilizes browser-native RegExp evaluation for
                    zero-latency detection. All linguistic deconstruction occurs
                    100% in local volatile memory.
                  </p>
                </div>
              </div>
            </CardContent>

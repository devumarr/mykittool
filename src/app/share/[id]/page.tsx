      ...
      p.on("open", () => {
        ...
        conn.on("data", (data: any) => {
          if (data.type === "meta") {
            ...
            if (incomingMeta) {
              setProgress(
                Math.round((bytesReceived / incomingMeta.size) * 100),
              );
            }
          } else if (data.type === "end") {
            const blob = new Blob(chunks, {
              type: incomingMeta?.mime || "application/octet-stream",
            });
        ...
        <CardContent className="p-8 sm:p-16 flex flex-col items-center gap-12">
          ...
          {status === "receiving" && meta && (
            <div className="w-full space-y-10 py-6 animate-in fade-in">
              <div className="flex items-center gap-6 p-6 rounded-3xl bg-background border border-white/5 shadow-inner">
                {meta.name.endsWith(".zip") ? (
                  <FileArchive className="w-8 h-8 text-primary/40" />
                ) : (
              ...
              <div className="space-y-6">
                ...
                <p className="text-center text-[9px] font-bold text-white/20 uppercase tracking-widest animate-pulse">
                  Streaming data tunnel active
                </p>
              </div>
            </div>
          )}
          ...
          {status === "not-found" && (
            <div className="text-center space-y-6 animate-in zoom-in">
              ...
              <Button
                asChild
                variant="outline"
                className="h-12 px-8 border-white/10 bg-white/5 rounded-xl"
              >
                <Link href="/">

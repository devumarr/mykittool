...
export default function OCRPage() {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        toast({
          variant: "destructive",
          title: "Heavy Payload",
          description: "Image capped at 10MB for protocol stability.",
        });
        return;
      }
  ...
  const extractTextLocal = async (img: string) => {
    ...
    try {
      const {
        data: { text },
      } = await Tesseract.recognize(img, language, {
        logger: (m) => {
    ...
    try {
      ...
      if (cloudResult.success && cloudResult.text && cloudResult.text.trim()) {
        ...
        toast({
          ...
          description: "Text extracted via high-fidelity Cloud Node.",
        });
      } else {
        // 2. Automated Fallback: Tesseract.js (Local Hardware)
        console.warn(
          "Cloud node returned empty results. Initializing local fallback...",
        );
        ...
        if (localText) {
          ...
          toast({
            title: "Fallback Sync",
            description: "Switched to hardware-native extraction.",
          });
        } else {
      ...
      if (activeEngine !== "local") {
        try {
          ...
          if (localText) {
            ...
            toast({
              title: "Local Sync",
              description: "Hardware-native extraction complete.",
            });
          } else {
          ...
          toast({
            ...
            description:
              "Extraction nodes are restricted or image is unreadable.",
          });
        }
      } else {
        setResult("");
        toast({
          variant: "destructive",
          title: "Protocol Failure",
          description: "Could not isolate text from this visual.",
        });
      }
    } finally {
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Professional Multi-Engine Optical Character Recognition.
            Prioritizing high-fidelity cloud neural networks with
            hardware-native Tesseract fallback.
          </p>
        </div>
              ...
              {!image ? (
                ...
                >
                  ...
                  <div className="text-center space-y-2">
                    <span className="text-sm font-headline font-black uppercase text-white/20 group-hover:text-white transition-colors">
                      Import Document Visual
                    </span>
        ...
        <div className="lg:col-span-5 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group flex flex-col min-h-[400px]">
            ...
            <CardContent className="flex-1 p-0 flex flex-col">
              ...
              {result && (
                <div className="p-6 bg-background border-t border-white/5 animate-in slide-in-from-bottom-4">
                  <Button
                    onClick={handleCopy}
                    className="w-full h-14 bg-white text-black font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-2xl active:scale-95 transition-all"
                  >
                    {isCopied ? (
            ...
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Extraction nodes prioritize hardware-local memory when
                  available. No data is stored permanently.
                </p>
              </div>
            </div>
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  The studio uses a dual-engine protocol to ensure the highest
                  extraction accuracy across all browser hardware.
                </p>
              </div>
            </div>
          </div>

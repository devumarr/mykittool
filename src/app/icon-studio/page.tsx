    ...
    try {
      ...
      if (response.success && response.icons.length > 0) {
        ...
        toast({
          ...
          description: `Discovered ${response.icons.length} identifiers.`,
        });
      } else {
        setError(
          response.error ||
            "No icons identified for this query. Try a different node.",
        );
      }
    } catch (err) {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ...
          toast({
            ...
            description: `${format.toUpperCase()} synthesized locally.`,
          });
        }
        URL.revokeObjectURL(url);
      };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional multi-node icon discovery. Isolate high-fidelity
              symbols from global registries with real-time chromatic synthesis
              and hardware-native exports.
            </p>
          </div>
          ...
          <Card className="glass-card border-border shadow-2xl overflow-visible relative group">
            ...
            <CardContent className="pt-10 space-y-8 relative">
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-secondary/50 border border-border">
                ...
                <Badge
                  variant="outline"
                  className="bg-background/50 border-primary/20 text-primary text-[8px] font-black uppercase px-3 py-1"
                >
                  {activeNode}
                </Badge>
              </div>
            ...
            <CardContent className="pt-8 space-y-10">
              <div className="grid grid-cols-1 gap-8">
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-foreground/40">
                    <Label className="flex items-center gap-2">
                      <Palette className="w-3.5 h-3.5" /> Chromatic Value
                    </Label>
                  ...
                  <div className="flex items-center gap-4 p-4 rounded-xl bg-secondary border border-border">
                    <div
                      className="w-10 h-10 rounded-lg relative overflow-hidden ring-2 ring-white/10"
                    ...
                    <Input
                      value={iconColor}
                      onChange={(e) => setIconColor(e.target.value)}
                      className="h-10 bg-background border-border text-xs font-mono uppercase text-center"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-foreground/40">
                    <Label className="flex items-center gap-2">
                      <Maximize className="w-3.5 h-3.5" /> Geometric Scale
                    </Label>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[32, 512, 1024].map((s) => (
                      <button
                        ...
                        className={cn(
                          "h-8 rounded-lg border text-[8px] font-black uppercase transition-all",
                          iconSize === s
              ...
              <div className="pt-4 grid grid-cols-1 gap-3">
                <Button
                  ...
                  className="h-14 w-full bg-primary text-white font-black uppercase text-[10px] tracking-widest rounded-xl shadow-xl active:scale-95"
                >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button
                    ...
                    className="h-11 border-border bg-white/5 text-foreground/60 text-[8px] font-black uppercase"
                  >
                    SVG Vector
                  </Button>
                  <Button
                ...
                <Button
                  ...
                  className="h-11 w-full border-white/5 bg-secondary/50 text-primary text-[8px] font-black uppercase"
                >
                  {isCopied === "code" ? (
              ...
              {error && (
                <div className="flex flex-col items-center gap-8 py-20 text-center animate-in shake duration-500">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-headline font-black text-destructive uppercase">
                      Reference Failure
                    </h3>
                    <p className="text-[11px] text-foreground/40 font-bold uppercase max-w-sm mx-auto leading-relaxed">
                      {error}
                    </p>
                  </div>
                </div>
                  ...
                  {results.map((icon) => (
                    ...
                    >
                      ...
                      <div className="absolute bottom-2 left-0 right-0 px-2 opacity-0 group-hover/item:opacity-100 transition-opacity">
                        <p className="text-[6px] font-black uppercase text-foreground/40 text-center truncate">
                          {icon.name}
                        </p>
                      </div>
                    </button>
            ...
            {selectedIcon && (
              <div className="p-8 sm:p-12 border-t border-white/5 bg-[#0a0a0c] flex flex-col md:flex-row items-center gap-12 shrink-0 animate-in slide-in-from-bottom-6">
                <div className="relative group/master shrink-0">
                  ...
                  <div className="relative w-40 h-40 rounded-[2.5rem] bg-white dark:bg-background border-4 border-white dark:border-white/5 flex items-center justify-center shadow-2xl ring-1 ring-border overflow-hidden bg-checkered">
                    <img
                ...
                <div className="flex-1 w-full space-y-6 text-center md:text-left min-w-0">
                  <div className="space-y-2">
                    ...
                    <h2 className="text-3xl sm:text-5xl font-headline font-black text-foreground uppercase tracking-tight truncate">
                      {selectedIcon.name}
                    </h2>
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-white/5 border border-white/10 space-y-4">
                    <div className="flex items-center justify-between px-1">
                      <Label className="text-[10px] font-black text-foreground/30 uppercase tracking-[0.2em]">
                        Source CDN Vector
                      </Label>
                      <button
                        onClick={() =>
                          handleCopy(
                            `https://api.iconify.design/${selectedIcon.prefix}/${selectedIcon.name}.svg`,
                            "CDN",
                          )
                        }
                        className="text-[8px] font-black text-primary/60 hover:text-primary uppercase tracking-widest"
                      >
                        Copy URL
                      </button>
                    </div>
                    <div className="bg-background p-3 rounded-xl border border-white/5 shadow-inner">
                      <p className="text-[10px] font-mono text-foreground/30 truncate">
                        https://api.iconify.design/{selectedIcon.prefix}/
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All chromatic re-matricing and format conversions occur 100%
                  locally in your browser memory. No identity data is stored.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  PNG and ICO exports are generated via a high-performance
                  hardware-accelerated 2D context to ensure peak pixel clarity.
                </p>
              </div>
            </div>
          </div>

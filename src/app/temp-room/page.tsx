    ...
    connection.on("data", (data: any) => {
      switch (data.type) {
        case "name-sync":
          ...
          toast({
            title: "Peer Joined",
            description: `${data.value} is now connected.`,
          });
          break;
        case "text":
        ...
        case "pin-fail":
          toast({
            variant: "destructive",
            title: "Access Denied",
            description: "Incorrect room PIN.",
          });
          break;
        case "file-meta":
          setInboundFile({
          ...
          setInboundFile((prev) =>
            prev
              ? {
                  ...prev,
                  progress: Math.round(
                    ((receivedChunksRef.current.length * CHUNK_SIZE) /
                      prev.size) *
                      100,
                  ),
                }
        ...
        case "file-end":
          const blob = new Blob(receivedChunksRef.current, {
            type: inboundFile?.mime,
          });
          setInboundFile((prev) =>
            prev
              ? { ...prev, progress: 100, blob, url: URL.createObjectURL(blob) }
              : null,
          );
          toast({
            title: "File Received",
            description: "Asset ready for download.",
          });
          break;
      }
    });
  ...
  const sendFile = async () => {
    ...
    if (outboundFile.size > 10 * 1024 * 1024) {
      toast({
        variant: "destructive",
        title: "Heavy Payload",
        description: "Max file size is 10MB for P2P sync.",
      });
      return;
    }
    ...
    conn.send({
      type: "file-meta",
      name: outboundFile.name,
      size: outboundFile.size,
      mime: outboundFile.type,
    });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Advanced live clipboard and secure data tunnel. Collaborate in
              real-time with encrypted P2P messaging and zero-storage file
              sharing.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[500px] bg-secondary/10">
            ...
            <CardHeader className="py-4 border-b border-white/5 bg-background flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
              ...
              {status === "connected" && (
                <div className="flex items-center gap-2 bg-background p-1 rounded-xl border border-white/5">
                  <button
                  ...
                  <button
                    ...
                    className={cn(
                      "px-3 py-1.5 rounded-lg text-[8px] font-black uppercase transition-all relative",
                      activeTab === "chat"
                        ? "bg-primary text-white"
                        : "text-white/20 hover:text-white",
                    )}
            ...
            <CardContent className="flex-1 flex flex-col p-6 sm:p-10 relative overflow-hidden">
              ...
              {status === "idle" && (
                <div className="flex-1 flex flex-col items-center justify-center gap-12 animate-in fade-in zoom-in duration-500">
                    ...
                    >
                      ...
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-[9px] text-white/40 font-bold uppercase tracking-tighter">
                          Generate 5-Char Matrix
                        </p>
                      </div>
                    </button>

                    <div className="p-10 rounded-[3rem] bg-background border border-white/5 flex flex-col items-center gap-8 group/join relative">
                      ...
                      <div className="w-full space-y-4">
                        ...
                        <Button
                          onClick={handleJoin}
                          disabled={isConnecting || peerIdInput.length < 5}
                          className="w-full h-14 bg-white text-black font-black uppercase tracking-widest text-[10px] rounded-2xl hover:bg-white/90 shadow-xl"
                        >
                          {isConnecting ? (
                            <Loader2 className="w-4 h-4 animate-spin" />
                          ) : (
                            "Access Room Protocol"
                          )}
                        </Button>
                      </div>
                    </div>
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-center gap-4 group/name">
                    <Label className="text-[10px] font-black uppercase text-foreground/30 ml-2">
                      Your Device ID:
                    </Label>
                    <Input
                      value={myName}
              ...
              {status === "waiting" && (
                <div className="flex-1 flex flex-col items-center justify-center gap-12 animate-in fade-in duration-700 text-center">
                  <div className="space-y-6 w-full max-w-lg">
                    <p className="text-[10px] font-black uppercase text-primary tracking-[0.5em]">
                      Identity Token (Room Code)
                    </p>
                    <div className="relative group/code">
                      <div className="absolute -inset-4 bg-primary/10 blur-3xl opacity-0 group-hover/code:opacity-100 transition-opacity" />
                      <div className="relative p-8 bg-background rounded-[3rem] border-2 border-primary/20 flex flex-col sm:flex-row items-center justify-center gap-6 shadow-2xl">
                        <h3 className="text-4xl sm:text-6xl font-mono font-bold text-white tracking-[0.2em] select-all">
                          {roomCode}
                        </h3>
                        <div className="flex gap-2">
                          <Button
                            ...
                            variant="outline"
                            className="h-14 w-14 rounded-2xl bg-white/5 border-white/10 text-white"
                          >
                            {isCopied === "code" ? (
                  ...
                  {isHost && (
                    <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border space-y-4 animate-in slide-in-from-bottom-4">
                      ...
                      />
                      <p className="text-[9px] text-foreground/20 font-bold uppercase">
                        If set, peer must enter this PIN to unlock data.
                      </p>
                    </div>
                  )}
                </div>
              ...
              {status === "verifying" && (
                <div className="flex-1 flex flex-col items-center justify-center gap-10 animate-in zoom-in-95">
                  ...
                  <div className="text-center space-y-2">
                    <h3 className="text-xl font-headline font-black text-foreground uppercase">
                      Access Restricted
                    </h3>
                    <p className="text-[10px] text-foreground/40 font-bold uppercase tracking-widest">
                      Enter the security PIN provided by the host
                    </p>
                  </div>
                  <div className="w-full max-w-xs space-y-4">
                    <Input
                      value={enteredPin}
                      onChange={(e) =>
                        setEnteredPin(e.target.value.substring(0, 4))
                      }
                      placeholder="••••"
                    ...
                    <Button
                      ...
                      className="w-full h-14 bg-primary text-white font-black rounded-2xl shadow-xl"
                    >
                      Unlock Session
                    </Button>
                  </div>
                ...
                <div className="flex-1 flex flex-col h-full animate-in fade-in duration-500">
                  ...
                  {activeTab === "notepad" && (
                    <div className="flex-1 flex flex-col gap-6 animate-in slide-in-from-bottom-2">
                      <Textarea
                        ...
                        className="flex-1 min-h-[40vh] bg-background border-white/10 rounded-[2.5rem] p-10 text-lg font-medium leading-relaxed resize-none focus:ring-primary/40 text-white custom-scrollbar shadow-inner"
                      />
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <Button
                          onClick={() => handleCopy(sharedText, "notepad")}
                          className="h-14 bg-primary text-white font-black uppercase text-[9px] tracking-widest rounded-2xl shadow-xl"
                        >
                          {isCopied === "notepad" ? (
                        ...
                        <Button
                          ...
                          className="h-14 bg-white/5 border-white/10 text-white/60 font-black uppercase text-[9px] rounded-2xl"
                        >
                          Paste Data
                        </Button>
                        <Button
                        ...
                        <Button
                          ...
                          className="h-14 bg-white/5 border-white/10 text-white/60 font-black uppercase text-[9px] rounded-2xl hover:text-destructive"
                        >
                          Clear
                        </Button>
                      </div>
                  ...
                  {activeTab === "chat" && (
                    <div className="flex-1 flex flex-col gap-6 animate-in slide-in-from-bottom-2 h-full max-h-[500px]">
                      <div
                        ref={chatScrollRef}
                        className="flex-1 overflow-y-auto custom-scrollbar space-y-4 p-4 rounded-3xl bg-background border border-white/5"
                      >
                        {chatMessages.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center opacity-20 gap-4">
                            <MessageSquare className="w-12 h-12 text-primary" />
                            <p className="text-[10px] font-black uppercase tracking-widest">
                              Protocol Buffer Empty
                            </p>
                          </div>
                        ) : (
                          chatMessages.map((msg) => (
                            ...
                            >
                              <div className="flex items-center gap-2 px-1">
                                <span className="text-[8px] font-black text-foreground/40 uppercase">
                                  {msg.sender}
                                </span>
                                <span className="text-[7px] font-bold text-foreground/20">
                  ...
                  {activeTab === "files" && (
                    <div className="flex-1 flex flex-col gap-8 animate-in slide-in-from-bottom-2">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        {/* Outbound */}
                        <div className="p-8 rounded-[3rem] bg-background border border-white/5 flex flex-col gap-6 items-center text-center">
                          ...
                          <div
                            ...
                            className="w-full h-32 rounded-2xl border-2 border-dashed border-white/10 hover:border-primary/20 flex flex-col items-center justify-center bg-background/20 cursor-pointer group/file"
                          >
                            {outboundFile ? (
                              <div className="space-y-2">
                                <CheckCircle2 className="w-8 h-8 text-primary mx-auto" />
                                <p className="text-[10px] font-bold text-white truncate max-w-[200px]">
                                  {outboundFile.name}
                                </p>
                              </div>
                            ) : (
                          ...
                          <Button
                            onClick={sendFile}
                            disabled={!outboundFile || isProcessing}
                            className="w-full h-12 bg-primary text-white font-black uppercase tracking-widest text-[9px] rounded-xl shadow-lg"
                          >
                            Send Stream
                          </Button>
                        </div>
                            ...
                            {inboundFile ? (
                              <div className="w-full space-y-6 animate-in zoom-in">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <p className="text-[9px] font-black text-foreground/30 uppercase">
                                    {(inboundFile.size / 1024 / 1024).toFixed(
                                      1,
                                    )}{" "}
                                    MB
                                  </p>
                                ...
                                {inboundFile.progress < 100 ? (
                                  <div className="space-y-2">
                                    <Progress
                                      value={inboundFile.progress}
                                      className="h-1"
                                    />
                                    <span className="text-[8px] font-black text-primary uppercase">
                                      {inboundFile.progress}% Received
                                    </span>
                                  </div>
                                ) : (
                                  <Button
                                    asChild
                                    className="w-full h-12 bg-white text-black font-black uppercase text-[9px] rounded-xl shadow-xl"
                                  >
                                    <a
                                      href={inboundFile.url}
                                      download={inboundFile.name}
                                    >
              ...
              {status === "closed" && (
                <div className="flex-1 flex flex-col items-center justify-center gap-10 animate-in zoom-in duration-500 text-center">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-3xl font-headline font-black uppercase text-foreground">
                      Room Definitively Closed
                    </h3>
                    <p className="text-[11px] text-foreground/40 font-bold uppercase tracking-widest max-w-sm mx-auto leading-relaxed">
                      The P2P tunnel has been destroyed. All shared data was
                      held in volatile memory and is now definitively purged.
                    </p>
                  </div>
                  <Button
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000">
          ...
          <Card className="glass-card border-border shadow-xl">
            <CardHeader className="py-6 border-b border-white/5 bg-white/2">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
                <Activity className="w-5 h-5 text-primary" /> Matrix
                Connectivity
              </CardTitle>
            </CardHeader>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/20 flex flex-col items-center justify-center text-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase text-foreground/40">
                      Duration
                    </span>
              ...
              {[
                {
                  icon: ShieldCheck,
                  title: "Zero Cloud",
                  desc: "No messages are stored in any database or cloud infrastructure.",
                },
                {
                  icon: Lock,
                  title: "Identity Locked",
                  desc: "Optional PIN protocol ensures only authorized peers can sync content.",
                },
                {
                  icon: Activity,
                  title: "Direct Tunnel",
                  desc: "Encrypted memory-to-memory streaming via WebRTC DataChannels.",
                },
              ].map((tip, i) => (
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-4">
            ...
            <p className="text-[10px] text-foreground/40 font-bold uppercase leading-relaxed">
              The studio remains active in the background. Only leave the room
              via the explicit Leave Session protocol to ensure your shared
              matrix is correctly closed.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="w-full max-w-lg space-y-10 text-center">
            <div className="space-y-4">
              <h2 className="text-3xl font-headline font-black text-white uppercase tracking-tight">
                Scan to <span className="text-primary italic">Sync</span>
              </h2>
              <p className="text-white/20 text-xs font-black uppercase tracking-widest">
                Hold your camera to the token matrix
              </p>
            </div>
            ...
            <div className="relative group/qr mx-auto w-fit">
              ...
              <div className="relative p-6 bg-white rounded-[3.5rem] shadow-2xl transition-transform duration-700 group-hover/qr:scale-105 border-4 border-primary/20">
                <div
                  ref={qrRef}
                  className="w-[250px] h-[250px] bg-white rounded-3xl"
                />
              </div>
            </div>

            <div className="flex flex-col items-center gap-6">
              <p className="text-2xl font-mono font-bold text-white tracking-[0.4em]">
                {roomCode}
              </p>
              <Button
                onClick={() => setShowQr(false)}
                className="h-16 px-12 bg-primary text-white font-black rounded-2xl text-xs uppercase tracking-widest shadow-xl shadow-primary/30"
              >
                Exit Preview
              </Button>
            </div>
        ...
        <AlertDialogContent className="glass-card border-white/10 rounded-[2.5rem] p-8 max-w-sm">
          <AlertDialogHeader className="space-y-4">
            ...
            <AlertDialogDescription className="text-[11px] font-medium text-foreground/40 uppercase tracking-widest leading-relaxed text-center">
              Are you sure you want to leave this room? This will definitively
              destroy the P2P tunnel and purge all volatile shared data.
            </AlertDialogDescription>
          </AlertDialogHeader>

...
export default function AudioJoinerPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({
      title: "Assets Queued",
      description: `Added ${selectedFiles.length} tracks to the pipeline.`,
    });
  ...
  const joinAudio = async () => {
    if (files.length < 2) {
      toast({
        variant: "destructive",
        title: "Payload Incomplete",
        description: "At least 2 tracks are required for merging.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Tracks successfully merged into a single file.",
      });
    } catch (err: any) {
  ...
  const handleClear = () => {
    ...
    toast({
      title: "Studio Reset",
      description: "Pipeline cleared and memory purged.",
    });
  };
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl">
          Professional-grade audio merging utility. Combine multiple MP3, WAV,
          or M4A tracks into a single high-fidelity track locally in your
          browser.
        </p>
      </div>
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group">
            ...
            <CardContent className="pt-10 space-y-8">
              ...
              >
                ...
                <p className="text-[10px] font-black uppercase text-foreground/40 tracking-widest group-hover:text-primary transition-colors">
                  Import Audio Tracks
                </p>
                <p className="text-[8px] text-foreground/20 uppercase font-bold mt-2">
                  MP3, WAV, M4A up to 20MB each
                </p>
                <input
                  type="file"
                  ref={fileInputRef}
              ...
              {files.length > 0 && (
                <div className="space-y-4 animate-in fade-in duration-500">
                  <div className="flex items-center justify-between px-2">
                    ...
                    <button
                      onClick={handleClear}
                      className="text-[10px] font-black uppercase text-destructive hover:opacity-70 transition-all"
                    >
                      Purge All
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-[440px] overflow-y-auto pr-2 custom-scrollbar">
                    {files.map((f, index) => (
                      <div
                        key={f.id}
                        className="group/item flex items-center gap-4 p-5 rounded-3xl bg-secondary border border-border hover:border-primary/20 transition-all relative overflow-hidden"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-foreground truncate uppercase pr-20">
                            {f.name}
                          </p>
                          <div className="flex items-center gap-3 mt-1">
                            ...
                            <span className="text-foreground/10 text-[8px]">
                              •
                            </span>
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-lg font-headline font-black text-foreground uppercase">
                ...
                {mergedUrl && (
                  <div className="space-y-8 w-full animate-in zoom-in duration-500">
                    ...
                    >
                      <a
                        href={mergedUrl}
                        download={`master-bundle-${Date.now()}.mp3`}
                      >
              ...
              <div className="p-6 rounded-2xl bg-primary/5 border border-primary/10 flex items-start gap-5">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                    Audio synthesis occurs entirely on your device using
                    WebAssembly. Your tracks never leave your machine, ensuring
                    100% data security.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group transition-all hover:bg-secondary/80">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-foreground/60 leading-relaxed font-medium">
                    Automatic re-encoding to 192kbps for bitstream consistency.
                  </p>
                </div>
              </div>
            </CardContent>

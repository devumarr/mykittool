  ...
  const executeGeneration = async (e?: React.FormEvent) => {
    ...
    if (!topic.trim() && !caption.trim()) {
      toast({
        ...
        description: "Provide a topic or caption matrix.",
      });
      return;
    }
    ...
    try {
      const response = await fetchHashtagsAction(
        topic,
        caption,
        platform,
        style,
      ...
      toast({
        ...
        description: `Active node: ${response.node}`,
      });
    } catch (err) {
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Professional reach-based bucketing. Isolate high-fidelity tags for
            growth, specific niches, and professional networks using multi-node
            failover logic.
          </p>
        </div>
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group">
            ...
            <CardContent className="pt-8 space-y-8">
              <form onSubmit={executeGeneration} className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <span className="text-[8px] font-black text-primary/40 uppercase">
                        Optional
                      </span>
                    </div>
                    <Textarea
                ...
                <Button
                  type="submit"
                  disabled={isLoading || (!topic.trim() && !caption.trim())}
                  className="w-full h-16 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/30 text-xs uppercase tracking-widest active:scale-95 transition-all"
                >
                  {isLoading ? (
          ...
          <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Linguistic discovery is processed anonymously. Search strings
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={() =>
                      handleCopy(
                        [
                          ...result.buckets.broad,
                          ...result.buckets.niche,
                          ...result.buckets.ultra,
                        ],
                        "all",
                      )
                    }
                    variant="outline"
                    size="sm"
                    className="h-9 px-4 rounded-xl border-white/10 bg-white/5 text-[8px] font-black uppercase tracking-widest"
                  >
                    {isCopied === "all" ? (
              ...
              {result && !isLoading && (
                <div className="space-y-12 animate-in zoom-in-95 duration-500">
                    ...
                    {[
                      {
                        ...
                        label: "Broad Reach",
                        color: "text-primary",
                        data: result.buckets.broad,
                      },
                      {
                        id: "niche",
                        label: "Niche Matrix",
                        color: "text-emerald-500",
                        data: result.buckets.niche,
                      },
                      {
                        id: "ultra",
                        label: "Long-Tail",
                        color: "text-amber-500",
                        data: result.buckets.ultra,
                      },
                    ].map((bucket) => (
                      <div key={bucket.id} className="space-y-4">
                        <div className="flex items-center justify-between px-1">
                          ...
                          <button
                            onClick={() => handleCopy(bucket.data, bucket.id)}
                        ...
                        <div className="p-5 rounded-[2rem] bg-secondary/30 border border-white/5 flex flex-wrap gap-1.5 max-h-[200px] overflow-y-auto custom-scrollbar">
                          {bucket.data.map((t, idx) => (
                            <Badge
                              key={idx}
                              variant="outline"
                              className="bg-background/50 border-white/5 text-[8px] font-bold uppercase py-0.5 px-2 text-foreground/40"
                            >
                              {t}
                            </Badge>
                          ))}
                  ...
                  <div className="p-8 rounded-[3.5rem] bg-primary/[0.03] border border-primary/10 space-y-6 relative overflow-hidden group/mix">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-[0.2em]">
                            {platform} Standard • {result.readyMix.length} Tags
                          </p>
                        </div>
                      </div>
                      <Button
                        onClick={() => handleCopy(result.readyMix, "mix")}
                        className={cn(
                    ...
                    <div className="p-6 bg-background rounded-3xl border border-white/5 shadow-inner relative z-10">

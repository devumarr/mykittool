...
export default function PdfToImagePage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile.type !== "application/pdf") {
      toast({
        ...
        description: "Only PDF documents are supported for conversion.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${pdf.numPages} pages identified. Ready for rendering.`,
      });
    } catch (err) {
      console.error(err);
      toast({
  ...
  const convertPdf = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Converted ${currentResults.length} pages to imagery.`,
      });
    } catch (err) {
      console.error(err);
      toast({
        ...
        description: "Internal error during rendering cycle.",
      });
    } finally {
      setIsProcessing(false);
    }
  ...
  const downloadAll = async () => {
    ...
    if (results.length === 1) {
      ...
      results.forEach((res) => {
        zip.file(
          `page_${res.index}.${format === "image/png" ? "png" : "jpg"}`,
          res.blob,
        );
      });
              ...
              {file && (
                <div className="space-y-8 animate-in zoom-in duration-500">
                  <div className="space-y-4">
                    ...
                    <Tabs
                      value={rangeMode}
                      onValueChange={(v: any) => setRangeMode(v)}
                      className="w-full"
                    >
                  ...
                  <div className="flex gap-4 pt-4">
                    <Button
                      onClick={convertPdf}
                      disabled={
                        isProcessing || !file || targetPages.length === 0
                      }
              ...
              {!file && !isProcessing ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar p-1">
                  {results.map((res) => (
                    <div
                      key={res.index}
                      className="group relative aspect-[3/4] bg-white rounded-2xl overflow-hidden shadow-xl ring-1 ring-border animate-in zoom-in duration-300"
                    >
                      <img
                        src={res.dataUrl}
                        alt={`Page ${res.index}`}
                      ...
                      <div className="absolute inset-0 bg-background opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-3 backdrop-blur-sm">
                        ...
                        >
                          <a
                            href={res.dataUrl}
                            download={`page_${res.index}.${format.split("/")[1]}`}
                          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  300 DPI support ensures peak fidelity for document archiving
            ...
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Automatic ZIP packaging for high-volume conversion cycles
                  ensures efficient local storage.
                </p>
              </div>
            </div>
          </div>

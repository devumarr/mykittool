...
export default function TranslatePage() {
    ...
    try {
      ...
      if (data.responseData && data.responseData.translatedText) {
        ...
        toast({
          ...
          description: "Linguistic matrix synchronized.",
        });
      } else {
      ...
      toast({
        ...
        description:
          "The translation node is currently restricted. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const swapLanguages = () => {
    ...
    toast({
      title: "Matrix Swapped",
      description: "Translation direction inverted.",
    });
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional-grade bilingual translation matrix. Translate between
              English and Urdu with clinical precision and zero-storage local
              privacy.
            </p>
          </div>
        ...
        <div className="lg:col-span-6 space-y-6 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[468px]">
            ...
            <CardContent className="flex-1 flex flex-col pt-8">
              <div className="flex-1 relative group/output rounded-[2rem] bg-background/5 dark:bg-background border border-border overflow-hidden shadow-inner p-8">
                {resultText ? (
                  <p
                    className={cn(
              ...
              {resultText && (
                <div className="mt-8 grid grid-cols-2 gap-4 animate-in zoom-in duration-300">
                  <Button
                    onClick={handleCopy}
                    className="h-14 bg-secondary border border-border text-foreground font-black uppercase text-[10px] tracking-widest rounded-2xl hover:bg-white/5 transition-all"
                  >
                    {isCopied ? (
                  ...
                  <Button
                    ...
                    className="h-14 border-white/10 bg-white/5 text-white/40 rounded-2xl flex items-center justify-center"
                  >
            ...
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Your text is processed strictly in your browser session. No
                  data is logged or stored in our matrix.
                </p>
              </div>
            </div>
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Utilizing the MyMemory high-speed API for clinical-grade
                  linguistic mapping.
                </p>
              </div>
            </div>

 ...
 * Supports purpose, recipient, tone, length, and language Preview.
 */
...
export async function POST(req: NextRequest) {
  try {
    const { purpose, recipient, tone, length, language, extra } =
      await req.json();
          ...
          "https://api.groq.com/openai/v1/chat/completions",
          {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "system",
                  content: `You are an expert executive communications assistant. Output JSON only in ${language}.`,
                },

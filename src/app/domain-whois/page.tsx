  ...
  const handleCheck = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.exists) {
        toast({
          title: "Registry Null",
          description: "This domain appears to be available.",
        });
      } else {
        toast({
          title: "Matrix Synced",
          description: "Whois data Isolated successfully.",
        });
      }
    } catch (err: any) {
      setError(
        "Discovery Protocol Error: Could not connect to registry nodes.",
      );
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight leading-none">
              Domain Whois{" "}
              <span className="text-primary italic">& Age Studio</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional registry auditing engine. Isolate domain birth-dates,
              registrar metadata, and security status locally using the global
              RDAP protocol matrix.
            </p>
          </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Our engine prioritizes the RDAP protocol for clinical
                    accuracy while maintaining 100% hardware-native privacy. No
                    lookup history is logged.
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              {result && !isLoading && (
                <div className="w-full space-y-10 animate-in zoom-in-95 duration-500">
                  ...
                  {isNewDomain && (
                    <div className="p-6 rounded-[2.5rem] bg-red-600/10 border border-red-600/20 flex items-center gap-5 shadow-xl shadow-red-600/5">
                      ...
                      <div className="space-y-1">
                        <h4 className="text-[11px] font-black uppercase text-red-600 tracking-widest">
                          Protocol Alert: New Domain
                        </h4>
                        <p className="text-[11px] text-foreground/50 font-bold uppercase tracking-tighter">
                          Registered within last 30 days. Exercise caution.
                        </p>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {[
                        ...
                        },
                        {
                          ...
                          label: "Domain Age",
                          val: ageData
                            ? `${ageData.years} Years, ${ageData.days} Days`
                            : "—",
                        },
                        {
                          icon: ShieldCheck,
                          label: "Expiry Matrix",
                          val: result.expiryDate
                            ? new Date(result.expiryDate).toLocaleDateString()
                            : "—",
                        },
                        {
                          icon: Server,
                          label: "Registrar Node",
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[13px] font-bold text-foreground truncate uppercase">
                              {item.val}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="p-8 rounded-[3rem] bg-secondary border border-border space-y-4 shadow-inner">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.nameservers.map((ns, i) => (
                          <span
                            key={i}
                            className="px-3 py-1 rounded-lg bg-background border border-border text-[11px] font-mono font-bold text-foreground/60 uppercase"
                          >
                            {ns}
                          </span>
                        ))}
                  ...
                  <div className="pt-6 border-t border-white/5 flex flex-col sm:flex-row gap-4">
                    <Button
                      onClick={handleCopy}
                      className="h-16 flex-1 bg-white text-black hover:bg-white/90 font-black rounded-2xl flex items-center justify-center gap-4 text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                    >
                      {isCopied ? (
                    ...
                    >
                      <a
                        href={`https://rdap.org/domain/${result.domain}`}
                        target="_blank"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-[2rem] bg-amber-500/5 border border-amber-500/10 flex items-start gap-4">
              ...
              <p className="text-[10px] text-foreground/40 font-bold leading-relaxed uppercase">
                RDAP data availability depends on the TLD registry policies. If
                private, owner details are automatically redacted for clinical
                privacy compliance.
              </p>
            </div>
            <div className="p-6 rounded-[2rem] bg-primary/5 border border-primary/10 flex items-start gap-4">
              ...
              <p className="text-[10px] text-foreground/40 font-bold leading-relaxed uppercase">
                All lookup signals are processed via secure server actions. Your
                browser hardware identifiers are never shared with the target
                registry.
              </p>
            </div>
          </div>

...
export default function HideMessagePhotoPage() {
  ...
  const capacityMetrics = useMemo(() => {
    ...
    const finalCapacity =
      strength === "strong" ? Math.floor(availableBytes / 2) : availableBytes;

    const usedBytes = embeddedFile
      ? embeddedFile.data.length
      : new TextEncoder().encode(message).length;
    const percent = Math.min(
      100,
      Math.round((usedBytes / finalCapacity) * 100),
    );
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          toast({
            title: "Carrier Integrated",
            description: "Ready for encoding.",
          });
        };
        img.src = result;
      };
  ...
  const executeHide = async () => {
    ...
    const encoder = new TextEncoder();
    const contentBytes = embeddedFile
      ? embeddedFile.data
      : encoder.encode(message);
    const filenameBytes = embeddedFile
      ? encoder.encode(embeddedFile.name)
      : new Uint8Array(0);
    ...
    const fullBinary = new Uint8Array(
      header.length + filenameBytes.length + contentBytes.length,
    );
    ...
    if (password) {
      ...
      bits = bits.map(
        (b, i) => b ^ ((key[Math.floor(i / 8) % key.length] >> (i % 8)) & 1),
      );
    }
    ...
    toast({
      ...
      description: "PNG master generated locally.",
    });
  };
  ...
  const executeReveal = async () => {
    ...
    const decodeBits = (bitSource: number[], isStrong: boolean) => {
      const bytes = new Uint8Array(
        Math.floor(bitSource.length / (isStrong ? 16 : 8)),
      );
      let bitPtr = 0;
    ...
    try {
      ...
      const magicCheck = String.fromCharCode(
        ...headerBytes.slice(0, MAGIC.length),
      );

      if (magicCheck !== MAGIC) {
        throw new Error(
          "Invalid matrix identification. Incorrect password or sanitized photo.",
        );
      }
      ...
      setIntegrityStatus(
        actualChecksum === expectedChecksum ? "ok" : "damaged",
      );

      if (isFile) {
        setRevealedData({
          file: { name: "extracted_file", blob: new Blob([payload]) },
        });
      } else {
    ...
    if (file) {
      if (file.size > MAX_FILE_SIZE) {
        toast({
          ...
          description: "Embedded files are capped at 50KB.",
        });
        return;
      }
      const reader = new FileReader();
    ...
    <div className="container mx-auto px-4 sm:px-6 py-12 md:py-20 max-w-full overflow-x-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              Hide Message{" "}
              <span className="text-primary italic">in Photo Pro</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Clinical-grade steganography. Embed encrypted text or small files
              invisibly into pixel buffers with 1:1 hardware fidelity.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[400px] max-h-[50vh] lg:max-h-none lg:min-h-[700px] bg-background/60">
          ...
          <div className="p-6 rounded-[2.5rem] bg-amber-500/5 border border-amber-500/10 flex items-start gap-5">
            ...
            <div className="space-y-1">
              <h4 className="text-[11px] font-black text-amber-700 uppercase tracking-widest">
                Compression Advisory
              </h4>
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Sending encoded images via WhatsApp or Facebook will destroy the
                hidden bitstream due to lossy re-compression. Use direct file
                sharing or email to preserve integrity.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000">
          ...
          >
            <TabsList className="grid grid-cols-2 bg-secondary p-1.5 rounded-2xl h-14 mb-8 border border-white/5 shadow-2xl">
              <TabsTrigger
                value="hide"
                className="rounded-xl text-[9px] font-black uppercase tracking-widest data-[state=active]:bg-background"
              >
                Hide Data
              </TabsTrigger>
              <TabsTrigger
            ...
            <div className="min-h-[600px]">
                ...
                <Card className="glass-card border-border shadow-2xl">
                  ...
                  <CardContent className="pt-8 space-y-8">
                      ...
                      <div className="flex justify-between items-center px-1">
                        ...
                        <span
                          className={cn(
                            ...
                            capacityMetrics.percent > 90
                              ? "text-red-500"
                              : "text-primary/60",
                          )}
                        >
                          Used {capacityMetrics.percent}% of{" "}
                          {capacityMetrics.total.toLocaleString()}
                        </span>
                      </div>
                      ...
                      <Tabs defaultValue="text" className="w-full">
                        ...
                        <TabsContent value="file">
                          <div
                            ...
                            className="h-32 rounded-2xl border-2 border-dashed border-border flex flex-col items-center justify-center bg-secondary/30 cursor-pointer group/file"
                          >
                            {embeddedFile ? (
                              ...
                              <div className="text-center">
                                ...
                                <span className="text-[8px] font-black uppercase text-foreground/30">
                                  Select TXT/JSON (Max 50KB)
                                </span>
                              </div>
                            )}
                            <input
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 bg-secondary p-1 rounded-xl border border-border h-11">
                          <button
                            ...
                            className={cn(
                              "rounded-lg text-[8px] font-black uppercase transition-all",
                              strength === "fast"
                    ...
                    <Button
                      ...
                      disabled={
                        !image ||
                        (!message.trim() && !embeddedFile) ||
                        isProcessing ||
                        capacityMetrics.percent > 100
                      }
              ...
              >
                <Card className="glass-card border-border shadow-2xl">
                  <CardHeader className="py-6 border-b border-white/5 bg-white/2">
                    <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
                      <Unlock className="w-5 h-5 text-primary" /> Extraction
                      Matrix
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-8 space-y-8">
                      ...
                      <div className="space-y-6 animate-in slide-in-from-top-4 duration-500">
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">
                              {integrityStatus === "ok"
                                ? "Bitstream Integrity OK"
                                : "Data Matrix Corrupted"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <Button
                            onClick={handleCopy}
                            variant="outline"
                            className="h-12 rounded-xl border-border bg-white/5 text-[9px] font-black uppercase tracking-widest"
                          >
                            {isCopied ? (
                          ...
                          <Button
                            variant="outline"
                            onClick={handleClear}
                            className="h-12 rounded-xl border-border bg-secondary text-[9px] font-black uppercase tracking-widest hover:text-destructive"
                          >
          ...
          <Card className="glass-card border-border shadow-xl overflow-hidden">
            <CardHeader className="py-4 border-b border-white/5 bg-secondary/30 flex flex-row items-center justify-between">
              ...
              <button
                onClick={() => {
                  setHistory([]);
                  localStorage.removeItem("mykit_steg_history");
                }}
                className="text-[8px] font-black uppercase text-foreground/20 hover:text-destructive transition-colors"
              >
                Purge
              </button>
            </CardHeader>
                  ...
                  {history.map((item) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="text-[10px] font-bold text-foreground/60 group-hover:text-foreground truncate max-w-[120px]">
                          {item.name}
                        </span>
                      </div>
                      <span className="text-[8px] font-black text-foreground/20 uppercase">
      ...
      {image && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#0a0a0c]/80 backdrop-blur-3xl border-t border-white/10 z-[100] lg:hidden flex gap-3 animate-in slide-in-from-bottom-full duration-500">
          <Button
            ...
            className="flex-1 h-14 bg-primary text-white font-black rounded-2xl flex items-center justify-center gap-3 text-xs uppercase tracking-widest shadow-2xl"
          >
          ...
          {revealedData && (
            <Button
              onClick={handleCopy}
              variant="outline"
              className="h-14 px-6 bg-secondary border-white/10 text-white/40 font-black rounded-2xl text-[9px] uppercase"
            >
              COPY
            </Button>
          )}

    ...
    if (selectedFile) {
      ...
      if (selectedFile.size > limit * 1024 * 1024) {
        toast({
          variant: "destructive",
          title: "Heavy Payload",
          description: `Standard limit for ${type} is ${limit}MB.`,
        });
        return;
      }
      ...
      if (type === "image") {
        ...
        img.onload = () => {
          ...
          toast({
            title: "Visual Imported",
            description: "Matrix ready for watermarking.",
          });
        };
        img.src = url;
      } else {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          setWmLogoImg(img);
          toast({
            title: "Logo Integrated",
            description: "Alpha channel detected.",
          });
        };
        img.src = src;
      };
    ...
    (ctx: CanvasRenderingContext2D, w: number, h: number) => {
      ...
      const renderSingle = (dx: number, dy: number) => {
        ...
        if (wmType === "text") {
          ...
          ctx.textAlign = positionKey.endsWith("l")
            ? "left"
            : positionKey.endsWith("r")
              ? "right"
              : "center";
          ...
          const lx = positionKey.endsWith("l")
            ? 0
            : positionKey.endsWith("r")
              ? -lW
              : -lW / 2;
          const ly = positionKey.startsWith("t")
            ? 0
            : positionKey.startsWith("b")
              ? -lH
              : -lH / 2;
          ...
          for (let ty = 0; ty < h + stepY; ty += stepY) {
            ...
            if (wmType === "text") {
              ...
              ctx.drawImage(
                ...
                -ts / 2,
                ts,
                (wmLogoImg.height / wmLogoImg.width) * ts,
              );
            }
            ctx.restore();
          }
  ...
  const handleExport = async () => {
    ...
    if (mediaType === "image") {
      ...
      toast({
        ...
        description: "Image saved with custom watermark.",
      });
    } else {
      ...
      recorder.onstop = () => {
        ...
        toast({
          ...
          description: "Video master synthesized locally.",
        });
      };
    ...
    <div className="container mx-auto px-4 sm:px-6 py-12 md:py-20 max-w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              Custom{" "}
              <span className="text-primary italic">Watermark Studio</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Advanced clinical asset protection. Apply textual or visual
              watermarks to photos and videos locally with 1:1 pixel fidelity.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[350px] max-h-[45vh] lg:max-h-none lg:min-h-[700px] bg-background/60">
              ...
              {!file ? (
                ...
                <div className="relative w-full h-full flex items-center justify-center group/canvas">
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover/canvas:opacity-100 transition-opacity pointer-events-none">
                    <div className="bg-background/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-2">
          ...
          {isProcessing && mediaType === "video" && (
            <div className="p-8 rounded-[2.5rem] bg-primary/10 border border-primary/20 space-y-4 animate-in zoom-in">
              ...
              <p className="text-center text-[9px] text-foreground/40 font-bold uppercase tracking-widest">
                WASM encoding in progress. Do not close the studio.
              </p>
            </div>
          )}
        </div>
                ...
                {wmType === "text" ? (
                  <div className="space-y-4 animate-in slide-in-from-top-2">
                    ...
                    <Input
                      ...
                      className="h-14 bg-secondary/50 border-border rounded-2xl text-sm font-bold"
                    />
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-secondary border border-border flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <Palette className="w-4 h-4 text-primary" />
                          <span className="text-[9px] font-black text-foreground/40 uppercase">
                            Color
                          </span>
                        </div>
                        <div
                  ...
                  <div className="space-y-6 animate-in slide-in-from-top-2">
                    ...
                    <div
                      ...
                      className="h-24 rounded-2xl border-2 border-dashed border-border hover:border-primary/40 flex flex-col items-center justify-center bg-secondary/30 cursor-pointer overflow-hidden transition-all group/logo"
                    >
                      {wmLogo ? (
                        <img
                          src={wmLogo}
                          alt="Logo"
                          className="max-h-16 w-auto object-contain p-2"
                        />
                      ) : (
                        <span className="text-[9px] font-black uppercase text-foreground/30 group-hover/logo:text-primary transition-colors">
                          Import Logo PNG
                        </span>
                      )}
                      <input
                      ...
                      >
                        <div
                          className={cn(
                            ...
                            positionKey === k ? "bg-white" : "bg-foreground/10",
                          )}
                        />
                      </button>
                    ))}
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/20 flex items-start gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-foreground/40 leading-relaxed font-medium uppercase">
                        Pattern distributes at 4x frequency with 50% baseline
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  100% local production. Video frames are re-synthesized in your
                  browser memory and never touch remote servers.
                </p>
              </div>
            </div>
          </div>

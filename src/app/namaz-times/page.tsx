...
export default function NamazTimesPage() {
  ...
  const fetchTimings = useCallback(
    async (isGps = false, lat?: number, lon?: number) => {
      setIsLoading(true);
      setError(null);
      ...
      if (isGps) {
        ...
        const searchCity =
          city.trim() || localStorage.getItem(STORAGE_KEY) || "Karachi";
      ...
      try {
        ...
        if (json.code === 200) {
          setData(json.data);
          if (!isGps)
            localStorage.setItem(STORAGE_KEY, city.trim() || "Karachi");
          toast({
            title: "Matrix Synced",
            description: "Prayer timings calibrated.",
          });
        } else {
          setError(
            "Linguistic Error: Location not identified in astronomical registry.",
          );
        }
      } catch (err) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        ...
        toast({
          variant: "destructive",
          title: "Access Denied",
          description: "Location permissions required.",
        });
      },
    );
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional astronomical calculation matrix. Synchronize prayer
              timings with precision Hijri mapping and hardware-native location
              discovery.
            </p>
          </div>
            ...
            <CardContent className="pt-10 space-y-8">
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-12 bg-secondary border-border rounded-xl font-bold uppercase"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[9px] font-black text-foreground/40 uppercase tracking-[0.2em] ml-1">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button
                    ...
                    className="h-14 flex-1 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/30"
                  >
                    {isLoading ? (
                  ...
                  <Button
                    ...
                    disabled={isLoading}
                    variant="outline"
                    className="h-14 px-8 border-border bg-secondary text-foreground font-black text-xs uppercase tracking-widest rounded-2xl"
                  >
                    <Navigation2 className="w-4 h-4 mr-2 text-primary" /> My
                    Location
                  </Button>
                </div>
              </div>
              ...
              <div className="p-6 rounded-[2rem] bg-secondary/50 border border-border space-y-6">
                ...
                <div className="pt-4 border-t border-white/5 flex items-center gap-3">
                  ...
                  <p className="text-[8px] font-bold text-foreground/20 uppercase leading-relaxed">
                    Default Method: Univ. of Islamic Sciences, Karachi
                  </p>
                </div>
              </div>
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Location searches are volatile and held strictly in local
                memory. The studio does not track or store your spiritual
                observance history.
              </p>
            </div>
          </div>
                  ...
                  {nextPrayer && (
                    <div className="p-10 rounded-[3rem] bg-primary/10 border border-primary/20 flex flex-col items-center text-center gap-6 relative overflow-hidden shadow-2xl">
                      ...
                      <div className="space-y-2 relative z-10">
                        <p className="text-[11px] font-black text-primary uppercase tracking-[0.6em]">
                          Next Observed Matrix
                        </p>
                        <h2 className="text-5xl sm:text-7xl font-headline font-black text-foreground uppercase tracking-tight">
                          {nextPrayer.name}
                        </h2>
                            ...
                            <div className="flex items-center gap-4">
                              <div
                                className={cn(
                                  ...
                                  isNext
                                    ...
                                    : "bg-background border-border text-primary/40",
                                )}
                              >
                                {name === "Sunrise" ? (
                              ...
                              <span
                                className={cn(
                                  ...
                                  isNext ? "text-white" : "text-foreground/60",
                                )}
                              >
                                {name}
                              </span>
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(
                          `Namaz Times for ${city || "Local"}: ${Object.entries(
                            data.timings,
                          )
                            .map((t) => `${t[0]}: ${t[1]}`)
                            .join(", ")}`,
                      ...
                      className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/20 hover:text-primary transition-colors"
                    >

  ...
  const handleAnalyze = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Link deconstructed successfully.",
      });
    } catch (err: any) {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional URL deconstruction studio. Unmask shorteners,
              identify redirect chains, and evaluate destination risk factors
              without exposing your hardware.
            </p>
          </div>
              ...
              <div className="pt-6 border-t border-white/5 space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    <h4 className="text-[11px] font-black uppercase text-foreground/60">
                      Anonymous Trace
                    </h4>
                    <p className="text-[10px] text-foreground/30 font-medium leading-relaxed uppercase">
                      Uplinks are followed server-side. No cookies or hardware
                      fingerprints are shared with the target.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {isLoading && (
                <div className="flex-1 flex flex-col items-center justify-center space-y-10 py-24">
                  ...
                  <p className="text-[11px] font-black uppercase text-primary tracking-[0.4em]">
                    Deconstructing Redirect Matrix...
                  </p>
                </div>
              )}
                ...
                <div className="w-full space-y-10 animate-in zoom-in-95 duration-500">
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <div className="relative w-48 h-48 flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center space-y-1">
                        ...
                        <h2 className="text-5xl sm:text-7xl font-headline font-black text-foreground leading-none">
                          {results.score}
                        </h2>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="p-6 rounded-[2.5rem] bg-secondary/50 border border-border space-y-4 shadow-inner">
                      <div className="flex items-center justify-between px-1">
                        ...
                        <span className="text-[9px] font-black text-primary uppercase bg-primary/10 px-2 py-0.5 rounded leading-none">
                          {results.domain}
                        </span>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <Button
                          onClick={handleCopy}
                          className="h-12 flex-1 bg-white text-black font-black uppercase text-[9px] tracking-widest rounded-xl shadow-xl active:scale-95 transition-all"
                        >
                          {isCopied ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 px-1">
                      ...
                      <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">
                        Redirection Trace ({results.chain.length} Hops)
                      </h4>
                    </div>
                      ...
                      {results.chain.map((hop, i) => (
                        <div key={i} className="flex items-center gap-4 group">
                          ...
                          <div className="flex-1 min-w-0 p-3 rounded-xl bg-white/[0.02] border border-white/5 group-hover:border-white/10 transition-all flex items-center justify-between gap-4">
                            <span className="text-[10px] font-mono text-foreground/50 truncate uppercase">
                              {hop.url}
                            </span>
                            <Badge
                              variant="outline"
                              className="bg-background/50 text-[8px] font-mono border-white/5 text-foreground/20"
                            >
                              {hop.status}
                            </Badge>
                          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      {results.reasons.map((r, i) => (
                        <div
                          key={i}
                          className="p-4 rounded-2xl bg-secondary/50 border border-border flex items-center gap-4"
                        >
                          <div
                            className={cn(
                          ...
                          />
                          <p className="text-[11px] font-bold text-foreground/60 uppercase tracking-tighter leading-tight">
                            {r}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-amber-500/5 border border-amber-500/10 flex items-start gap-4">
                    ...
                    <p className="text-[10px] text-foreground/40 font-bold leading-relaxed uppercase">
                      Analysis based on current cryptographic signals and remote
                      registries. Security of dynamic JavaScript content beyond
                      the initial load cannot be clinically verified. Proceed
                      with caution.
                    </p>
                  </div>
                </div>

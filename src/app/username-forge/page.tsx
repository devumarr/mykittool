...
const CATEGORY_DNA: Record<
  string,
  { roots: string[]; styles: string[]; modifiers: string[] }
> = {
  gaming: {
    roots: [
      "apex",
...
export default function UsernameForgePage() {
  ...
  const [style, setStyle] = useState<"short" | "brand" | "pro" | "mixed">(
    "brand",
  );
  ...
  const [checkResults, setCheckResults] = useState<
    Record<string, Record<string, ForgeResult>>
  >({});
      ...
      platforms.map(async (p) => {
        try {
          ...
          setCheckResults((prev) => ({
            ...
            [n]: {
              ...prev[n],
              [p]: { platform: p, status: "unknown", url: "", node: "Failed" },
            },
          }));
        }
      }),
  ...
  const forgeUsernames = useCallback(
    (overrideBase?: string) => {
      ...
      const base = (overrideBase || baseWord.trim() || inventRoot())
        .toLowerCase()
        .replace(/\s+/g, "");
      ...
      while (results.size < count) {
        ...
        const needsNoDots = Array.from(selectedPlatforms).some(
          (p) => PLATFORMS.find((pl) => pl.id === p)?.noDots,
        );
      ...
      const finalBatch = Array.from(results).sort(
        (a, b) => getIdentityScore(b) - getIdentityScore(a),
      );
  ...
  const handleMoreLikeThis = (name: string) => {
    ...
    toast({
      title: "Refining",
      description: `Creating variants of "${name}".`,
    });
  };
  ...
  const stats = useMemo(() => {
    const allResults = Object.values(checkResults).flatMap((r) =>
      Object.values(r),
    );
    return {
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight leading-none">
                Username{" "}
                <span className="text-primary italic">Forge Studio</span>
              </h1>
              <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
                Professional handle generation and availability checker. Find
                the perfect name across all your social profiles in one click.
              </p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0 pb-2">
          ...
          {step > 1 && (
            <Button
              ...
              className="h-10 px-6 rounded-xl border-border bg-secondary text-[8px] font-black uppercase tracking-widest transition-all"
            >
        ...
        {[1, 2, 3].map((s) => (
          <React.Fragment key={s}>
                ...
                className={cn(
                  ...
                  step === s
                    ...
                    : step > s
                      ...
                      : "bg-secondary border-border text-foreground/20",
                )}
              >
                {step > s ? (
                  <Check className="w-6 h-6" />
                ) : (
                  <span className="text-xs font-black">0{s}</span>
                )}
              </div>
              <span
                className={cn(
            ...
            {s < 3 && (
              <div
                className={cn(
                  "flex-1 h-[1px] mb-8 transition-all duration-1000",
            ...
            <Card className="glass-card border-border shadow-2xl overflow-hidden relative">
              <CardHeader className="py-8 border-b border-border bg-secondary/30">
                <CardTitle className="text-[11px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
                  <Settings2 className="w-5 h-5 text-primary" /> Identity
                  Settings
                </CardTitle>
              </CardHeader>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          ...
                          <div
                            key={t.label}
                            className="p-5 rounded-2xl bg-secondary/30 border border-border flex items-center justify-between group hover:border-primary/20 transition-all"
                          >
                  ...
                  <div className="space-y-10">
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                          ...
                          <span className="text-[8px] font-black text-primary uppercase bg-primary/10 px-2 py-0.5 rounded">
                            Auto Mode Enabled
                          </span>
                        </div>
                        <Input
                          ...
                          placeholder="e.g. Vesper, Nova"
                          className="h-16 bg-secondary border-border rounded-2xl font-bold uppercase text-lg px-6"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <Slider
                        ...
                        max={30}
                        step={1}
                        onValueChange={(v) => setMaxLength(v[0])}
                      />
                    </div>
                    ...
                    <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-start gap-4">
                      ...
                      <p className="text-[10px] text-foreground/40 font-bold uppercase leading-relaxed">
                        If left blank, the studio will invent a unique name for
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        selectedPlatforms.has(p.id)
                          ...
                          : "bg-secondary/30 border-border text-foreground/40 hover:border-primary/20",
                      )}
                    >
                      <p.icon
                        className={cn(
                          "w-5 h-5 transition-transform group-hover/p:scale-110",
                          selectedPlatforms.has(p.id)
                            ? "text-white"
                            : "text-primary/40",
                        )}
            ...
            {!isProcessing && bestPicks.length > 0 && (
              <div className="space-y-6 animate-in slide-in-from-top-4 duration-700">
                <div className="flex items-center gap-3 px-2">
                  ...
                  <h3 className="text-xl font-headline font-black uppercase tracking-tight text-foreground/60">
                    Perfect Matches
                  </h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {bestPicks.map((name) => (
                    <Card
                      key={name}
                      className="glass-card border-emerald-500/20 bg-emerald-500/[0.03] shadow-2xl relative overflow-hidden group/best"
                    >
                      ...
                      <CardContent className="p-8 space-y-6">
                        <div className="space-y-2">
                          ...
                          <h4 className="text-2xl font-headline font-black text-foreground uppercase tracking-tight select-all truncate">
                            {name}
                          </h4>
                        </div>
                        <Button
                          onClick={() => handleCopy(name, `best-${name}`)}
                          className="w-full h-11 bg-emerald-500 hover:bg-emerald-600 text-white font-black uppercase text-[10px] rounded-xl shadow-lg"
                        >
                          {isCopied === `best-${name}` ? (
                  ...
                  <CardContent className="p-0 flex-1 flex flex-col">
                    <div className="divide-y divide-border max-h-[800px] overflow-auto custom-scrollbar">
                      {sortedNames.map((name, idx) => {
                        ...
                        const availCount = Object.values(resMap).filter(
                          (r) => r.status === "available",
                        ).length;
                        const isPerfect =
                          availCount === selectedPlatforms.size &&
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                              <div className="space-y-6 min-w-0 flex-1">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                  <h3 className="text-2xl sm:text-3xl font-headline font-black text-foreground uppercase tracking-tight truncate select-all">
                                    {name}
                                  </h3>
                                  <div className="flex gap-2">
                                    <Badge
                                        ...
                                        className={cn(
                                          ...
                                          s === "available"
                                            ...
                                            : s === "taken"
                                              ...
                                              : s === "checking"
                                                ? "bg-primary/5 text-primary/40 border-primary/10 animate-pulse"
                              ...
                              <div className="flex gap-3 shrink-0">
                                <Button
                                  onClick={() =>
                                    handleCopy(name, `nick-${idx}`)
                                  }
                                  variant="outline"
                                  className="h-12 px-6 rounded-xl border-border bg-background text-[9px] font-black uppercase"
                                >
                                  {isCopied === `nick-${idx}` ? (
                ...
                <Card className="glass-card border-border shadow-xl">
                  ...
                  <CardContent className="pt-8 space-y-8">
                    ...
                    <div className="p-8 rounded-[3rem] bg-primary/5 border border-primary/10 space-y-6">
                      <div className="flex items-center gap-4">
                        ...
                        <h4 className="text-[11px] font-black uppercase text-foreground">
                          Verified Registry Check
                        </h4>
                      </div>
                      <p className="text-[11px] text-foreground/40 font-medium leading-relaxed uppercase tracking-tight">
                        We use clinical-grade node checking to verify if a
                        handle is truly free.
                      </p>
                    </div>
                    ...
                    <Button
                      onClick={() => setStep(1)}
                      variant="outline"
                      className="w-full h-16 border-border bg-white/5 text-foreground/60 text-[9px] font-black uppercase tracking-widest rounded-2xl shadow-lg"
                    >
                      Adjust Settings
                    </Button>
                  </CardContent>

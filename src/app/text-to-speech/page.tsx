...
export default function TextToSpeechPage() {
  ...
  const speak = () => {
    ...
    utterance.onerror = (e) => {
      ...
      toast({
        ...
        description: "Hardware stream interrupted.",
      });
    };
  ...
  const applyPreset = (p: keyof typeof PRESETS) => {
    ...
    toast({
      ...
      description: `${val.label} profile active.`,
    });
  };
  ...
  const renderHighlightedText = () => {
    ...
    return (
      <>
        {text.slice(0, start)}
        <mark className="bg-primary text-primary-foreground px-0.5 rounded shadow-lg transition-all duration-75">
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Professional linguistic synthesis engine. Convert textual matrices
            into natural-sounding audio streams locally with 1:1 textual
            fidelity and real-time word tracking.
          </p>
        </div>
      ...
      {!isSupported ? (
        <Card className="glass-card border-destructive/20 bg-destructive/5 p-12 text-center flex flex-col items-center gap-6 rounded-[3rem]">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-foreground/40 font-bold uppercase max-sm">
              The SpeechSynthesis matrix is not identified in this hardware
              context. Use Chrome or Edge for 1:1 fidelity.
            </p>
          </div>
        </Card>
      ) : (
            ...
            <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[600px] bg-background/10">
              ...
              <CardHeader className="py-6 border-b border-border bg-secondary/30 flex flex-col sm:flex-row sm:items-center justify-between shrink-0 px-6 sm:px-10 gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[9px] font-mono font-bold text-foreground/20 uppercase tracking-widest">
                    {wordCount} Words
                  </span>
                </div>
              </CardHeader>
              ...
              <CardContent className="flex-1 p-0 overflow-hidden flex flex-col">
                <div className="flex-1 relative group/output">
                  ...
                  <Textarea
                    value={text}
                    onChange={(e) => {
                      setText(e.target.value.substring(0, 5000));
                      setHighlightRange(null);
                    }}
                  ...
                  <div className="px-10 py-4 bg-primary/[0.03] border-t border-white/5 animate-in slide-in-from-bottom-2">
                    <div className="flex justify-between text-[8px] font-black uppercase tracking-[0.4em] text-primary mb-2">
                      <span className="flex items-center gap-2">
                        <Activity className="w-3 h-3 animate-pulse" /> Signal
                        Active
                      </span>
                      <span>
                        {Math.round((currentIndex / totalLength) * 100)}%
                      </span>
                    </div>
                    <Progress
                      value={(currentIndex / totalLength) * 100}
                ...
                <div className="p-8 border-t border-white/5 bg-[#0a0a0c] flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0">
                  <div className="flex-1 w-full flex gap-3">
                    ...
                    {isSpeaking && (
                      <Button
                        onClick={handleStop}
                        variant="outline"
                        className="h-16 w-16 rounded-2xl border-destructive/20 bg-destructive/5 text-destructive hover:bg-destructive hover:text-white"
                      >
                  ...
                  <div className="flex gap-2">
                    <Button
                      onClick={handleCopy}
                      variant="outline"
                      className="h-12 px-6 rounded-xl border-white/5 bg-white/5 text-white/40 font-black uppercase text-[9px]"
                    >
                      {isCopied ? (
                      ...
                      <SelectContent className="glass-card max-h-[400px] w-[280px]">
                        {filteredVoices.map((v) => (
                          <SelectItem
                            key={v.name}
                            value={v.name}
                            className="text-[10px] font-black uppercase py-3 border-b border-white/5 last:border-0"
                          >
                    ...
                    {currentVoice && (
                      <div className="px-3 py-1.5 rounded-xl bg-primary/5 border border-primary/20 flex items-center gap-3 animate-in zoom-in">
                        ...
                        <span className="text-[8px] font-black text-primary uppercase tracking-widest">
                          {currentVoice.lang} Protocol Verified
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <CardHeader className="py-4 border-b border-white/5 bg-secondary/30 flex items-center justify-between shrink-0 px-6">
                ...
                {history.length > 0 && (
                  <button
                    onClick={() => {
                      setHistory([]);
                      localStorage.removeItem("mykit_tts_archive_v1");
                    }}
                    ...
                    {history.map((item) => (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-foreground/60 truncate uppercase italic">
                            "{item.text}"
                          </p>
                          <p className="text-[8px] font-black text-foreground/20 uppercase tracking-widest mt-1">

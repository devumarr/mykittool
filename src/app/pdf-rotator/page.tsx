...
export default function PdfRotatorPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile.type !== "application/pdf") {
      toast({
        ...
        description: "Only PDF documents are supported for rotation.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${numPages} pages identified for production.`,
      });
    } catch (err) {
      console.error(err);
      toast({
        variant: "destructive",
        title: "Load Error",
        description: "Failed to decode PDF matrix.",
      });
    } finally {
  ...
  const rotateAll = (direction: "cw" | "ccw") => {
    ...
    toast({
      title: "Global Sync",
      description: `All pages rotated 90° ${direction === "cw" ? "clockwise" : "counter-clockwise"}.`,
    });
  };
  ...
  const resetAll = () => {
    ...
    toast({
      title: "Studio Reset",
      description: "All manual rotations cleared.",
    });
  };
  ...
  const saveFixedPdf = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Fixed PDF master exported.",
      });
    } catch (err) {
      console.error(err);
      toast({
        ...
        description: "Internal error during re-matricing.",
      });
    } finally {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Clinical orientation management for PDF documents. Correct
              upside-down or sideways pages with pixel-perfect precision and
          ...
          {file && (
            <div className="flex gap-3">
              <Button
                variant="outline"
                onClick={handleClear}
                className="h-12 px-6 rounded-xl border-border bg-secondary text-[10px] font-black uppercase tracking-widest hover:text-destructive"
              >
                <Trash2 className="w-4 h-4 mr-2" /> Reset
              </Button>
              <Button
                onClick={saveFixedPdf}
                disabled={isProcessing}
                className="h-12 px-8 rounded-xl bg-primary text-white font-black uppercase tracking-widest text-[10px] shadow-xl shadow-primary/30"
              >
                {isProcessing ? (
                  ...
                  <div className="p-6 rounded-3xl bg-secondary border border-border space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Button
                        ...
                        className="h-12 bg-background border-border text-[9px] font-black uppercase tracking-widest rounded-xl hover:text-primary transition-all"
                      >
                  ...
                  <div className="p-6 rounded-[2rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                        Orientation logic is executed locally. Re-matricing
                        happens entirely within your browser session for 100%
                        data security.
                      </p>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/60 leading-relaxed font-medium uppercase">
                  Original document resolution and layer stack are preserved
            ...
            <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/60 leading-relaxed font-medium uppercase">
                  Thumbnail rendering supports high-DPI scaling for accurate
                  visual verification.
                </p>
              </div>
            </div>
              ...
              {!file && !isRendering ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-6 max-h-[800px] overflow-auto pr-2 custom-scrollbar p-1">
                  {pages.map((p) => (
                    <div
                      key={p.index}
                      className="flex flex-col gap-4 animate-in zoom-in duration-300"
                    >
                      <div className="relative aspect-[3/4] bg-white rounded-2xl overflow-hidden shadow-xl ring-1 ring-white/10 group/page">
                        <div
                          className="absolute inset-0 flex items-center justify-center p-4 transition-transform duration-500 ease-in-out"
                        ...
                        <div className="absolute top-3 left-3 px-2 py-0.5 rounded bg-background/60 text-white text-[8px] font-black uppercase z-20">
                          P.{p.index}
                        </div>
                        {p.rotation !== 0 && (
                        ...
                        <div className="absolute inset-0 bg-background opacity-0 group-hover/page:opacity-100 transition-all flex items-center justify-center gap-3 backdrop-blur-sm z-30">
                          <button
                            onClick={() => rotatePage(p.index, "ccw")}
                            className="w-10 h-10 rounded-full bg-white/20 border border-white/40 text-white flex items-center justify-center hover:bg-primary transition-all"
                          >
                            <RotateCcw className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => rotatePage(p.index, "cw")}
                            className="w-10 h-10 rounded-full bg-white/20 border border-white/40 text-white flex items-center justify-center hover:bg-primary transition-all"
                          >

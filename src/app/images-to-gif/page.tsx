  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (frames.length + selectedFiles.length > 30) {
      toast({
        ...
        description: "Studio supports a maximum of 30 frames per animation.",
      });
      return;
    }
    ...
    Promise.all(loaders).then((loadedFrames) => {
      ...
      toast({
        ...
        description: `Added ${loadedFrames.length} frames to the pipeline.`,
      });
    });
  ...
  const handleDownload = async () => {
    if (frames.length < 2) {
      toast({
        ...
        description: "Provide at least 2 frames for animation.",
      });
      return;
    }
    ...
    const targetWidth =
      width === "original" ? firstFrame.width : parseInt(width);
    const targetHeight =
      width === "original"
        ? firstFrame.height
        : (firstFrame.height / firstFrame.width) * targetWidth;
    ...
    try {
        ...
        (obj: any) => {
          if (!obj.error) {
            ...
            toast({
              ...
              description: "GIF saved to local storage.",
            });
          } else {
      ...
      toast({
        ...
        description: "Internal matrix error during synthesis.",
      });
      setIsProcessing(false);
    }
  };
    ...
    <div className="container mx-auto px-4 sm:px-6 py-12 md:py-20 max-w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="min-w-0">
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional-grade sequence synthesis. Convert photo sets into
              high-fidelity animated GIFs with precision timing and automated
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[350px] max-h-[45vh] lg:max-h-none lg:min-h-[650px] bg-background/60">
            ...
            <CardContent className="flex-1 flex flex-col items-center justify-center p-6 lg:p-12 relative overflow-hidden">
              {!frames.length && !isProcessing ? (
                ...
                <div className="relative group/master max-w-full h-full flex items-center justify-center">
                  ...
                  />
                  {isProcessing && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-40 bg-background backdrop-blur-sm rounded-xl">
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar scroll-smooth snap-x snap-mandatory pb-4">
                {frames.map((f, i) => (
                  ...
                  >
                    ...
                    />
                    <div className="absolute inset-0 bg-background/60 opacity-0 group-hover/item:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 backdrop-blur-[2px]">
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => moveFrame(i, "up")}
                          disabled={i === 0}
                          className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center text-white hover:bg-primary transition-all disabled:opacity-20"
                        >
                    ...
                    <div className="absolute bottom-3 left-3 px-2 py-0.5 rounded-md bg-background/60 text-white text-[8px] font-black uppercase">
                      F.{i + 1}
                    </div>
                  </div>
                ))}
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-foreground/30">
                    <Label className="flex items-center gap-2">
                      <Clock className="w-3.5 h-3.5 text-primary" /> Frame Delay
                    </Label>
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Our studio utilizes a main-thread encoding protocol for 100%
                  browser-side synthesis. Your photos never leave your device
                  memory, ensuring absolute data privacy.
                </p>
              </div>
            </div>
          </div>

...
export default function ExifViewerPage() {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          camMappings.forEach((m) => {
            if (tags[m.key])
              newSections[1].tags.push({
                label: m.label,
                value: String(tags[m.key].description || tags[m.key].value),
              });
          });
          ...
          if (tags["Image Width"])
            newSections[3].tags.push({
              label: "Pixel Width",
              value: `${tags["Image Width"].value} px`,
            });
          if (tags["Image Height"])
            newSections[3].tags.push({
              label: "Pixel Height",
              value: `${tags["Image Height"].value} px`,
            });
          if (tags["ColorSpace"])
            newSections[3].tags.push({
              label: "Color Space",
              value: String(tags["ColorSpace"].description),
            });
          if (tags["Orientation"])
            newSections[3].tags.push({
              label: "Display Orientation",
              value: String(tags["Orientation"].description),
            });
          if (tags["FileType"])
            newSections[3].tags.push({
              label: "Binary Format",
              value: String(tags["FileType"].value),
            });
          ...
          if (finalSections.length > 0) {
            toast({
              ...
              description: "Identity headers extracted successfully.",
            });
          } else {
            toast({
              title: "Zero Data Detected",
              description: "File contains no recognizable metadata headers.",
            });
          }
        } catch (err) {
          ...
          toast({
            ...
            description: "Failed to read binary headers from this asset.",
          });
        }
      };
  ...
  const handleCopyAll = () => {
    ...
    toast({
      ...
      description: "Full metadata matrix saved to clipboard.",
    });
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
          Deep clinical inspection of image headers. Extract GPS, camera optics,
          and hidden forensic identifiers locally and privately within your
          browser.
        </p>
      </div>
            ...
            <CardContent className="pt-10 space-y-10">
                ...
                >
                  {image ? (
                    <div className="relative w-full h-full p-4 flex items-center justify-center">
                      ...
                      />
                      <div className="absolute inset-0 bg-background opacity-0 group-hover/upload:opacity-100 transition-all flex flex-col items-center justify-center gap-4 backdrop-blur-sm">
                    ...
                    <>
                      ...
                      <p className="text-[10px] font-black uppercase text-foreground/40 tracking-widest group-hover:text-primary transition-colors text-center px-12 leading-relaxed">
                        {isProcessing
                          ? "Decoding Binary Headers..."
                          : "Drop high-res imagery or click to browse"}
                        <br />
                        <span className="text-[8px] opacity-60 uppercase font-bold">
                          (JPG, PNG, WebP)
                        </span>
                      </p>
                    </>
              ...
              {sections.length > 0 && (
                <div className="space-y-4 animate-in zoom-in duration-500">
                  ...
                  <p className="text-center text-[9px] font-black uppercase tracking-widest text-foreground/20">
                    Identified{" "}
                    {sections.reduce((acc, s) => acc + s.tags.length, 0)}{" "}
                    identifiers
                  </p>
                </div>
              )}
          ...
          <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                Extraction occurs entirely on your device via binary bitstream
                inspection. Your imagery never leaves your browser session,
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          {!hasScanned && !isProcessing ? (
            ...
            <Card className="glass-card border-border shadow-2xl h-[600px] flex flex-col items-center justify-center text-center p-12">
              ...
              <p className="mt-8 text-[11px] font-black uppercase tracking-[0.3em] text-primary">
                Scanning Bitstream Matrix...
              </p>
            </Card>
          ) : sections.length === 0 ? (
            <Card className="glass-card border-border shadow-2xl h-[600px] flex flex-col items-center justify-center text-center p-12 bg-yellow-500/5">
              ...
              <h3 className="text-xl font-headline font-black text-yellow-500 uppercase tracking-widest">
                No Metadata Identified
              </h3>
              <p className="text-sm text-foreground/40 font-medium max-w-sm mt-4 leading-relaxed uppercase tracking-tighter">
                The asset appears to be sanitized. No forensic headers (EXIF,
                IPTC, XMP) were discovered in the bitstream.
              </p>
            </Card>
          ) : (
                  ...
                  <CardContent className="p-0">
                    <div className="divide-y divide-border">
                      {section.tags.map((tag, tIdx) => (
                        <div
                          key={tIdx}
                          className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-secondary/50 transition-all"
                        >
                          <span className="text-[9px] font-black uppercase tracking-widest text-foreground/40 shrink-0">
                            {tag.label}
                          </span>
                          <div className="flex items-center gap-4 overflow-hidden">
                            ...
                            <button
                              ...
                              className="text-foreground/10 hover:text-primary transition-colors shrink-0"
                            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                  Full synchronization with EXIF 2.32, XMP, and IPTC standard
                  data blocks.
                </p>
              </div>
            </div>
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                  Binary parsing occurs in a strictly isolated browser context

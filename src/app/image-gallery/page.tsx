...
export default function ImageGalleryPage() {
    ...
    async (searchQuery: string, pageNum: number) => {
      ...
      if (searchTerm.toLowerCase() === "waifu") {
        try {
          ...
          const waifuResults: ImageResult[] = (data.files || []).map(
            (url: string, i: number) => ({
              id: `waifu-${i}-${Date.now()}`,
          ...
          toast({
            ...
            description: `Isolated ${waifuResults.length} anime identifiers.`,
          });
          setIsLoading(false);
          return;
        } catch (e) {
      ...
      const fetchers = [
        fetch(
          `https://api.openverse.org/v1/images/?q=${encodeURIComponent(searchTerm)}&page=${pageNum}&page_size=24`,
        )
          .then((r) => r.json())
          .then((data) =>
        ...
        fetch(
          `https://images-api.nasa.gov/search?q=${encodeURIComponent(searchTerm)}&media_type=image&page=${pageNum}`,
        )
          .then((r) => r.json())
          .then((data) =>
        ...
        fetch(
          `https://api.artic.edu/api/v1/artworks/search?q=${encodeURIComponent(searchTerm)}&fields=id,title,image_id,artist_display&limit=24`,
        )
          .then((r) => r.json())
          .then((data) =>
            (data.data || [])
      ...
      try {
        ...
        if (combined.length === 0) {
          toast({
            variant: "destructive",
            title: "Zero Signal",
            description: "No visual assets identified for this query.",
          });
        } else {
          toast({
            title: "Matrix Synced",
            description: `Isolated ${combined.length} global visual identifiers.`,
          });
        }
      } catch (err) {
  ...
  const handleDownload = async (asset: ImageResult) => {
    try {
      ...
      toast({
        title: "CORS Redirect",
        description: "Direct download blocked. Use save as in new tab.",
      });
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional multi-source discovery. Isolate high-fidelity assets
              from NASA, Openverse, Art Institute, and Anime nodes locally with
              1:1 hardware fidelity.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <CardContent className="pt-8 space-y-8">
              <form onSubmit={handleSearch} className="space-y-4">
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="w-full h-14 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/30 text-xs uppercase tracking-widest"
                >
                  {isLoading ? (
                ...
                <div className="p-6 rounded-[2rem] bg-secondary/50 border border-border flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[9px] text-foreground/40 font-medium leading-relaxed uppercase">
                      Discovery signals are processed locally. Your visual
                      history is never transmitted or stored.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-8 xl:col-span-9 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          {isLoading && results.length === 0 ? (
            ...
            <div className="space-y-12">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button className="h-10 px-4 rounded-xl bg-white/10 backdrop-blur-md text-white text-[9px] font-black uppercase flex-1 border border-white/10">
                          View Matrix
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="absolute top-4 left-4">
                      <Badge className="bg-background/60 backdrop-blur-md border-white/10 text-[7px] font-black uppercase tracking-widest text-white/40">
                        {img.source.split(" ")[0]}
                      </Badge>
                    </div>
                  </div>
              ...
              <div className="p-4 sm:p-8 rounded-[2rem] sm:rounded-[3rem] bg-secondary/50 border border-border flex flex-row items-center justify-center gap-2 sm:gap-6">
                <Button
                  ...
                  disabled={page === 1}
                  className="h-10 px-4 rounded-xl border-border bg-background text-[9px] font-black uppercase tracking-widest"
                >
            ...
            <Card className="glass-card border-border shadow-2xl h-[600px] flex flex-col items-center justify-center text-center p-12 border-dashed bg-background/10">
              ...
              <p className="text-sm text-foreground/20 font-medium max-w-xs mt-4 uppercase tracking-tighter">
                Enter a keyword or select a category to isolate visual
                identifiers from global registries.
              </p>
            </Card>
          )}
        </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-hidden relative bg-[#060608] flex items-center justify-center p-4 sm:p-12">
                ...
                <button
                  ...
                  className="absolute top-6 right-6 w-12 h-12 rounded-full bg-background/60 text-white flex items-center justify-center hover:bg-primary transition-all border border-white/10"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="p-8 bg-secondary/30 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-8 shrink-0">
                <div className="space-y-2 min-w-0">
                  <DialogTitle className="text-2xl sm:text-3xl font-headline font-black text-foreground uppercase tracking-tight truncate max-w-lg">
                    {selectedImage.title}
                  </DialogTitle>
                  <div className="flex flex-wrap items-center gap-4">
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">
                      {selectedImage.source} Registry
                    </p>
                    <span className="text-white/10">•</span>
                    <p className="text-[10px] font-bold text-foreground/40 uppercase truncate">
                      By: {selectedImage.author || "Registry Native"}
                    </p>
                  </div>
                </div>
                <div className="flex gap-4 w-full sm:w-auto">
                  <Button
                    ...
                    className="h-16 px-10 bg-primary text-white font-black uppercase text-xs tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all flex-1"
                  >
                  ...
                  {selectedImage.originalUrl && (
                    <Button
                      asChild
                      variant="outline"
                      className="h-16 px-8 border-white/10 bg-white/5 text-white font-black uppercase text-[10px] rounded-2xl"
                    >
                      <a

  ...
  const searchWord = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 404) {
        setError(
          "Linguistic protocol not found. The word does not exist in our primary matrix.",
        );
      } else if (!response.ok) {
        ...
        toast({
          title: "Word Decoded",
          description: `Matrix updated for "${cleanQuery}".`,
        });
      }
    } catch (err) {
      setError(
        "API connection failure. Please check your uplink and try again.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    audio
      ...
      .catch((e) =>
        toast({
          ...
          description: "Audio stream restricted.",
        }),
      );
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              English{" "}
              <span className="text-primary italic">Dictionary Studio</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional linguistic analysis and definition engine. Access
              high-fidelity meanings, phonetic matrices, and audio
              pronunciations locally with 1:1 textual fidelity.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                ...
                session. The studio does not log or transmit your search history
                to any database.
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          {!results && !isLoading && !error && (
            <Card className="glass-card border-border shadow-2xl h-[500px] flex flex-col items-center justify-center text-center p-12 border-dashed bg-background/10">
              ...
              <p className="text-sm text-foreground/20 font-medium max-w-xs mt-4 uppercase tracking-tighter">
                Enter a word in the search matrix to isolate its clinical
                linguistic data.
              </p>
            </Card>
          )}
          ...
          {error && (
            <Card className="glass-card border-border shadow-2xl h-[500px] flex flex-col items-center justify-center text-center p-12 bg-destructive/5 animate-in shake duration-500">
              ...
              <h3 className="text-xl font-headline font-black text-destructive uppercase tracking-widest">
                Protocol Failed
              </h3>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-10">
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      <h2 className="text-5xl md:text-7xl font-headline font-black text-foreground uppercase tracking-tighter leading-none">
                        {entry.word}
                      </h2>
                    ...
                    <div className="flex flex-wrap items-center gap-6">
                      {entry.phonetic && (
                        <span className="text-primary font-mono text-xl tracking-widest">
                          {entry.phonetic}
                        </span>
                      )}
                      {entry.phonetics.map(
                  ...
                  {entry.meanings.map((meaning, mIndex) => (
                    ...
                    >
                      <div className="flex items-center gap-6">
                        <span className="italic font-headline font-black text-2xl text-primary/40 uppercase tracking-tighter">
                              ...
                              <div className="space-y-4 flex-1 min-w-0">
                                ...
                                {(def.synonyms?.length > 0 ||
                                  ...
                                  <div className="flex flex-wrap gap-2 pt-2">
                                    <span className="text-[8px] font-black text-primary/40 uppercase tracking-[0.2em] mt-1 mr-2">
                                      Synonyms:
                                    </span>
                                    {[
                                      ...(def.synonyms || []),
                                      ...
                                      .map((s, si) => (
                                        <Badge
                                          ...
                                          className="cursor-pointer bg-white/5 text-foreground/40 border-white/10 hover:text-primary transition-all text-[9px] font-bold uppercase py-1 px-3"
                                        >
                                          {s}
                                        </Badge>
                                      ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Deep extraction of multiple parts of speech and usage examples
                  for peak context.
                </p>
              </div>
            </div>
            <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-start gap-5 group hover:border-primary/20 transition-all">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Direct synchronization with hardware audio buffers for
                  original pronunciation playback.
                </p>
              </div>
            </div>
          </div>

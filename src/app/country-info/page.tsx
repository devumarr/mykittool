...
export default function CountryInfoPage() {
  ...
  const performSearch = useCallback(async (val: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://restcountries.com/v3.1/name/${encodeURIComponent(val.trim())}?fields=name,flags,cca2`,
      );
      if (response.ok) {
  ...
  const selectCountry = (data: CountryData) => {
    ...
    toast({
      ...
      description: `Clinical profile for ${data.name.common} active.`,
    });
  };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        response = await fetch(
          `https://countries.dev/name/${encodeURIComponent(query.trim())}`,
        );
      }
      ...
      if (response.ok && Array.isArray(data) && data.length > 0) {
        ...
        throw new Error(
          "Target identity not identified in the global registry.",
        );
      }
    } catch (err: any) {
      setError(
        `Discovery Node Failure: ${err.message || "Location not identified."}`,
      );
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional geographic diagnostic unit. Isolate global
              identities, demographic matrices, and fiscal protocols locally and
              securely using multi-node REST synchronization.
            </p>
          </div>
                      ...
                      {suggestions.map((s) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-foreground group-hover/item:text-primary transition-colors">
                              {s.name.common}
                            </span>
                          </div>
                          <Badge className="bg-background/50 text-[8px] font-mono border-white/5">
                            {s.cca2}
                          </Badge>
                        </button>
                      ))}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-widest">
                  RESTCountries.com & Countries.dev
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
                    ...
                    {[
                      {
                        ...
                        val: country.capital?.join(", ") || "—",
                      },
                      {
                        icon: PhoneCall,
                        label: "Calling Code",
                        val: formatCallingCode(country.idd),
                      },
                      {
                        icon: Users,
                        label: "Population Density",
                        val: country.population.toLocaleString(),
                      },
                      {
                        icon: Maximize,
                        label: "Land Mass (sq km)",
                        val: country.area.toLocaleString(),
                      },
                      {
                        icon: Coins,
                      ...
                      },
                      {
                        ...
                        val: country.timezones?.join(", ") || "—",
                      },
                      {
                        icon: Shield,
                        label: "Sovereign Borders",
                        val: country.borders?.join(", ") || "Isolated",
                      },
                    ].map((item, i) => (
                      <div
                        key={i}
                  ...
                  {country.maps?.googleMaps && (
                    <div className="pt-6 border-t border-white/5 flex gap-4">
                      ...
                      >
                        ...
                        >
                          <Navigation2 className="w-5 h-5 mr-1" /> Launch Map
                          Protocol
                        </a>
                      </Button>
                    </div>

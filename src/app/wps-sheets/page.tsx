...
const TEMPLATES = {
  attendance: {
    ...
    rows: [
      ["2024-03-20", "Umar Farooq", "Present", "Lead"],
      ["2024-03-20", "Jane Smith", "Absent", "On Leave"],
    ],
  },
  inventory: {
    headers: ["Item Name", "SKU", "Stock", "Price", "Value"],
    rows: [
      ["Monitor 4K", "HW-101", "10", "45000", "=C2*D2"],
      ["Keyboard", "HW-102", "50", "1200", "=C3*D3"],
    ],
  },
  salary: {
    headers: ["Employee", "Basic", "Bonus", "Tax", "Net Payable"],
    rows: [
      ["Umar", "150000", "10000", "5000", "155000"],
      ["Ali", "80000", "5000", "2000", "83000"],
    ],
  },
  expense: {
    headers: ["Date", "Category", "Description", "Amount"],
    rows: [
      ["2024-03-01", "Server", "Hosting V7", "1500"],
      ["2024-03-05", "Caffeine", "Studio Coffee", "450"],
    ],
  },
  udhar: {
    headers: ["Customer", "Amount (Rs)", "Date", "Status"],
    rows: [
      ["Hammad Ali", "1500", "2024-03-15", "Unpaid"],
      ["Usman Tech", "8500", "2024-03-18", "Partial"],
    ],
  },
  student: {
    headers: ["Name", "Math", "Science", "English", "Total"],
    rows: [
      ["Zain", "88", "92", "85", "=SUM(B2:D2)"],
      ["Sara", "95", "88", "90", "=SUM(B3:D3)"],
    ],
  },
};
...
export default function WpsSheetsAdvancedPage() {
  ...
  const addCol = () => {
    pushHistory();
    setHeaders((prev) => [...prev, String.fromCharCode(65 + prev.length)]);
  ...
  const loadTemplate = (id: keyof typeof TEMPLATES) => {
    ...
    toast({
      ...
      description: `${id.toUpperCase()} protocol loaded.`,
    });
  };
  ...
  const handleClear = () => {
    ...
    setHistory([]);
    toast({ title: "Studio Reset", description: "All Preview cleared." });
  };
      ...
      try {
        ...
        if (json.length > 0) {
          ...
          toast({
            ...
            description: "Document matrix synchronized.",
          });
        }
      } catch (err) {
        toast({
          variant: "destructive",
          title: "Import Failed",
          description: "Malformed file structure.",
        });
      } finally {
    ...
    reader.onload = (ev) => {
      try {
        ...
        toast({
          variant: "destructive",
          title: "Read Error",
          description: "Failed to parse document matrix.",
        });
      } finally {
        setIsProcessing(false);
      }
  ...
  const executeClean = (data: string[][]) => {
    ...
    let workingRows = data.filter((row) =>
    ...
    let processed = workingRows.map((row) => {
      return row.map((cell) => {
        ...
        if (
          val.length > 5 &&
          (val.match(/\d/g)?.length || 0) > val.length * 0.7
        ) {
    ...
    const finalHeaders = colsToKeep.map(
      (i) => headersRaw[i] || `Column_${i + 1}`,
    );
  ...
  const applyCleanedData = () => {
    ...
    toast({
      ...
      description: "Cleaned data injected into workspace.",
    });
  };
  ...
  const downloadXlsx = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      headers,
      ...rows.map((r) => r.map((c) => evaluateFormula(c, rows))),
    ]);
  ...
  const handleExportCsv = () => {
    ...
    const content = [
      ...
      ...rows.map((r) =>
        r.map((c) => escape(evaluateFormula(c, rows))).join(","),
      ),
    ].join("\n");
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-12 md:py-20 max-w-full overflow-hidden">
      <div className="mb-8 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex items-center gap-3 shrink-0 pb-2">
          ...
          <Button
            ...
            className="h-10 px-4 rounded-xl border-border bg-secondary text-[8px] font-black uppercase tracking-widest"
          >
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-2 px-1">
          {Object.keys(TEMPLATES).map((t) => (
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-secondary/50 border border-border text-[9px] font-black uppercase tracking-widest hover:border-primary/40 hover:text-primary transition-all whitespace-nowrap"
            >
              {t} Matrix
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 gap-6">
        <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[650px]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex-1 max-w-md group/title">
                <Input
                  ...
                  className="h-10 bg-transparent border-none text-xl font-headline font-black uppercase tracking-tight focus-visible:ring-0 p-0"
                />
              ...
              <div className="flex items-center gap-3">
                <div className="relative group/search">
                  ...
                  <Input
                    ...
                    className="h-10 pl-9 w-40 sm:w-64 bg-background/50 border-border rounded-xl text-xs font-bold"
                  />
                </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex bg-background border border-border p-1 rounded-xl">
                <Button
                  ...
                  size="sm"
                  onClick={addRow}
                  className="h-8 px-3 text-[8px] font-black uppercase"
                >
              ...
              <Button
                ...
                className="h-9 px-4 rounded-xl border-border bg-background text-[9px] font-black uppercase"
              >
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="outline"
                  className="h-9 px-4 border-border bg-background text-foreground font-black text-[9px] uppercase tracking-widest rounded-xl"
                >
                  CSV
                </Button>
                <Button
                  onClick={downloadXlsx}
                  className="h-9 px-6 bg-primary text-white font-black text-[9px] uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20"
                >
                  Download XLSX
                </Button>
              </div>
          ...
          <CardContent className="p-0 flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-auto custom-scrollbar bg-background/5 dark:bg-background relative">
              <table className="w-full border-collapse text-left min-w-full table-fixed">
                <thead className="sticky top-0 z-30 shadow-sm">
                  <tr className="bg-secondary/90 backdrop-blur-xl border-b border-border">
                    <th className="w-12 p-2 text-center border-r border-border bg-secondary font-black text-[8px] text-foreground/20">
                      #
                    </th>
                    {headers.map((h, i) => (
                      <th
                        key={i}
                        className="p-0 border-r border-border group/th min-w-[120px]"
                      >
                ...
                <tbody>
                  {rows.map((row, rIdx) => (
                    <tr
                      key={rIdx}
                      className="border-b border-border hover:bg-primary/[0.02] transition-colors group/tr h-12"
                    >
                      ...
                      {row.map((cell, cIdx) => {
                        ...
                        return (
                          <td
                            key={cIdx}
                            className={cn(
                              "p-0 border-r border-border h-full relative",
                  ...
                  {showTotals && (
                    <tr className="bg-primary/[0.03] font-black border-t-2 border-primary/20 sticky bottom-0 z-20 backdrop-blur-md">
                      <td className="w-12 p-4 text-center border-r border-border text-[8px] uppercase tracking-widest text-primary">
                        SUM
                      </td>
                      {columnTotals.map((tot, i) => (
                        <td
                          key={i}
                          className="px-4 py-3 border-r border-border text-xs font-headline font-black text-primary"
                        >
          ...
          <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:border-primary/20 transition-all">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                100% local synthesis. Spreadsheet payloads are held strictly in
                browser memory and never transmitted.
              </p>
            </div>
          </div>
          <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:border-primary/20 transition-all">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Supports standard arithmetic and range operators including
                =SUM(A1:A5) for clinical data modeling.
              </p>
            </div>
          </div>
          <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:border-primary/20 transition-all">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Advanced Clean & Arrange protocol for re-structuring messy
                CSV/Excel datasets automatically.
              </p>
            </div>
          </div>
        </div>
      ...
      {showCleaningLab && (
        <div className="fixed inset-0 z-[200] bg-background/95 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-500 overflow-hidden">
          <div className="w-full max-w-7xl h-full max-h-[90vh] bg-card rounded-[3rem] border border-white/10 shadow-2xl flex flex-col relative">
            <CardHeader className="py-8 px-10 border-b border-white/5 bg-secondary/30 flex flex-row items-center justify-between shrink-0">
              ...
              <button
                onClick={() => setShowCleaningLab(false)}
              ...
              <aside className="w-full lg:w-80 border-r border-white/5 p-8 space-y-10 bg-secondary/10 shrink-0">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 rounded-2xl bg-background border border-white/5 group hover:border-primary/20 transition-all">
                ...
                <div className="space-y-4 pt-10 border-t border-white/5">
                  <Button
                    ...
                    className="w-full h-16 bg-primary text-white font-black rounded-2xl uppercase tracking-widest text-[10px] shadow-xl shadow-primary/30"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      ...
                      className="h-10 text-[8px] font-black uppercase border-white/10 bg-white/5"
                    >
                      XLSX
                    </Button>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const escape = (v: string) =>
                          `"${(v || "").replace(/"/g, '""')}"`;
                        const content = [
                          cleanedResult.headers.map(escape).join(","),
                          ...cleanedResult.rows.map((r) =>
                            r.map(escape).join(","),
                          ),
                        ].join("\n");
                  ...
                  <div className="flex flex-col h-full space-y-4">
                    <div className="flex items-center justify-between px-2">
                      <h4 className="text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-2">
                        <History className="w-3.5 h-3.5" /> Source Stream
                        (Dirty)
                      </h4>
                    ...
                    <div className="flex-1 rounded-[2.5rem] bg-background border border-white/5 overflow-auto custom-scrollbar p-6">
                          ...
                          {rawUploadData.slice(0, 20).map((row, i) => (
                            <tr key={i} className="border-b border-white/5 h-8">
                              {row.slice(0, 4).map((c, j) => (
                                <td
                                  key={j}
                                  className="text-[9px] font-mono text-white/40 px-2 truncate max-w-[100px]"
                                >
                                  {c}
                                </td>
                              ))}
                  ...
                  <div className="flex flex-col h-full space-y-4">
                    <div className="flex items-center justify-between px-2">
                      <h4 className="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2">
                        <ShieldCheck className="w-3.5 h-3.5" /> Optimized Matrix
                        (Clean)
                      </h4>
                      <span className="text-[8px] font-mono text-green-500">
                        {cleanedResult?.rows.length || 0} Rows Sanitized
                      </span>
                    </div>
                    <div className="flex-1 rounded-[2.5rem] bg-background border border-primary/20 overflow-auto custom-scrollbar p-6 shadow-2xl">
                      <table className="w-full border-collapse">
                        <thead className="border-b border-white/10">
                          <tr>
                            {cleanedResult?.headers.slice(0, 4).map((h, i) => (
                              <th
                                key={i}
                                className="text-[9px] font-black text-primary uppercase tracking-tighter p-2"
                              >
                                {h}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {cleanedResult?.rows.slice(0, 20).map((row, i) => (
                            <tr
                              key={i}
                              className="border-b border-white/5 h-10 hover:bg-primary/5 transition-colors"
                            >
                              {row.slice(0, 4).map((c, j) => (
                                <td
                                  key={j}
                                  className="text-[10px] font-medium text-white/80 px-2 truncate max-w-[120px]"
                                >
                                  {c}
                                </td>
                              ))}

...
export default function FILEHOSTPage() {
  ...
  const processedHistory = useMemo(() => {
    return history
      .filter((item) => {
        ...
        const matchesFilter =
          filterType === "all" ||
          item.data.mime.toLowerCase().includes(filterType);
    ...
    if (selectedFile) {
      if (selectedFile.size > 100 * 1024 * 1024) {
        toast({
          variant: "destructive",
          title: "Heavy Payload",
          description: "Max limit is 100MB for stability.",
        });
        return;
      }
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        toast({
          ...
          description: "File successfully hosted on Telegram.",
        });
        setFile(null);
        setImage(null);
      } else {
        throw new Error(
          response.message || "Telegram node restricted or rejected payload.",
        );
      }
    } catch (err: any) {
  ...
  const handleGenerateLink = async (item: HistoryItem) => {
    ...
    try {
      const response = await getDownloadProtocol(
        item.data.fileId!,
        activeNode?.token,
      );
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex items-center gap-3 shrink-0 pb-2">
          ...
          >
            ...
            {activeNode
              ? activeNode.username
                ? `@${activeNode.username.toUpperCase()}`
                : "HOST ACTIVE"
              : "HOST"}
          </Button>
        </div>
        ...
        <Card className="glass-card border-border shadow-2xl p-12 sm:p-24 text-center flex flex-col items-center gap-8 relative overflow-hidden bg-background/10 rounded-[2.5rem]">
          ...
          <div className="space-y-4 relative z-10">
            ...
            <p className="text-[10px] sm:text-xs text-foreground/30 font-black uppercase tracking-[0.4em] leading-relaxed max-w-md mx-auto">
              To maintain protocol integrity and ensure high-bandwidth uplinks,
              you must be logged into the professional studio.
            </p>
          </div>
                ...
                <div className="space-y-6">
                  ...
                  {(file || result) && (
                    <button
                      ...
                      className="w-full text-[9px] font-black uppercase text-foreground/20 hover:text-primary transition-all"
                    >
                      Clear Workspace
                    </button>
                  )}
              ...
              <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                    History is synchronized across all your devices via
                    Firestore. Your hosted metadata is always accessible through
                    your profile.
                  </p>
                </div>
              </div>
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              <p className="text-[8px] font-bold text-primary uppercase tracking-widest">
                                {formatSize(item.data.size)} • Telegram
                              </p>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="space-y-6 pt-6">
                            ...
                            <div className="flex gap-2 justify-center">
                              ...
                              {(generatedUrls[item.id] ||
                                ...
                                <Button
                                  asChild
                                  className="h-10 px-8 bg-white text-black text-[9px] font-black uppercase rounded-xl shadow-xl"
                                >
                                  <a
                                    href={
                                      generatedUrls[item.id] ||
        ...
        <AlertDialogContent className="glass-card border-white/10 rounded-[2.5rem] p-8 max-w-sm">
          <AlertDialogHeader className="space-y-4">
            ...
            <AlertDialogDescription className="text-[11px] font-medium text-foreground/40 uppercase tracking-widest leading-relaxed text-center">
              Are you sure you want to disconnect your private host node? This
              action is specific to your current identity session.
            </AlertDialogDescription>
          </AlertDialogHeader>

...
export default function HtmlSiteRescuePage() {
  ...
  const ActiveFile = useMemo(
    () => assets.find((a) => a.id === activeFileId),
    [assets, activeFileId],
  );
  ...
  const scanHtml = useCallback((html: string, currentAssets: LocalFile[]) => {
    ...
    const findInAssets = (path: string) => {
      ...
      return currentAssets.find(
        (a) => a.path === cleanPath || a.name === cleanPath,
      );
    };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of uploaded) {
      const isText =
        ...
        f.name.match(/\.(css|js|mjs|json|html|htm|txt|md|webmanifest|xml)$/i);
    ...
    setAssets((prev) => {
      ...
      if (!indexHtmlId) {
        const foundIndex = combined.find(
          (a) => a.name.toLowerCase() === "index.html",
        );
    ...
    toast({
      title: "Assets Buffered",
      description: `${newAssets.length} nodes injected.`,
    });
  ...
  const addNewFile = (type: "html" | "css" | "js") => {
    ...
    toast({
      title: "Node Created",
      description: `Added ${name} to project matrix.`,
    });
  };
  ...
  const executeGlobalReplace = () => {
    ...
    toast({
      ...
      description: `Updated ${count} files.`,
    });
  };
  ...
  const undoActiveFile = () => {
    ...
    const newRedoStack = [currentContent, ...(file.redoStack || [])].slice(
      0,
      20,
    );
  ...
  const generatePreview = () => {
    ...
    if (!index || !index.content) {
      toast({
        variant: "destructive",
        title: "Entry Point Missing",
        description: "index.html not identified.",
      });
      return;
    }
    ...
    assets.forEach((asset) => {
      ...
      const regex = new RegExp(
        `(src|href)=["'](.*${escapedPath}|${asset.name})["']`,
        "g",
      );
  ...
  const handleEditorKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Tab") {
      ...
      if (ActiveFile) {
        setAssets((prev) =>
          prev.map((a) =>
            a.id === ActiveFile.id ? { ...a, content: next, isDirty: true } : a,
          ),
        );
      }
      setTimeout(() => {
        if (editorTextareaRef.current) {
          editorTextareaRef.current.selectionStart =
            editorTextareaRef.current.selectionEnd = start + 2;
        }
      }, 0);
    }
    if ((e.ctrlKey || e.metaKey) && e.key === "s") {
      ...
      if (ActiveFile?.content !== undefined)
        saveFileContent(ActiveFile.id, ActiveFile.content);
    }
  ...
  return (
    <div
      className={cn(
        "min-h-screen bg-[#0a0a0c] flex flex-col",
        step === 2 && mode === "update" && "fixed inset-0 z-[100]",
      )}
    >
      {/* HEADER / TOOLBAR */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-widest">
                {mode === "update" ? "Advanced IDE" : "Simple Mode"}
              </p>
            </div>
          </div>
            ...
            <>
              <div className="flex p-1 bg-white/5 rounded-xl border border-white/5">
                <button
                  ...
                  className={cn(
                    "px-4 py-1.5 rounded-lg text-[8px] font-black uppercase transition-all",
                    mode === "simple"
        ...
        <div className="flex items-center gap-3">
          {step === 2 && (
            <>
              ...
              <Button
                ...
                variant="outline"
                className="h-9 px-4 rounded-xl border-white/10 bg-white/5 text-[9px] font-black uppercase hover:text-primary"
              >
                <Eye className="w-3.5 h-3.5 mr-2" /> Preview
              </Button>
              <Button
                onClick={() => setStep(3)}
                className="h-9 px-6 rounded-xl bg-primary text-white font-black text-[9px] uppercase shadow-lg shadow-primary/20"
              >
          ...
          {step === 3 && (
            <Button
              variant="outline"
              onClick={clearStudio}
              className="h-9 px-4 rounded-xl border-white/5 bg-secondary text-[8px] font-black uppercase hover:text-destructive"
            >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {step === 1 && (
          <div className="container mx-auto px-6 py-20 max-w-4xl animate-reveal">
            <Card className="glass-card border-white/5 shadow-2xl p-12 text-center flex flex-col items-center gap-10 relative overflow-hidden bg-background/10 rounded-[3rem]">
              ...
              <div className="space-y-3 relative z-10">
                <h2 className="text-3xl sm:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
                  Project Intake
                </h2>
              ...
              <div className="w-full max-w-lg space-y-6 relative z-10">
                <Button
                  ...
                  className="h-20 w-full bg-primary text-white font-black uppercase text-xs tracking-widest rounded-3xl shadow-xl shadow-primary/30 active:scale-95 transition-all"
                >
                ...
                <Button
                  ...
                  className="h-16 w-full border-white/10 bg-white/5 text-white/40 font-black uppercase text-[10px] tracking-widest rounded-2xl"
                >
                ...
                <Card className="glass-card border-border shadow-2xl">
                  <CardHeader className="py-6 border-b border-border bg-secondary/30 flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="h-9 rounded-xl border-dashed border-primary/20 bg-primary/5 text-primary text-[8px] font-black uppercase"
                    >
                      <Plus className="w-3.5 h-3.5 mr-1.5" /> Inject Assets
                    </Button>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className="divide-y divide-white/5 max-h-[500px] overflow-auto custom-scrollbar bg-background/10">
                      {references.length === 0 ? (
                        <div className="p-20 text-center opacity-10 uppercase tracking-widest text-[10px] font-black">
                          ...
                          >
                            <div className="flex items-center gap-6 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-[8px] font-black text-foreground/20 uppercase tracking-[0.2em]">
                                  {ref.tag} Protocol • {ref.type}
                                </p>
                              </div>
                            </div>
                            <Badge
                    ...
                    <div className="p-8 border-t border-border bg-secondary/30 flex justify-between items-center">
                      <div className="space-y-1">
                        ...
                        <p className="text-lg font-headline font-black text-foreground">
                          {assets.length} Nodes Buffered
                        </p>
                      </div>
                      <Button
                        onClick={executeFixAll}
                        className="h-14 px-10 bg-primary text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl shadow-primary/30 active:scale-95"
                      >
              ...
              <div className="lg:col-span-4 space-y-6">
                <div className="p-8 rounded-[3rem] bg-primary/5 border border-primary/20 flex flex-col gap-6">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase tracking-tight">
                    The studio automatically identifies root-level (/) paths and
                    converts them to relative mappings to ensure your project
                    functions on any hosting protocol without structural
                    failure.
                  </p>
                </div>
              </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            {isSidebarOpen && (
              <div
                ...
                className="absolute inset-0 z-40 bg-background/60 backdrop-blur-sm lg:hidden animate-in fade-in"
              />
            )}
            ...
            >
              <div className="p-4 border-b border-white/5 flex items-center justify-between shrink-0 bg-background">
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="p-1.5 text-white/20 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                        <Plus className="w-3.5 h-3.5" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="glass-card border-white/10 w-40">
                      <DropdownMenuItem
                        onClick={() => addNewFile("html")}
                        className="text-[10px] font-black uppercase cursor-pointer"
                      >
                        <FileCode className="w-3.5 h-3.5 mr-2" /> New HTML
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => addNewFile("css")}
                        className="text-[10px] font-black uppercase cursor-pointer"
                      >
                        <Zap className="w-3.5 h-3.5 mr-2" /> New CSS
                      </DropdownMenuItem>
                      <DropdownMenuItem
                        onClick={() => addNewFile("js")}
                        className="text-[10px] font-black uppercase cursor-pointer"
                      >
                        <Terminal className="w-3.5 h-3.5 mr-2" /> New JS
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                  <button
                    onClick={() => setIsSidebarOpen(false)}
                    className="lg:hidden p-1.5 text-white/20 hover:text-white"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-auto custom-scrollbar p-3 space-y-1 bg-background">
                {assets.map((asset) => (
                  <div key={asset.id} className="group relative">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {asset.isDirty && (
                          <div className="absolute top-2.5 right-2 w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_5px_rgba(59,130,246,1)]" />
                        )}
                      </div>
                    </button>
                    ...
                    <div className="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button className="p-1.5 text-white/20 hover:text-white">
                            <MoreVertical className="w-3 h-3" />
                          </button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="glass-card border-white/10">
                          <DropdownMenuItem
                            onClick={() => setIndexHtmlId(asset.id)}
              ...
              <div className="p-6 bg-[#0d0d0f] border-t border-white/5 space-y-4 shrink-0">
                <Button
                  onClick={() => setShowGlobalReplace(true)}
                  variant="outline"
                  className="w-full h-10 border-white/5 bg-white/5 text-[8px] font-black uppercase tracking-widest hover:text-primary"
                >
                ...
                <button
                  ...
                  className="w-full text-[8px] font-black uppercase text-foreground/20 hover:text-primary disabled:opacity-0 transition-all"
                >
                  Restore Matrix
                </button>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        activeFileId === id
                          ? "bg-white/5 border-t-2 border-t-primary"
                    ...
                    >
                      <FileCode
                        className={cn(
                          "w-3.5 h-3.5",
                          activeFileId === id ? "text-primary" : "opacity-30",
                        )}
                      />
              ...
              <div className="flex-1 flex overflow-hidden relative">
                {ActiveFile ? (
                  <div className="flex-1 flex overflow-hidden relative">
                    ...
                    <div className="flex-1 relative bg-transparent">
                      ...
                      <div className="absolute top-4 right-8 z-20 flex items-center gap-3">
                        {/* Internal Search Bar */}
                        {showSearch && (
                          <div className="bg-background/90 backdrop-blur-xl border border-white/10 rounded-xl px-4 py-1.5 flex items-center gap-3 shadow-2xl animate-in slide-in-from-top-4">
                            ...
                            <button
                              ...
                              className="p-1.5 text-white/20 hover:text-white ml-1"
                            >
                              <X className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        ...
                        <div className="flex bg-background/80 backdrop-blur-md p-1.5 rounded-xl border border-white/10 shadow-2xl">
                          <button
                            onClick={undoActiveFile}
                            disabled={!ActiveFile?.undoStack?.length}
                            className="p-1.5 text-white/20 hover:text-white disabled:opacity-5 transition-all"
                          >
                            <Undo2 className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={redoActiveFile}
                            disabled={!ActiveFile?.redoStack?.length}
                            className="p-1.5 text-white/20 hover:text-white disabled:opacity-5 transition-all"
                          >
                          ...
                          <span className="w-10 text-center text-[9px] font-black text-white/30">
                            {editorFontSize}px
                          </span>
                          <button
                            onClick={() =>
                        ...
                        <Button
                          ...
                          className="h-10 px-5 bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[9px] uppercase tracking-widest rounded-xl shadow-xl shadow-emerald-500/10"
                        >
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center gap-8 opacity-10">
                    ...
                    <p className="text-xl font-headline font-black uppercase tracking-[0.5em]">
                      Select Node to Initialize
                    </p>
                    {!isSidebarOpen && (
                      <Button
                        onClick={() => setIsSidebarOpen(true)}
                        variant="outline"
                        className="h-11 px-8 rounded-xl bg-white/5 border-white/10 text-white font-black uppercase text-[10px] tracking-widest"
                      >
                        Open Explorer
                      </Button>
                    )}
          ...
          <div className="max-w-4xl mx-auto w-full space-y-8 animate-in zoom-in-95 duration-500 p-6 md:p-12">
            <Card className="glass-card border-emerald-500/20 bg-emerald-500/[0.02] shadow-2xl p-16 text-center flex flex-col items-center gap-10">
              ...
              <div className="w-full max-w-sm space-y-4">
                <Button
                  ...
                  className="w-full h-20 bg-primary text-white font-black text-xl uppercase tracking-widest rounded-3xl shadow-xl shadow-primary/30 active:scale-95 transition-all"
                >
                  {isProcessing ? (
                ...
                <p className="text-[9px] text-foreground/20 font-bold uppercase tracking-tighter">
                  Includes sanitized index.html and {assets.length} buffered
                  assets
                </p>
              </div>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="glass-card border-border p-8 space-y-6 group hover:border-primary/20 transition-all">
                ...
                <div className="p-4 bg-background rounded-2xl border border-white/5 font-mono text-[10px] text-foreground/50 space-y-2">
              ...
              <Card className="glass-card border-border p-8 space-y-6 group hover:border-primary/20 transition-all">
                ...
                <div className="space-y-4">
                  <p className="text-[11px] text-foreground/40 font-medium leading-relaxed uppercase tracking-tight">
                    Drop the extracted folder into the deployment dashboard. Our
                    normalized paths ensure instant visual calibration.
                  </p>
                </div>
              </Card>
            </div>
      ...
      <Dialog open={showGlobalReplace} onOpenChange={setShowGlobalReplace}>
        <DialogContent className="glass-card border-white/20 p-8 max-w-lg">
          <DialogHeader className="space-y-4">
            ...
            <DialogTitle className="text-xl font-headline font-black text-center uppercase tracking-tight">
              Global Replacement Matrix
            </DialogTitle>
            <DialogDescription className="text-center text-[10px] font-bold text-foreground/30 uppercase tracking-widest">
              A snapshot will be created before execution
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-8">
            <Button
              ...
              className="w-full h-14 bg-primary text-white font-black uppercase text-[10px] rounded-2xl shadow-xl shadow-primary/30"
            >
        ...
        <AlertDialogContent className="glass-card border-white/10 rounded-[2.5rem] p-8 max-w-sm">
          <AlertDialogHeader className="space-y-4">
            ...
            <AlertDialogDescription className="text-[11px] font-medium text-foreground/40 uppercase tracking-widest leading-relaxed text-center">
              This will definitively purge this node from the local project
              matrix. This action cannot be reversed.
            </AlertDialogDescription>
          </AlertDialogHeader>

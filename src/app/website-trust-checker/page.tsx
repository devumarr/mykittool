...
export default function WebsiteTrustCheckerPage() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Trust matrix calibrated.",
      });
    } catch (err) {
      setError(
        "Protocol Failure: One or more discovery nodes are unreachable.",
      );
  ...
  const audit = useMemo(() => {
    ...
    if (results.dnsInfo?.Status !== 0 || !results.dnsInfo?.Answer) {
      score -= 10;
      reasons.push(
        "Unresolved: No valid 'A' records identified in DNS matrix.",
      );
      flags.push("suspicious");
    }
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-black text-foreground uppercase tracking-tight">
              Website{" "}
              <span className="text-primary italic">Trust Checker PRO</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional multi-node security diagnostics. Evaluate domain
              safety, malware signatures, and network metadata locally via
              high-performance REST protocols.
            </p>
          </div>
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Identity deconstruction occurs 100% locally. Discovery nodes are
                queried via secure server actions to maintain your hardware
                privacy.
              </p>
            </div>
          </div>
            ...
            <CardHeader className="py-8 border-b border-border bg-secondary/30 flex flex-row items-center justify-between shrink-0">
              ...
              {results && (
                <div className="flex gap-2">
                  <Badge className="bg-white/5 text-white/40 border-white/10 text-[9px] font-black uppercase px-3 py-1">
                    Node Active
                  </Badge>
                </div>
              )}
              ...
              {results && audit && !isLoading && (
                <div className="w-full space-y-12 animate-in zoom-in-95 duration-500">
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <p className="text-[11px] font-black uppercase tracking-[0.6em] opacity-40">
                    ...
                    <div className="flex flex-col items-center gap-3 pt-6 border-t border-white/5">
                      {audit.reasons.map((r, i) => (
                        <div
                          key={i}
                          className="text-[10px] font-bold uppercase tracking-widest text-foreground/50 flex items-center gap-2"
                        >
                          <div
                            className={cn(
                              "w-1.5 h-1.5 rounded-full",
                              audit.color.replace("text-", "bg-"),
                            )}
                          />
                          {r}
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      ...
                      },
                      {
                        icon: MapPin,
                        label: "Hosting Node",
                        val: `${results.ipInfo?.flag?.emoji || ""} ${results.ipInfo?.country || "Global"}`,
                      },
                      {
                        icon: Lock,
                        label: "SSL Encryption",
                        val: results.isHttps ? "HTTPS SECURE" : "INSECURE HTTP",
                        color: results.isHttps
                          ? "text-green-500"
                          : "text-red-500",
                      },
                      {
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p
                            className={cn(
                              "text-[12px] font-bold truncate uppercase",

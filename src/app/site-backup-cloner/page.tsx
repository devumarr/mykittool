...
export default function SiteBackupClonerPage() {
  ...
  const [status, setStatus] = useState<
    "idle" | "scanning" | "downloading" | "complete" | "error"
  >("idle");
  ...
  const extractAssets = (html: string, baseUrl: string): AssetItem[] => {
    ...
    const normalizeUrl = (attrValue: string) => {
      try {
        ...
        const fileName =
          ...
          `asset_${Math.random().toString(36).substr(2, 5)}`;
    ...
    doc
      ...
      .forEach((el) => add(el.getAttribute("href"), "css"));
    doc
      .querySelectorAll("script[src]")
      .forEach((el) => add(el.getAttribute("src"), "js"));
    doc
      .querySelectorAll("img[src]")
      .forEach((el) => add(el.getAttribute("src"), "image"));
    ...
    doc
      ...
      .forEach((el) => add(el.getAttribute("href"), "icon"));
    doc
      .querySelectorAll("video[poster]")
      .forEach((el) => add(el.getAttribute("poster"), "image"));
    doc
      .querySelectorAll("source[src]")
      .forEach((el) => add(el.getAttribute("src"), "media"));
  ...
  const handleCancel = () => {
    ...
    toast({
      ...
      description: "Archival process terminated.",
    });
  };
  ...
  const executeClone = async () => {
    ...
    try {
      ...
      const downloadAsset = async (item: AssetItem): Promise<void> => {
        ...
        const attempt = async (retryNum: number): Promise<boolean> => {
          try {
            setAssets((prev) =>
              prev.map((a) =>
                a.id === item.id ? { ...a, status: "downloading" } : a,
              ),
            );
      ...
      for (let i = 0; i < initialAssets.length; i += concurrency) {
        ...
        setProgress(
          Math.round(((i + batch.length) / initialAssets.length) * 90),
        );
      }
      ...
      toast({
        ...
        description: "Project bundle generated.",
      });
    } catch (err: any) {
  ...
  const getIcon = (type: AssetItem["type"]) => {
    switch (type) {
      ...
      default:
        return <FileText className="w-4 h-4 text-primary/40" />;
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="min-w-0">
            <h1 className="text-3xl md:text-6xl font-headline font-black text-foreground uppercase tracking-tight leading-none overflow-wrap-anywhere">
              Site Backup{" "}
              <span className="text-primary italic">Cloner Studio</span>
            </h1>
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed overflow-wrap-anywhere">
              Professional linguistic asset archival. Isolate public components
              into a structured ZIP bundle locally in your browser.
            </p>
          </div>
            ...
            <CardContent className="pt-10 space-y-8">
              ...
              <div className="p-6 rounded-[2rem] bg-amber-500/5 border border-amber-500/10 space-y-3">
                ...
                <p className="text-[10px] text-foreground/40 font-bold uppercase leading-relaxed">
                  Browser security protocols strictly prevent the archival of
                  files restricted by remote CORS headers. Private backend logic
                  and server-side code cannot be retrieved.
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All binary synthesis and ZIP generation occur 100% locally in
                  your browser memory. Your target URLs are never stored on our
              ...
              {status !== "idle" && (
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className="text-[10px] font-black text-foreground uppercase leading-none">
                      {status.toUpperCase()}
                    </p>
                    <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-widest">
                      {progress}% Matrix Sync
                    </p>
                  </div>
                </div>
              )}
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[8px] font-black text-foreground/20 uppercase tracking-widest truncate">
                              {asset.url}
                            </p>
                          </div>
                        </div>
              ...
              <div className="p-8 border-t border-white/5 bg-[#0a0a0c] space-y-8 animate-in slide-in-from-bottom-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-5 rounded-2xl bg-secondary border border-border text-center space-y-1">
                    <p className="text-xl font-headline font-black text-foreground">
                      {stats.total}
                    </p>
                  ...
                  <div className="p-5 rounded-2xl bg-emerald-500/5 border border-emerald-500/20 text-center space-y-1">
                    <p className="text-xl font-headline font-black text-emerald-500">
                      {stats.success}
                    </p>
                  ...
                  <div className="p-5 rounded-2xl bg-red-500/5 border border-red-500/20 text-center space-y-1">
                    <p className="text-xl font-headline font-black text-red-500">
                      {stats.failed}
                    </p>
                    <p className="text-[8px] font-black text-red-600/40 uppercase tracking-widest">
                      Failed (CORS)
                    </p>
                  </div>
                  <div className="p-5 rounded-2xl bg-primary/5 border border-primary/20 text-center space-y-1">
                    ...
                    <p className="text-[8px] font-black text-primary/40 uppercase tracking-widest">
                      Total Volume
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button
                    ...
                    variant="outline"
                    className="h-16 px-10 border-white/10 bg-white/5 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl flex-1"
                  >
                  ...
                  {stats.failed > 0 && (
                    <Button
                      ...
                      variant="outline"
                      className="h-16 px-10 border-red-500/20 bg-red-500/5 text-red-500 font-black uppercase text-[10px] tracking-widest rounded-2xl flex-1"
                    >
                      {isCopied ? (

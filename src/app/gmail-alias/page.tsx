...
export default function GmailAliasGeneratorPage() {
  ...
  const toggleFavorite = (email: string) => {
    ...
    toast({
      title: favorites.includes(email)
  ...
  const validateEmail = (email: string) => {
    return /^[a-zA-Z0-9._%+-]+@(gmail\.com|googlemail\.com)$/i.test(
      email.trim(),
    );
  };
  ...
  const normalizeAlias = () => {
    ...
    toast({
      title: "Identity Normalized",
      description: "Dots and tags removed.",
    });
  };
  ...
  const generateAliases = useCallback(() => {
    ...
    if (!validateEmail(trimmed)) {
      toast({
        ...
        description: "Input must be @gmail.com or @googlemail.com",
      });
      return;
    }

    setIsProcessing(true);
    const [username, domain] = trimmed
      .replace(/\./g, "")
      .split("+")[0]
      .split("@");
    ...
    toast({
      ...
      description: `Generated ${finalResults.length} identities.`,
    });
      ...
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Advanced linguistic mailbox synthesis. Generate professional aliases
            for tracking, privacy, and automated filtering locally in your
            browser.
          </p>
        </div>
              ...
              {emailInput.includes("+") ||
                (emailInput.includes(".") && (
                  <button
                    ...
                    className="text-[8px] font-black uppercase text-primary/60 hover:text-primary transition-all"
                  >
                    Normalize
                  </button>
                ))}
                ...
                {emailInput.split("@")[0].length > 40 && (
                  <div className="flex items-center gap-2 text-amber-500 bg-amber-500/5 p-2 rounded-lg border border-amber-500/10">
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest">
                      Entropy Warning: User part exceeds standard 40-char limit
                    </span>
                  </div>
                )}
              </div>
                  ...
                  {PRESETS.map((p) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedPresets.includes(p.toLowerCase())
                          ? "bg-primary/10 text-primary border-primary/20 shadow-inner"
                          : "bg-secondary/30 border-white/5 text-foreground/30 hover:text-primary",
                      )}
                    >
                      {p}
                    </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    value={customTags}
                    onChange={(e) =>
                      setCustomTags(e.target.value.replace(/[^a-z0-9,]/gi, ""))
                    }
          ...
          <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Identity synthesis occurs 100% locally. No email strings are
                ever transmitted or logged on our infrastructure.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000">
            ...
            <div className="space-y-6 animate-in slide-in-from-top-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {favorites.map((fav, i) => (
                  <div
                    key={i}
                    className="p-5 rounded-3xl bg-primary/[0.03] border border-primary/20 flex items-center justify-between group shadow-xl"
                  >
                    <p className="text-sm font-bold text-foreground truncate uppercase select-all">
                      {fav}
                    </p>
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="p-2 text-yellow-500 hover:text-red-500 transition-all"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                ...
                {filteredResults.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {filteredResults.map((alias, i) => (
                      <div
                        key={i}
                        className="p-4 rounded-2xl bg-secondary/40 border border-border flex items-center justify-between gap-6 transition-all hover:bg-secondary/60 group"
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <button
                            onClick={() => toggleFavorite(alias)}
                            className="w-9 h-9 rounded-xl bg-background border border-border flex items-center justify-center text-foreground/10 hover:text-yellow-500 transition-all"
                          >
                            <Star
                              className={cn(
                                "w-4 h-4",
                          ...
                          <p className="text-sm font-bold text-foreground truncate select-all uppercase tracking-tight">
                            {alias}
                          </p>
                        </div>
                        <button
                          ...
                          className={cn(
                            ...
                            isCopied === `item-${i}`
                              ...
                              : "text-foreground/10 hover:text-primary",
                          )}
                        >
                          {isCopied === `item-${i}` ? (
              ...
              {results.length > 0 && (
                <div className="p-8 border-t border-white/5 bg-[#0a0a0c] flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0">
                  <Button
                    ...
                    className="h-12 flex-1 bg-white text-black font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl hover:bg-white/90"
                  >
                    {isCopied === "all" ? (
                  ...
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      onClick={() => handleExport(results, "txt")}
                      className="h-12 px-6 rounded-xl border-white/5 bg-white/5 text-[9px] font-black uppercase tracking-widest text-white/40 hover:text-primary"
                    >
              ...
              {history.length > 0 && (
                <div className="flex gap-3">
                  <button
                    onClick={() =>
                      handleExport(
                        history.map((h) => h.email),
                        "txt",
                      )
                    }
                    className="text-[9px] font-black text-primary/60 hover:text-primary uppercase transition-colors"
                  >
                    Export
                  </button>
                  <button
              ...
              {history.length === 0 ? (
                ...
                <div className="divide-y divide-white/5">
                  {history.map((item, i) => (
                    <div
                      key={item.id || i}
                      className="p-5 flex items-center justify-between group hover:bg-white/5 transition-all cursor-pointer"
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[8px] font-black text-primary uppercase bg-primary/10 px-1.5 py-0.5 rounded leading-none">
                            {item.mode}
                          </span>
                      ...
                      <button
                        ...
                        className="p-2 text-foreground/10 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
            <div className="p-8 rounded-[3rem] bg-primary/5 border border-primary/20 space-y-4">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Gmail ignores periods in your username. `u.m.a.r@gmail.com` is
                identical to `umar@gmail.com` for incoming signals. This matrix
                expands your address into thousands of unique combinations.
              </p>
            </div>
            <div className="p-8 rounded-[3rem] bg-indigo-500/5 border border-indigo-500/10 space-y-4">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Appending `+tag` after your username allows for instant
                server-side filtering. Use unique tags for each service to
                isolate data leaks and manage marketing high-volume streams.
              </p>
            </div>
          </div>
        </div>

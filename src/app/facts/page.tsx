...
export default function FactStudioPage() {
  ...
  const fetchFact = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://uselessfacts.jsph.pl/api/v2/facts/random?t=${Date.now()}`,
      );
      ...
      setError(
        "Matrix Retrieval Failure: The knowledge registry is unreachable.",
      );
  ...
  const handleCopy = () => {
    ...
    toast({
      ...
      description: "Fact saved to clipboard.",
    });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
              Professional high-fidelity knowledge synthesis. Extract unique
              randomized facts from the global registry with real-time
              calibration and local archiving.
            </p>
          </div>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center gap-6 py-24 text-center">
                  ...
                  <Button
                    onClick={() => fetchFact()}
                    variant="outline"
                    className="h-11 px-8 rounded-xl border-border"
                  >
                    Retry Protocol
                  </Button>
                </div>
                ...
                <div className="w-full space-y-12 animate-in zoom-in-95 duration-500">
                  <div className="text-center space-y-12">
                    <div className="relative p-10 sm:p-16 rounded-[3rem] bg-white/5 border border-white/5 shadow-2xl overflow-hidden group">
                      ...
                      <div className="absolute top-6 right-8 text-[10px] font-mono font-bold text-primary/20 uppercase tracking-widest">
                        Signal Verified
                      </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-4">
                      ...
                      <Button
                        onClick={handleShare}
                        variant="outline"
                        className="h-14 px-8 border-white/10 bg-white/5 text-white/40 rounded-2xl"
                      >
          ...
          <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                Knowledge discovery lookups are volatile and held strictly in
                local memory. The studio does not track or store your reading
                history.
              </p>
            </div>
          </div>
            ...
            <CardContent className="pt-8 space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Retrieved directly from the Useless Facts registry with 1:1
                    linguistic fidelity.
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-foreground/40 leading-relaxed font-medium uppercase">
                    Responsive matrix ensures high-fidelity knowledge across all
                    hardware sizes.
                  </p>
                </div>
              </div>

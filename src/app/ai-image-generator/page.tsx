...
const STYLE_PRESETS: Record<ImageStyle, { label: string; suffix: string }> = {
  ...
  realistic: {
    label: "Realistic",
    suffix:
      ", highly detailed, 8k resolution, cinematic lighting, photorealistic, professional photography, dslr",
  },
  anime: {
    label: "Anime",
    suffix:
      ", anime style, vibrant colors, clean lines, high quality digital art, studio ghibli inspired, 4k",
  },
  "3d": {
    label: "3D Render",
    suffix:
      ", octane render, unreal engine 5, 3d isometric, c4d, hyper-detailed, ray tracing, cinematic bloom",
  },
  logo: {
    label: "Logo",
    suffix:
      ", minimalist logo design, vector art, flat design, white background, high contrast, symbol, professional branding",
  },
  poster: {
    label: "Poster",
    suffix:
      ", typographic poster design, graphic design style, bold colors, artistic composition, minimal, modern",
  },
};
...
export default function AiImageGeneratorPage() {
    ...
    try {
      ...
      if (enhanced && enhanced.toLowerCase() !== prompt.toLowerCase()) {
        ...
        toast({
          ...
          description: "Linguistic depth increased via AI.",
        });
      } else {
      ...
      const needed = suffixes.filter(
        (s) => !currentLower.includes(s.split(" ")[0]),
      );
      ...
      if (needed.length > 0) {
        ...
        toast({
          ...
          description: "Applied local quality protocols.",
        });
      } else {
        toast({
          title: "Density Optimal",
          description: "Linguistic signal already high-fidelity.",
        });
      }
    } finally {
  ...
  const executeSynthesis = async (overrideSeed?: number) => {
    ...
    const currentSeed =
      overrideSeed ??
      (seed ? parseInt(seed) : Math.floor(Math.random() * 10000000));
    ...
    img.onerror = () => {
      ...
      toast({
        ...
        description: "The image node failed to respond. Please try again.",
      });
    };
        ...
        <aside className="lg:col-span-5 xl:col-span-4 space-y-8 animate-in fade-in slide-in-from-left-6 duration-700">
          <Card className="glass-card border-border shadow-2xl overflow-visible relative group">
            ...
            <CardHeader className="py-6 border-b border-border bg-secondary/30">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-4 text-foreground">
                <Settings2 className="w-5 h-5 text-primary" /> Preview
              </CardTitle>
            </CardHeader>
                ...
                <div className="flex justify-between items-center px-1">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="text-[9px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-2 hover:opacity-70 transition-all disabled:opacity-20"
                    >
                      {isEnhancing ? (
                    ...
                    <button
                      ...
                      className="text-[9px] font-black text-primary uppercase tracking-widest flex items-center gap-2 hover:opacity-70 transition-all"
                    >
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(["1:1", "16:9", "9:16", "4:5"] as AspectRatio[]).map(
                        (a) => (
                          <button
                            key={a}
                            onClick={() => setAspect(a)}
          ...
          <Card className="glass-card border-border shadow-xl flex flex-col max-h-[500px]">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar bg-background/10">
                <TabsContent
                  value="archive"
                  className="m-0 divide-y divide-white/5"
                >
                  {archive.length === 0 ? (
                    ...
                    archive.map((item) => (
                      <div
                        key={item.id}
                        className="p-4 flex items-center justify-between group hover:bg-white/5 transition-all cursor-pointer"
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            ...
                            className={cn(
                              ...
                              favorites.some((f) => f.url === item.url)
                                ? "text-yellow-500"
            ...
            <CardContent className="flex-1 p-8 sm:p-12 flex flex-col items-center justify-center relative overflow-hidden bg-checkered">
              {!resultUrl && !isProcessing ? (
                ...
                <div className="relative w-full h-full flex flex-col items-center justify-center">
                  {isProcessing && (
                    <div className="absolute inset-0 z-30 bg-background/60 backdrop-blur-md flex flex-col items-center justify-center gap-6 rounded-[2.5rem]">
                        ...
                        <div className="flex flex-col sm:flex-row justify-center gap-4 animate-in slide-in-from-bottom-4 duration-500 w-full">
                          ...
                          <div className="flex gap-2">
                            <Button
                              onClick={() => executeSynthesis()}
                              variant="outline"
                              className="h-16 px-8 border-white/10 bg-white/5 text-primary font-black uppercase text-[10px] rounded-2xl hover:bg-primary hover:text-white transition-all"
                            >
                            ...
                            <Button
                              ...
                              variant="outline"
                              className={cn(
                                "h-16 px-6 border-white/10 bg-white/5 rounded-2xl transition-all",
                                favorites.some((f) => f.url === resultUrl)
                            ...
                            >
                              <Star
                                className={cn(
                                  "w-5 h-5",
                                  favorites.some((f) => f.id === resultUrl) &&
                                    "fill-current",
                                )}
                              />
                            </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All generation happens via secure nodes. Your data is not
                  stored or shared.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Utilizing high-performance global nodes for rapid image
                  synthesis.
                </p>
              </div>
            </div>

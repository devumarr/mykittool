...
> = {
  Aesthetic: {
    openings: [
      "Quiet nights, city lights.",
      "Vibing in the shadows.",
      "Lost in ethereal stars.",
      "Whispers of a silent soul.",
      "Art in constant motion.",
      "Ethereal state of mind.",
      "Golden thoughts only.",
      "Bloom where you are planted.",
      "Minimalist at heart.",
      "Poetry in every pixel.",
    ],
    middles: [
      "Collecting moments of peace.",
      "Exploring the hidden beauty.",
      "Lost in my own frequency.",
      "Searching for silver linings.",
      "Finding art in every detail.",
    ],
    closers: [
      "Stay dreamy.",
      "Pure vibes only.",
  ...
  Ego: {
    openings: [
      ...
      "Rare breed, high frequency.",
      "Elite protocol initiated.",
      "God tier confidence.",
      "Winning by default.",
    ],
    middles: [
      "Making my own rules.",
      "Success is my only logic.",
      "Top of the food chain.",
      "Unmatched ambition.",
      "Silent moves, loud impact.",
    ],
    closers: [
      "Boss up.",
      "I am the one.",
  ...
  Angry: {
    openings: [
      "Cold as ice.",
      "Sharp mind, silent storm.",
      "Broken trust, zero noise.",
      "Expect nothing.",
      "Shadow self active.",
    ],
    middles: [
  ...
  Funny: {
    openings: [
      ...
      "Send snacks, not drama.",
      "Error 404: Bio not found.",
    ],
    middles: [
      "CEO of procrastination.",
      "I need coffee to exist.",
      "Eating my feelings.",
      "Just here for the memes.",
      "Making bad choices look good.",
    ],
    closers: [
      "Unsubscribed from reality.",
      "LOL",
      "Stay weird.",
      "Be back later.",
      "Joke is on you.",
  ...
  Simple: {
    ...
    middles: [
      ...
      "Doing my best.",
      "Happy enough.",
      "Constant state of calm.",
      "Quiet life, loud mind.",
    ],
...
export default function AdvancedBioMakerPage() {
  ...
  const generateSingleBio = useCallback(
    (targetMood: Mood, targetLayout: LayoutType): BioResult => {
      ...
      const applyFont = (t: string) =>
        chosenFont === "Normal" ? t : FONT_MAPS[chosenFont](t);
      ...
      switch (targetLayout) {
        ...
        case "B": // Layout B: 🌟 Umar | Your Go-To Consultant \n Line 2 \n Line 3 \n Closer
          lines.push(
            `${mainEmojis[0]} ${name ? applyFont(name) : "Identity"} | ${job || pick(bank.roles)}`,
          );
        ...
        case "C": // Layout C: 🎨 Artist & Creative Vibes \n Line 2 \n Line 3 \n #ArtLover
          lines.push(
            `${mainEmojis[0]} ${job || pick(bank.roles)} & ${pick(bank.roles)}`,
          );
        ...
        case "D": // Layout D: Name: Role 1 \n Role 2 \n Role 3 \n Closer
          if (name)
            lines.push(
              `${applyFont(name)}: ${mainEmojis[0]} ${job || pick(bank.roles)}`,
            );
  ...
  const remixBio = (id: string) => {
    setBios((prev) =>
      prev.map((b) => (b.id === id ? generateSingleBio(b.mood, b.layout) : b)),
    );
    toast({
      title: "Remixed",
      description: "Linguistic vectors recalculated.",
    });
  };
  ...
  const updateRaw = (id: string, text: string) => {
    setBios((prev) =>
      prev.map((b) =>
        b.id === id
          ? { ...b, raw: text, styled: text, font: "Normal" as FontStyle }
          : b,
      ),
    );
  };
  ...
  const handleCopy = (text: string, id: string) => {
    ...
    toast({
      title: "Identity Isolated",
      description: "Bio saved to clipboard.",
    });
  };

  const selectedBio = useMemo(
    () => bios.find((b) => b.id === selectedBioId) || bios[0],
    [bios, selectedBioId],
  );
            ...
            <CardContent className="pt-8 space-y-8">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        platform === p
                          ...
                          : "bg-secondary/50 border-border text-foreground/40",
                      )}
                    >
                      {p === "Instagram" ? (
              ...
              <div className="space-y-6 pt-2 border-t border-white/5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <Label className="text-[9px] font-black text-foreground/40 uppercase tracking-[0.2em] ml-1">
                      Line Density
                    </Label>
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div
                      key={opt.id}
                      className="flex items-center gap-3 p-3 rounded-xl bg-secondary/30 border border-border group hover:border-primary/20 transition-all cursor-pointer"
                      onClick={() =>
                        setOptions((prev) => ({
                          ...prev,
                          [opt.id as keyof typeof options]:
                            !prev[opt.id as keyof typeof options],
                        }))
                      }
                    >
                      <Checkbox
                ...
                <div className="space-y-2">
                  {Object.entries(options).map(
                    ([k, active]) =>
                      active &&
                      ["name", "profession", "hobby", "location"].includes(
                        k,
                      ) && (
                        <Input
                          key={k}
              ...
              <div className="flex flex-col gap-3 pt-6">
                ...
                <Button
                  ...
                  onClick={() => {
                    setMood(
                      Object.keys(PHRASE_BANK)[
                        Math.floor(Math.random() * 7)
                      ] as Mood,
                    );
                    generateBatch(false);
                  }}
                  className="h-12 border-border bg-secondary text-[10px] font-black uppercase tracking-widest"
                >
                  Surprise Matrix
                </Button>
              </div>
        ...
        <div className="lg:col-span-8 xl:col-span-9 space-y-12 animate-in fade-in slide-in-from-right-8 duration-1000">
          ...
          {bios.length > 0 && selectedBio && (
            <div className="space-y-6 animate-reveal">
              ...
              <Card className="glass-card border-border shadow-2xl p-8 sm:p-12 relative overflow-hidden bg-white dark:bg-background/60">
                ...
                <div className="max-w-md mx-auto space-y-8">
                  <div className="flex items-center gap-8">
                    <div className="w-24 h-24 rounded-full bg-secondary border-4 border-white dark:border-black shadow-xl shrink-0 overflow-hidden ring-1 ring-border">
                      <img
                        src={`https://picsum.photos/seed/${inputs.name}/200/200`}
                        alt="Avatar"
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center justify-between gap-4">
                        <h4 className="text-xl font-bold text-foreground">
                          @
                          {inputs.name.toLowerCase().replace(/\s/g, "_") ||
                            "user"}
                        </h4>
                        <button className="p-2 rounded-lg bg-secondary text-foreground/40">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <Button className="h-10 rounded-xl bg-secondary text-foreground text-[10px] font-black uppercase">
                      Edit Protocol
                    </Button>
                    <Button className="h-10 rounded-xl bg-secondary text-foreground text-[10px] font-black uppercase">
                      Share identity
            ...
            <div className="space-y-12">
                      ...
                      <CardHeader className="py-4 border-b border-border bg-secondary/30 flex flex-row items-center justify-between shrink-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-black uppercase tracking-widest text-foreground/40">
                            {bio.mood} (Type {bio.layout})
                          </span>
                        </div>
                        <div
                          className={cn(
                            "text-[9px] font-mono font-bold",
                            isOver ? "text-red-500" : "text-primary/60",
                          )}
                        >
                      ...
                      <CardContent className="pt-6 flex-1 flex flex-col gap-6">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {bio.chips.map((emo, i) => (
                            <div
                              key={i}
                              className="w-8 h-8 rounded-lg bg-secondary border border-border flex items-center justify-center text-sm shadow-inner"
                            >
                              {emo}
                            </div>
                          ))}
                        ...
                        <div className="space-y-4 pt-4 border-t border-white/5">
                          <div className="flex overflow-x-auto no-scrollbar gap-1.5 pb-1">
                            {(
                              ["Normal", "Bold", "Small Caps"] as FontStyle[]
                            ).map((f) => (
                              <button
                                key={f}
                                onClick={(e) => {
                                ...
                                className={cn(
                                  ...
                                  bio.font === f
                                    ...
                                    : "bg-secondary text-foreground/40 hover:text-primary",
                                )}
                              >
                                {f === "Small Caps" ? "SC" : f}
                              </button>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            <Button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleCopy(bio.styled, bio.id);
                              }}
                              className="h-10 w-full bg-primary text-white font-black uppercase text-[9px] tracking-widest rounded-xl shadow-lg"
                            >
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <Button
                                ...
                                className="h-10 border-border bg-secondary hover:text-primary rounded-xl text-[8px] font-black uppercase"
                              >
                              ...
                              <Button
                                ...
                                className={cn(
                                  "h-10 border-border bg-secondary rounded-xl transition-all text-[8px] font-black uppercase",
                                  favorites.some((f) => f.id === bio.id) &&
              ...
              <div className="flex justify-center pt-8">
                <Button
                  ...
                  variant="outline"
                  className="h-16 px-12 rounded-[2rem] border-dashed border-primary/20 bg-primary/5 text-primary font-black uppercase tracking-[0.4em] text-xs hover:bg-primary/10 transition-all"
                >
            ...
            <div className="space-y-6 pt-12 border-t border-white/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {favorites.map((fav) => (
                  <Card
                    key={fav.id}
                    className="glass-card border-yellow-500/20 bg-yellow-500/[0.02] p-6 space-y-6 animate-in slide-in-from-bottom-2"
                  >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-yellow-500/10">
                      <span className="text-[8px] font-black uppercase text-yellow-600/60 tracking-widest">
                        {fav.mood} Matrix
                      </span>
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleCopy(fav.styled, fav.id)}
                          className="p-2 text-yellow-600/40 hover:text-yellow-600"
                        >

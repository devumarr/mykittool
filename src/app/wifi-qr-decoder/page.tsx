    ...
    parts.forEach((part) => {
      ...
      switch (key) {
        ...
        case "T":
          ...
          break;
        case "H":
          wifi.hidden = val.toLowerCase() === "true";
          break;
      }
    });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        if (parsed) {
          toast({
            title: "Network Decoded",
            description: "WiFi credentials isolated.",
          });
        } else {
          toast({
            title: "Matrix Decoded",
            description: "Non-WiFi QR identified.",
          });
        }
        ...
        setError(
          "Could not identify QR matrix. Ensure high contrast and clear focus.",
        );
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All decoding logic is hardware-native. Network names and
                  security keys are processed strictly in your browser session.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Utilizing the high-performance ZXing protocol for rapid and
                  accurate credential extraction from any WiFi QR.
                </p>
              </div>
            </div>
          </div>
            ...
            <CardContent className="pt-8 flex-1 flex flex-col justify-center">
              ...
              {error && (
                <div className="p-8 text-center space-y-6 animate-in shake duration-500">
                  ...
                  <p className="text-xs font-bold text-destructive uppercase tracking-tighter leading-relaxed">
                    {error}
                  </p>
                  <Button
                    variant="outline"
                    onClick={handleClear}
                    className="h-11 px-6 rounded-xl border-white/10 bg-secondary text-[9px] font-black uppercase tracking-widest"
                  >
                    Try New Asset
                  </Button>
                </div>
                ...
                <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2 group/field">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          onClick={() =>
                            handleCopy(wifiData.password || "", "Password")
                          }
                          variant="outline"
                          className={cn(
                            "h-14 w-14 rounded-2xl bg-secondary border-border transition-all",
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-4 rounded-2xl bg-secondary/50 border border-border flex flex-col justify-center gap-1">
                        ...
                        <span className="text-[10px] font-black text-foreground uppercase">
                          {wifiData.hidden ? "HIDDEN" : "BROADCAST"}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-white/5">
                    <Button
                      onClick={() =>
                        handleCopy(
                          `Network: ${wifiData.ssid}\nPassword: ${wifiData.password}\nSecurity: ${wifiData.type}`,
                          "Full Config",
                        )
                      }
                      className="w-full h-16 bg-primary hover:bg-primary/90 text-white font-black rounded-2xl flex items-center justify-center gap-4 text-xs uppercase tracking-widest shadow-xl shadow-primary/30 active:scale-95 transition-all"
                    >
                ...
                <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500">
                  <div className="p-6 rounded-[2rem] bg-yellow-500/5 border border-yellow-500/10 flex items-start gap-4">
                    ...
                    <p className="text-[10px] text-yellow-600/70 font-black uppercase leading-relaxed">
                      QR isolated, but no standard WiFi protocol detected. Raw
                      matrix content displayed below.
                    </p>
                  </div>
                  <div className="p-8 bg-secondary rounded-[2rem] border border-border shadow-inner max-h-[200px] overflow-auto custom-scrollbar">
                    <p className="text-xs font-mono font-bold text-foreground/60 break-all leading-relaxed">
                      {rawText}
                    </p>
                  </div>
                  <Button
                    onClick={() => handleCopy(rawText, "Raw Data")}
                    variant="outline"
                    className="w-full h-14 rounded-2xl border-border bg-secondary text-[10px] font-black uppercase tracking-widest"
                  >

...
export default function VocalSeparatorPage() {
  ...
  // Settings
  const [mode, setMode] = useState<"vocal-reduce" | "vocal-focus">(
    "vocal-reduce",
  );
    ...
    try {
      ...
      if (decodedBuffer.numberOfChannels < 2) {
        toast({
          ...
          title: "Mono Detected",
          description:
            "This tool requires a stereo field to perform phase-cancellation.",
        });
      } else {
        ...
        toast({
          ...
          description: `Stereo matrix decoded (${formatTime(decodedBuffer.duration)}).`,
        });
      }
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    if (gainMidRef.current && gainSideRef.current && filterNodeRef.current) {
      const ctx = getAudioContext();
      if (mode === "vocal-reduce") {
        gainMidRef.current.gain.setTargetAtTime(
          1.0 - strength * 0.95,
          ctx.currentTime,
          0.05,
        );
        ...
        if (filterNodeRef.current.type === "peaking") {
          filterNodeRef.current.gain.setTargetAtTime(
            enableFilter ? -15 * strength : 0,
            ctx.currentTime,
            0.05,
          );
        }
        ...
        gainSideRef.current.gain.setTargetAtTime(
          1.0 - strength * 0.95,
          ctx.currentTime,
          0.05,
        );
  ...
  const exportWav = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `mykittool-${mode}-${Math.round(strength * 100)}p.wav`;
      link.click();
      toast({
        title: "Master Exported",
        description: "Studio quality separation saved as WAV.",
      });
    } catch (err) {
      console.error(err);
      toast({
        variant: "destructive",
        title: "Export Failed",
        description: "Internal audio rendering failed.",
      });
    } finally {
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
          Professional-grade stereo phase-cancellation. Isolate center-panned
          audio or reduce vocals for high-quality karaoke masters locally.
        </p>
      </div>
                  ...
                  {isDecoding ? (
                    ...
                    <div className="text-center p-6 space-y-2">
                      ...
                      <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">
                        {formatTime(duration)} Stereo Matrix
                      </p>
                    </div>
                  ) : (
                    <>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="space-y-1">
                        ...
                        <p className="text-[9px] text-foreground/30 font-bold uppercase">
                          Matrix Depth Intensity
                        </p>
                      </div>
                      <span className="text-lg font-headline font-black text-primary uppercase">
                  ...
                  <div className="p-6 rounded-[2.5rem] bg-secondary border border-border flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-foreground/40 font-medium">
                        Target 300Hz–3.5kHz vocal range
                      </p>
                    </div>
                    <Switch
          ...
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative group min-h-[300px]">
            ...
            <CardContent className="pt-10 space-y-8">
              ...
              <div className="p-6 rounded-[2rem] bg-yellow-500/5 border border-yellow-500/10 flex items-start gap-5">
                ...
                <div className="space-y-2">
                  <h4 className="text-[11px] font-black text-yellow-700 uppercase tracking-widest">
                    Production Note
                  </h4>
                  <p className="text-[11px] text-foreground/50 leading-relaxed font-medium">
                    This tool works best on high-quality stereo tracks where
                    vocals are center-panned. This is a local matrix utility,
                    not an AI multi-stem split.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group transition-all hover:bg-secondary/80">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-foreground/60 leading-relaxed font-medium">
                    Sampled at {audioBuffer?.sampleRate || 44100}Hz with 16-bit
                    PCM precision.
                  </p>
                </div>
              </div>

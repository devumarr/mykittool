    ...
    const hit = [...regions]
      ...
      .find(
        (r) =>
          ...
          x <= Math.max(r.x, r.x + r.w) &&
          y >= Math.min(r.y, r.y + r.h) &&
          y <= Math.max(r.y, r.y + r.h),
      );
      ...
      prev.map((r) => {
        if (r.id === d.id) {
          if (d.type === "create") {
            ...
            return {
              ...r,
              x: d.initialX + (x - d.startX),
              y: d.initialY + (y - d.startY),
            };
          }
        }
        return r;
    ...
    try {
      if ("FaceDetector" in window) {
        ...
        if (faces.length > 0) {
          ...
          toast({
            title: "Faces Discovered",
            description: `Found ${faces.length} people.`,
          });
        } else {
        ...
        toast({
          ...
          description: "Face detection is not available on this device.",
        });
      }
    } catch (e) {
    ...
    <div className="container mx-auto px-4 sm:px-6 py-12 md:py-20 max-w-full overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-foreground/40 text-sm md:text-base font-medium mt-2 max-w-2xl leading-relaxed">
              Professional visual anonymity. Mask people and sensitive data
              locally using multiple obfuscation protocols.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[350px] max-h-[45vh] lg:max-h-none lg:min-h-[700px] bg-background/80">
              ...
              <div className="flex items-center gap-3">
                {image && (
                  <div className="flex bg-secondary/50 rounded-lg p-0.5 border border-white/5">
                    ...
                    <span className="px-2 py-1 text-[8px] font-black text-white/20 uppercase">
                      {(scale * 100).toFixed(0)}%
                    </span>
                    <button
            ...
            <CardContent className="flex-1 flex flex-col items-center justify-center p-4 lg:p-12 relative overflow-hidden">
              {!image ? (
                ...
                >
                  ...
                  <span className="text-xl font-headline font-black uppercase text-white/40 group-hover:text-white">
                    Import Photo
                  </span>
                  <input
                    type="file"
            ...
            <div className="p-8 rounded-[3rem] bg-secondary border border-white/5 flex items-start gap-6 group hover:bg-secondary/80 transition-all">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  All processing occurs 100% locally in your browser memory.
                  Your un-redacted photo never leaves your device.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary border border-white/5 flex items-start gap-6 group hover:bg-secondary/80 transition-all">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  High-fidelity visual redaction using hardware-native rendering
                  protocols.
                </p>
              </div>
            </div>
            ...
            <CardHeader className="py-6 border-b border-white/5 bg-white/2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="h-9 px-4 rounded-xl bg-primary/10 border border-primary/20 text-primary text-[9px] font-black uppercase"
                >
            ...
            <CardContent className="pt-8 space-y-10">
                ...
                <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-500">
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            activeRegion?.type === t.id
                              ? "bg-primary text-white border-primary shadow-lg"
                              : "bg-secondary/50 border-white/5 text-foreground/40 hover:text-foreground",
                          )}
                        >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() =>
                          setRegions((prev) =>
                            prev.map((r) =>
                              r.id === selectedId ? { ...r, shape: "rect" } : r,
                            ),
                          )
                        }
                        className={cn(
                          "h-14 rounded-2xl border flex items-center justify-center gap-3 transition-all",
                          activeRegion?.shape === "rect"
                            ? "bg-primary text-white border-primary shadow-lg"
                            : "bg-secondary/50 border-white/5 text-foreground/40",
                        )}
                      >
                      ...
                      <button
                        onClick={() =>
                          setRegions((prev) =>
                            prev.map((r) =>
                              r.id === selectedId ? { ...r, shape: "oval" } : r,
                            ),
                          )
                        }
                        className={cn(
                          "h-14 rounded-2xl border flex items-center justify-center gap-3 transition-all",
                          activeRegion?.shape === "oval"
                            ? "bg-primary text-white border-primary shadow-lg"
                            : "bg-secondary/50 border-white/5 text-foreground/40",
                        )}
                      >
                  ...
                  <div className="p-6 rounded-[2rem] bg-secondary border border-white/5 space-y-6">
                    <Button
                      ...
                      variant="ghost"
                      className="w-full h-11 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all"
                    >
              ...
              <div className="pt-4 border-t border-white/5">
                <Button
                  ...
                  disabled={!image}
                  className="w-full h-16 bg-primary hover:bg-primary/90 text-white font-black rounded-2xl flex items-center justify-center gap-4 text-lg shadow-xl shadow-primary/30 active:scale-95 transition-all"
                >

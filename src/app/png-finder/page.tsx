...
export default function PngFinderPage() {
  ...
  const executeSearch = async (pageNum: number = 1, append = false) => {
    ...
    try {
      ...
      if (response.success && response.results.length > 0) {
        ...
        if (!append)
          setError(response.error || "No results identified for this matrix.");
      }
    } catch (err) {
  ...
  const glyphResults = useMemo(
    () => results.filter((r) => r.isIcon),
    [results],
  );
  const colorfulResults = useMemo(
    () => results.filter((r) => !r.isIcon),
    [results],
  );
  ...
  const handleDownload = async (asset: PngResult) => {
    try {
      if (asset.isIcon) {
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            link.click();
            toast({
              title: "Icon Synthesized",
              description: "Exported as 1024px PNG.",
            });
          }
        };
        img.src = asset.url;
      ...
      toast({
        title: "CORS Redirect",
        description: "Direct download blocked. Use 'Save As' in the new tab.",
      });
    }
  };
            ...
            <CardContent className="pt-8 space-y-8 relative">
              ...
              >
                      ...
                      <div className="absolute left-0 right-0 top-full mt-2 z-50 animate-in slide-in-from-top-2 duration-300">
                        <div className="glass-card border-border shadow-2xl rounded-2xl overflow-hidden divide-y divide-white/5">
                          {suggestions.map((s, i) => (
                            ...
                            >
                              <span className="text-[10px] font-black uppercase tracking-widest text-foreground/60 group-hover/sug:text-primary">
                                {s}
                              </span>
                ...
                <Button
                  ...
                  className="w-full h-14 bg-primary text-white font-black rounded-2xl shadow-xl shadow-primary/30 text-xs uppercase tracking-widest active:scale-95 transition-all"
                >
                  {isLoading ? (
              ...
              {recentSearches.length > 0 && (
                <div className="space-y-3 pt-4 border-t border-white/5">
                  <Label className="text-[9px] font-black text-foreground/20 uppercase tracking-widest">
                    Recent Matrix HITS
                  </Label>
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Discovery signals are volatile and held strictly in local
                  memory.
                </p>
              </div>
            </div>
          ...
          {favorites.length > 0 && results.length === 0 && (
            <div className="space-y-6 animate-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-3 px-2">
                ...
                <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-foreground/40">
                  Identity Shortlist
                </h3>
              </div>
                ...
                {favorites.map((fav) => (
                  ...
                  >
                    ...
                    <img
                      src={fav.previewUrl}
                      alt={fav.title}
                      className="relative z-10 w-full h-full object-contain p-4 group-hover/card:scale-110 transition-transform duration-700"
                    />
                    <div className="absolute top-3 right-3 z-30">
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-yellow-500 text-white flex items-center justify-center shadow-lg"
                      >
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
                  <TabsList className="bg-secondary/50 p-1 rounded-2xl h-14 border border-white/5 w-full sm:w-fit">
                    <TabsTrigger
                      value="glyphs"
                      className="flex-1 sm:flex-none rounded-xl px-8 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-primary data-[state=active]:text-white transition-all"
                    >
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black uppercase text-foreground/20 tracking-widest">
                      Sort:
                    </span>
                      ...
                      {glyphResults.map((img) => (
                        ...
                        >
                          ...
                          <img
                            src={img.previewUrl}
                            alt={img.title}
                            className="relative z-10 w-full h-full object-contain p-8 group-hover/card:scale-110 transition-transform duration-700"
                          />
                          ...
                          <div className="absolute inset-0 z-20 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity p-4 flex flex-col justify-end">
                            <div className="flex items-center justify-between mb-3">
                              <p className="text-[9px] font-bold text-white uppercase truncate flex-1">
                                {img.title}
                              </p>
                              <button
                                onClick={(e) => {
                                ...
                                className="p-1.5 rounded-lg text-white/40 hover:text-yellow-500"
                              >
                                <Star
                                  className={cn(
                                    "w-3.5 h-3.5",
                            ...
                            <div className="flex gap-2">
                              ...
                              <div className="h-8 px-2 rounded-lg bg-white/10 backdrop-blur-md text-white text-[7px] font-black uppercase flex items-center justify-center flex-1 border border-white/10">
                                View Master
                              </div>
                            </div>
                          </div>
                        ...
                        >
                          ...
                          <div className="absolute inset-0 z-20 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                            <div className="flex items-center justify-between mb-4">
                              <p className="text-[10px] font-bold text-white uppercase truncate flex-1">
                                {img.title}
                              </p>
                              <button
                                onClick={(e) => {
                            ...
                            <div className="flex gap-2">
                              ...
                              <div className="h-10 px-3 rounded-xl bg-white/10 backdrop-blur-md text-white text-[8px] font-black uppercase flex items-center justify-center flex-1 border border-white/10 tracking-widest">
                                Master Preview
                              </div>
                            </div>
                          </div>
              ...
              {results.length > 0 && (
                <div className="pt-10 flex flex-col items-center gap-8">
                  ...
                  <p className="text-[10px] font-black uppercase text-foreground/20 tracking-[0.4em]">
                    Page {page} • Synchronized with global nodes
                  </p>
                </div>
              )}
            </div>
          ) : (
            <Card className="glass-card border-border shadow-2xl h-[700px] flex flex-col items-center justify-center text-center p-12 border-dashed bg-background/10">
              ...
              <div className="space-y-4 max-w-sm relative z-10">
                <h3 className="text-2xl font-headline font-black text-foreground/40 uppercase tracking-widest">
                  Awaiting Discovery Signal
                </h3>
                <p className="text-sm text-foreground/20 font-medium max-w-xs mt-4 uppercase tracking-tighter">
                  Enter a linguistic identifier to isolate transparent visual
                  identities from the world's most reliable open-source
                  registries.
                </p>
              </div>
              <div className="mt-12 flex flex-wrap justify-center gap-2 relative z-10">
                {["leaf", "logo", "ribbon", "shield", "fire", "bolt"].map(
                  (tag) => (
                    <Badge
                      key={tag}
                      onClick={() => {
          ...
          {selectedAsset && (
            <>
              <DialogHeader className="sr-only">
                ...
                <DialogDescription>
                  Full resolution visual master viewport.
                </DialogDescription>
              </DialogHeader>
              <div className="flex-1 overflow-hidden relative bg-[#060608] flex items-center justify-center p-6 sm:p-12">
                ...
                <div
                  className="relative z-10 max-w-full max-h-full flex items-center justify-center transition-all duration-500"
                  style={{ transform: `scale(${previewZoom / 100})` }}
                >
                  <img
                    src={selectedAsset.url}
                    alt=""
                    className="max-w-full max-h-[60vh] object-contain shadow-2xl drop-shadow-[0_20px_50px_rgba(0,0,0,0.8)]"
                  />
                </div>

                {/* Zoom Controls */}
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-background/60 backdrop-blur-xl px-6 h-12 rounded-full border border-white/10 z-50">
                  <button
                    onClick={() => setPreviewZoom((z) => Math.max(25, z - 25))}
                    className="p-2 text-white/40 hover:text-primary"
                  >
                    <Minimize2 className="w-4 h-4" />
                  </button>
                  <span className="text-[10px] font-black font-mono text-white/80 w-12 text-center uppercase tracking-widest">
                    {previewZoom}%
                  </span>
                  <button
                ...
                <button
                  ...
                  className="absolute top-6 right-6 w-12 h-12 rounded-full bg-background/60 text-white flex items-center justify-center hover:bg-primary transition-all border border-white/10 z-50"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-8 bg-secondary/30 border-t border-white/5 flex flex-col xl:flex-row items-center justify-between gap-12 shrink-0">
                <div className="flex flex-col sm:flex-row items-center gap-10 min-w-0 flex-1">
                  ...
                  <div className="text-center sm:text-left space-y-4 min-w-0 flex-1">
                    <div className="space-y-1">
                      <h2 className="text-2xl sm:text-4xl font-headline font-black text-foreground uppercase tracking-tight truncate max-w-xl">
                        {selectedAsset.title}
                      </h2>
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-3">
                      {[
                        {
                          label: "License",
                          val: selectedAsset.license || "Open source",
                          icon: ShieldCheck,
                        },
                        {
                          label: "Geometry",
                          val: selectedAsset.width
                        ...
                        },
                        {
                          label: "Protocol",
                          val:
                            selectedAsset.url.split(".").pop()?.toUpperCase() ||
                            "PNG",
                          icon: FileImage,
                        },
                      ].map((meta, mi) => (
                        <div
                          key={mi}
                          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 text-[8px] font-black uppercase text-foreground/60 tracking-widest"
                        >
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                  <Button
                    ...
                    className="h-16 px-10 bg-primary text-white font-black uppercase text-xs tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all flex-1 sm:flex-none"
                  >
                    <Download className="w-5 h-5 mr-3" /> Save Master PNG
                  </Button>
                  <div className="flex gap-2">
                    <Button
                      onClick={() => handleCopy(selectedAsset.url, "asset-url")}
                      variant="outline"
                      className="h-16 px-6 border-white/10 bg-white/5 text-white font-black uppercase text-[10px] rounded-2xl flex-1"
                    >
                      {isCopied === "asset-url" ? (
                    ...
                    <Button
                      ...
                      className={cn(
                        ...
                        favorites.some((f) => f.id === selectedAsset.id)

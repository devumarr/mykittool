  ...
  const startStudio = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Linguistic stream initialized.",
      });
      ...
      const updateMeter = () => {
        ...
        setAudioLevel(
          Math.min(100, Math.round((sum / bufferLength / 128) * 100)),
        );
      ...
      setError(
        "Mic Access Denied: Please enable microphone permissions in your hardware protocol.",
      );
  ...
  const startRecording = () => {
    ...
    mediaRecorder.onstop = () => {
      ...
      toast({
        ...
        description: "Vocal master archived.",
      });
    };
    ...
    <div className="container mx-auto px-4 md:px-6 py-12 md:py-20 max-w-7xl">
      <div className="mb-12 animate-reveal flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <h1 className="text-3xl md:text-6xl font-headline font-black text-foreground uppercase tracking-tight leading-none">
            Voice{" "}
            <span className="text-primary italic">Changer Studio Pro</span>
          </h1>
          <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
            Advanced hardware-native acoustic modulation. Transform your vocal
            signature using high-fidelity DSP filters, spatial feedback, and
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
              ...
              {!isActive ? (
                ...
                <div className="space-y-10 animate-in zoom-in duration-500">
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            activeEffect === eff.id
                              ...
                              : "bg-secondary/30 border-border text-foreground/40 hover:text-primary",
                          )}
                        >
                          <eff.icon
                            className={cn(
                              "w-4 h-4 transition-transform group-hover/eff:scale-110",
                              activeEffect === eff.id
                                ? "text-white"
                                : "text-primary/40",
                            )}
                  ...
                  <div className="space-y-8 pt-4 border-t border-white/5">
                    {[
                      {
                        label: "Signal Saturation (Distortion)",
                        key: "dist",
                        icon: Zap,
                        val: distortion,
                        set: setDistortion,
                      ...
                      <div key={ctrl.key} className="space-y-4">
                        <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-foreground/30">
                          <Label className="flex items-center gap-2">
                            <ctrl.icon className="w-3 h-3 text-primary" />{" "}
                            {ctrl.label}
                          </Label>
                          <span className="text-primary font-mono">
                            {typeof ctrl.val === "number"
                              ? ctrl.val.toFixed(ctrl.step ? 1 : 0)
                              : ctrl.val}
                          </span>
                        </div>
                        <Slider
                  ...
                  <Button
                    variant="outline"
                    onClick={stopStudio}
                    className="w-full h-12 border-destructive/20 text-destructive bg-destructive/5 font-black uppercase tracking-widest text-[9px] rounded-xl"
                  >
        ...
        <div className="lg:col-span-7 space-y-8 animate-in fade-in slide-in-from-right-6 duration-1000 stagger-2">
          <Card className="glass-card border-border shadow-2xl overflow-hidden relative flex flex-col min-h-[400px] bg-background">
            ...
            <CardHeader className="py-8 border-b border-border bg-secondary/30 flex flex-row items-center justify-between px-10">
              ...
              {isActive && (
                <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[9px] font-black uppercase tracking-widest px-3 py-1 animate-pulse">
                  STREAM ACTIVE
                </Badge>
              )}
            </CardHeader>
                  ...
                  <div className="flex flex-col items-center gap-12">
                    <div className="relative w-48 h-48 sm:w-64 sm:h-64 flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
                        ...
                        <h2 className="text-4xl sm:text-7xl font-headline font-black text-foreground leading-none">
                          {audioLevel}%
                        </h2>
                      </div>
                    </div>
                    ...
                    <div className="text-center space-y-4">
                      <h3 className="text-2xl font-headline font-black uppercase text-foreground tracking-tight">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        <Badge
                          variant="outline"
                          className="bg-secondary/50 border-white/10 text-[9px] font-black uppercase tracking-widest px-4"
                        >
                          {EFFECTS.find((e) => e.id === activeEffect)?.desc}
                        </Badge>
                        <button
                          onClick={() => toggleFavorite(activeEffect)}
                          className={cn(
                            "p-2 transition-all",
                            favorites.has(activeEffect)
                              ? "text-yellow-500"
                              : "text-white/10",
                          )}
                        >
                          <Star
                            className={cn(
                              "w-5 h-5",
                              favorites.has(activeEffect) && "fill-current",
                            )}
                          />
                        </button>
                      </div>
                  ...
                  <div className="p-8 rounded-[3rem] bg-secondary border border-border space-y-8 shadow-2xl relative overflow-hidden group/rec">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-[0.2em]">
                          Record post-modulation stream
                        </p>
                      </div>
                      {isRecording && (
                        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 text-red-500 border border-red-500/20 text-[8px] font-black uppercase">
                      ...
                      {recordedUrl ? (
                        <div className="flex gap-2">
                          <Button
                            asChild
                            variant="outline"
                            className="h-16 flex-1 bg-white text-black font-black uppercase text-[10px] rounded-2xl shadow-xl hover:bg-white/90"
                          >
                            <a
                              href={recordedUrl}
                              download={`vocal_mod_${activeEffect}.wav`}
                            >
                              <Download className="w-5 h-5 mr-2" /> WAV
                            </a>
                          </Button>
                          <Button
                            variant="outline"
                            ...
                            className="h-16 w-16 bg-secondary border-border rounded-2xl hover:text-destructive"
                          >
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Acoustic synthesis occurs 100% in local memory. No audio
                  waveforms or biometric voice data are ever transmitted or
                  stored on remote studio servers.
                </p>
              </div>
            </div>
            <div className="p-8 rounded-[3rem] bg-secondary/50 border border-border flex items-start gap-6 group hover:bg-secondary/80 transition-all duration-500 shadow-lg">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-foreground/40 leading-relaxed font-medium uppercase">
                  Utilizing the standard Web Audio API with bitstream buffering
                  for 1:1 acoustic fidelity across modern hardware nodes.
                </p>
              </div>
            </div>
          </div>

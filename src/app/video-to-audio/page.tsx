  ...
  const loadFFmpeg = async (force = false) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Failed to load FFmpeg. Browser-side processing requires SharedArrayBuffer support.",
      });
      return false;
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      toast({
        ...
        description: `Analyzed ${isAudioOnly ? "audio" : "video"} metadata.`,
      });
    }
  };
  ...
  const convertToAudio = async () => {
    ...
    try {
      ...
      const url = URL.createObjectURL(
        new Blob([(data as any).buffer], {
          type: `audio/${selectedFormat.ext}`,
        }),
      );
      ...
      toast({
        ...
        description: `Track successfully synthesized as ${selectedFormat.id.toUpperCase()}.`,
      });
      ...
      if (err.message?.includes("memory access out of bounds")) {
        toast({
          ...
          description:
            "This asset is too large for the current WASM sandbox. Please try a shorter duration or smaller file.",
        });
    ...
    <div className="container mx-auto px-6 py-12 md:py-20">
      <div className="mb-12 animate-reveal">
        ...
        <p className="text-foreground/40 text-sm md:text-base font-medium mt-4 max-w-2xl leading-relaxed">
          Professional-grade audio synthesis. Convert video or audio assets into
          high-fidelity masters with precision trimming and multi-format
          encoding protocols.
        </p>
      </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Scissors className="w-3 h-3 text-primary" /> Visual
                        Timeline Matrix
                      </Label>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            const val = parseMMSSToSeconds(e.target.value);
                            if (val > startTime && val <= totalDuration)
                              setEndTime(val);
                          }}
                          className="h-14 bg-secondary border-border rounded-2xl text-lg font-mono font-bold text-center"
                        />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  {selectedFormat.supportsBitrate && (
                    <div className="space-y-4 animate-in fade-in">
                      <Label className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-2">
                        <FastForward className="w-3 h-3 text-primary" /> Bitrate
                        Quality
                      </Label>
                      <RadioGroup
                        defaultValue="192k"
              ...
              <div className="relative group/output min-h-[300px] flex flex-col items-center justify-center rounded-[2.5rem] bg-secondary/30 border border-border p-10 text-center">
                ...
                {isProcessing && (
                  <div className="w-full space-y-8 animate-in fade-in duration-500">
                    ...
                    <div className="mt-4 p-4 rounded-xl bg-background/90 border border-white/5 text-left font-mono text-[9px] text-green-500/80 overflow-hidden shadow-inner">
                      {logs.map((log, i) => (
                        <div
                          key={i}
                          className="truncate whitespace-nowrap opacity-60 hover:opacity-100 transition-opacity"
                        >
                          &gt; {log}
                        </div>
                      ))}
                ...
                {audioUrl && (
                  <div className="space-y-8 w-full animate-in zoom-in duration-500">
                    ...
                    >
                      <a
                        href={audioUrl}
                        download={`${file?.name.split(".")[0] || "master"}_optimized.${selectedFormat.ext}`}
                      >
              ...
              <div className="p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10 flex items-start gap-5">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-foreground/40 leading-relaxed font-medium">
                    Conversions are strictly local. The 2GB WASM memory limit
                    applies; if the process fails with a memory error, try
                    trimming a smaller segment of the source media.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4 p-5 rounded-2xl bg-secondary border border-border group transition-all hover:bg-secondary/80">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-foreground/60 leading-relaxed font-medium">
                    Automatic re-encoding with optimized bitstream alignment.
                  </p>
                </div>
              </div>
            </CardContent>

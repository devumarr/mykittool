...
const PERSONAS: Persona[] = [
  {
    id: "helper",
    label: "Detailed Helper",
    icon: Bot,
    prompt:
      "Provide clear, accurate, and detailed answers. Stay on topic and maintain a professional tone.",
  },
  {
    id: "coder",
    ...
    icon: Code2,
    prompt:
      "Provide clean, secure, and production-ready code. Include brief technical explanations.",
  },
  {
    id: "writer",
    ...
    icon: Edit3,
    prompt:
      "Focus on engaging language, impeccable clarity, and sophisticated narrative flow.",
  },
  {
    id: "teacher",
    label: "Educator",
    icon: BookOpen,
    prompt: "Break down complex concepts into simple, understandable steps.",
  },
  {
    id: "short",
    label: "Quick Signal",
    icon: Zap,
    prompt:
      "Provide the shortest possible accurate answers. Zero fluff, maximum precision.",
  },
];
...
export default function AIChatbotPage() {
  ...
  const { data: cloudSessions } = useCollection<Session>(
    (cloudSessionsQuery as any) || null,
  );
  ...
  useEffect(() => {
    if (activeSession?.persona) {
      ...
      if (persona)
        setConfig((prev) => ({ ...prev, systemPrompt: persona.prompt }));
    }
  ...
  const handleStop = () => {
    if (abortControllerRef.current) {
      ...
      toast({
        ...
        description: "Linguistic synthesis halted.",
      });
    }
  };
  ...
  const handleSend = async (
    ...
    if (
      !targetId ||
      (activeSession && activeSession.messages.length === 0 && !retryText)
    ) {
        ...
        prev.map((s) =>
          s.id === targetId
            ? {
                ...
                lastUpdated: Date.now(),
                title:
                  s.title === "New Discussion"
                    ? textToProcess.substring(0, 30)
                    : s.title,
              }
            : s,
        ),
  ...
  const renameSession = (id: string) => {
    ...
    setSessions((prev) =>
      prev.map((s) =>
        s.id === id
          ? { ...s, title: renameValue.trim(), lastUpdated: Date.now() }
          : s,
      ),
    );
  ...
  const handleConnectCustom = () => {
    ...
    toast({
      ...
      description: "Personal node calibrated and active.",
    });
  };
  ...
  const renderContent = (content: string, role: string) => {
    ...
    try {
      ...
      return (
        <div
          className="text-sm sm:text-[15px] font-medium leading-relaxed markdown-content"
  ...
  return (
    <div className="flex h-[calc(100dvh-112px)] w-full overflow-hidden bg-background selection:bg-primary/20 relative">
      {/* SIDEBAR OVERLAY */}
      {isSidebarOpen && (
        <div
          className="absolute inset-0 z-40 bg-background backdrop-blur-sm transition-all duration-300"
      ...
      <aside
        className={cn(
          "absolute inset-y-0 left-0 flex flex-col bg-background border-r border-white/5 transition-all duration-500 z-50 overflow-hidden shadow-2xl",
          isSidebarOpen ? "translate-x-0 w-80" : "-translate-x-full w-80",
        )}
      >
        <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-background">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-xl bg-primary/10 text-primary hover:bg-primary hover:text-foreground transition-all shadow-lg"
            >
              <Plus className="w-4 h-4" />
            </button>
            <button
              onClick={() => setIsSidebarOpen(false)}
              className="p-2 text-foreground/20 hover:text-foreground"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="p-4 border-b border-white/5 bg-background/10">
                ...
                {isRenaming === s.id ? (
                  <div className="flex items-center gap-2 p-2">
                    <Input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && renameSession(s.id)
                      }
                  ...
                  >
                    <div className="relative">
                      ...
                      {s.isPinned && (
                        <Pin className="absolute -top-1 -right-1 w-2 h-2 text-primary fill-current" />
                      )}
                    </div>
                ...
                {isRenaming !== s.id && (
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all bg-secondary/80 backdrop-blur-md p-1 rounded-lg">
                    ...
                    <button
                      ...
                      className="p-1.5 text-foreground/20 hover:text-primary"
                    >
                      <Edit3 className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => setSessionToDelete(s.id)}
                      className="p-1.5 text-foreground/20 hover:text-red-500"
                    >
        ...
        <div className="p-6 bg-background/40 border-t border-white/5 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Badge
              variant="outline"
              className="bg-primary/10 text-primary border-primary/20 text-[7px] font-black px-2"
            >
              v7.3 PRO
            </Badge>
          </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {/* Top Monitor Bar */}
        <header className="h-16 border-b border-white/5 bg-background/80 backdrop-blur-xl flex items-center justify-between px-6 shrink-0 z-30">
        ...
        {/* Message Viewport */}
        <div
          className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-10 space-y-12 bg-background/20"
          ref={scrollRef}
        >
              ...
              {activeSession.messages.map((msg, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "space-y-3 min-w-0 flex-1 max-w-[88%]",
                      ...
                      <div
                        className={cn(
                          ...
                          msg.role === "assistant"
                            ...
                            : "bg-primary text-foreground rounded-tr-none shadow-primary/15",
                        )}
                      >
                        ...
                        >
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-background/60 text-foreground/40 hover:text-foreground shadow-xl"
                          >
                            {isCopied === `msg-${i}` ? (
                      ...
                      >
                        {msg.role === "assistant" ? (
                          <div className="flex flex-wrap items-center gap-4 opacity-40 hover:opacity-100 transition-opacity">
                            <button
                              onClick={() =>
                                handleCopy(msg.content, `ai-copy-${i}`)
                              }
                              className="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest hover:text-primary transition-colors"
                            >
                              {isCopied === `ai-copy-${i}` ? (
                          ...
                          <div className="flex items-center gap-4 opacity-40 hover:opacity-100 transition-opacity">
                            <span className="text-[8px] font-black uppercase text-foreground/20 tracking-widest">
                              You
                            </span>
                            <button
                              onClick={() =>
                                handleCopy(msg.content, `user-copy-${i}`)
                              }
                              className="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest hover:text-primary transition-colors"
                            >
                              {isCopied === `user-copy-${i}` ? (
                                <CheckCircle2 className="w-2.5 h-2.5 text-emerald-400" />
                              ) : (
                                <Copy className="w-2.5 h-2.5" />
                              )}
                          ...
                          i === activeSession.messages.length - 1 && (
                            <button
                              onClick={() => {
                                ...
                                handleSend(undefined, msg.content);
                              }}
                              className="text-[8px] font-black uppercase text-primary/40 hover:text-primary transition-all"
                            >
                              Retry Signal
                            </button>
                          )}
          ...
          <>
            ...
            <Card className="absolute top-4 right-6 z-50 flex max-h-[calc(100%-32px)] w-80 flex-col border-border bg-background shadow-2xl">
              <CardHeader className="py-4 border-b border-white/5 flex flex-row items-center justify-between shrink-0">
                ...
                <button
                  onClick={() => setIsConfigOpen(false)}
                  className="rounded-md p-1 text-foreground/50 hover:bg-muted hover:text-foreground"
                >
                  <X className="h-4 w-4" />
                </button>
              </CardHeader>
              <CardContent className="p-5 space-y-6 overflow-y-auto custom-scrollbar">
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(["auto", "groq", "openrouter", "custom"] as const).map(
                      (n) => (
                        <button
                          key={n}
                          ...
                          className={cn(
                            ...
                            config.node === n
                              ...
                              : "bg-secondary/50 border-black bg-muted text-foreground hover:text-foreground",
                          )}
                        >
                          {n === "auto"
                            ? "Default"
                ...
                {config.node === "custom" && (
                  <div className="space-y-4 p-4 rounded-2xl bg-primary/5 border border-primary/20 animate-in slide-in-from-top-2">
                    <div className="flex items-center justify-between">
                      ...
                      {isCustomConnected ? (
                        <Badge className="bg-emerald-500/10 text-emerald-500 text-[7px] border-emerald-500/20">
                          LINKED
                        </Badge>
                      ) : (
                        <Badge className="bg-white/5 text-foreground/40 text-[7px]">
                          STANDBY
                        </Badge>
                      )}
                    </div>
                    <div className="space-y-3">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          className="h-8 bg-background/40 border-white/5 text-[9px] font-bold"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-[8px] font-black uppercase text-foreground/20">
                        ...
                        <div className="relative">
                          <KeyRound className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-foreground/10" />
                          <Input
                            type="password"
                            ...
                            placeholder="sk-..."
                            className="h-8 pl-7 bg-background/40 border-white/5 text-[9px] font-mono"
                          />
                        </div>
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <div className="relative">
                          <Globe className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-foreground/10" />
                          <Input
                      ...
                      <div className="flex gap-2 pt-2">
                        <Button
                          ...
                          className="h-9 flex-1 bg-primary text-foreground text-[8px] font-black uppercase rounded-lg"
                        >
                          Connect
                        </Button>
                        {isCustomConnected && (
                          <Button
                            ...
                            className="h-9 w-9 p-0 border-red-500/20 text-red-500 hover:bg-red-500/10"
                          >
                ...
                <div className="space-y-3">
                  ...
                  <Slider
                    ...
                    step={1}
                    onValueChange={(v) =>
                      setConfig({ ...config, temperature: v[0] / 100 })
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className="h-24 bg-background/40 border-border text-[10px] leading-relaxed resize-none p-4"
                  />
                </div>
                ...
                <div className="pt-4 grid grid-cols-2 gap-2">
                  <Button
                    ...
                    className="h-9 text-[8px] font-black uppercase rounded-xl border-white/5 bg-white/5"
                  >
                ...
                <button
                  onClick={() => {
                    ...
                    setActivePersona("helper");
                  }}
                  className="w-full h-8 text-[8px] font-black uppercase text-foreground/20 hover:text-primary transition-colors border-t border-white/5 pt-2"
                >
        ...
        {/* Input Matrix */}
        <div className="p-4 sm:p-5 border-t border-white/5 bg-background] shrink-0 max-md:pb-1 max-md:pt-2 max-md:px-2">
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar pb-1 px-1">
              {[
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 text-[8px] font-black uppercase text-foreground/30 hover:text-primary hover:border-primary/20 transition-all whitespace-nowrap"
                >
                  {t.label}
                </button>
              ))}
              ...
              <div className="absolute right-1.5 bottom-1.5">
                {isProcessing ? (
                  <Button
                    type="button"
                    onClick={handleStop}
                    className="h-8 w-8 rounded-xl bg-destructive text-foreground shadow-xl shadow-destructive/30 active:scale-95 transition-all p-0"
                  >
                  ...
                  <Button
                    type="submit"
                    disabled={!input.trim()}
                    className="h-8 w-8 rounded-xl bg-primary text-foreground shadow-xl shadow-primary/30 active:scale-95 transition-all group/btn p-0"
                  >
      ...
      >
        <AlertDialogContent className=" border-white/10 rounded-[2.5rem] p-8 max-w-sm">
          <AlertDialogHeader className="space-y-4">
            ...
            <AlertDialogDescription className="text-[11px] font-medium text-foreground/40 uppercase tracking-widest leading-relaxed text-center">
              This will definitively purge the linguistic registry for this
              specific discussion.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 flex gap-3">
            ...
            <AlertDialogAction
              ...
              className="h-12 flex-1 rounded-xl bg-destructive text-foreground font-black uppercase text-[9px] shadow-xl"
            >
              Purge
            </AlertDialogAction>
          </AlertDialogFooter>
      ...
      <style jsx global>{`
        ...
        .markdown-content code {
          @apply bg-background/40 px-1.5 py-0.5 rounded text-primary font-mono text-[13px] border border-white/5;
        }
        .markdown-content pre {
          @apply bg-background/60 p-4 rounded-xl mb-3 overflow-x-auto border border-white/5;
        }
        .markdown-content pre code {
          @apply bg-transparent p-0 border-none;
        }
        .markdown-content blockquote {
          @apply border-l-2 border-primary/40 pl-4 italic text-foreground/60 mb-3;
        }
        .markdown-content a {
